          ...
          return {
            ...
            now_who:
              val.po_to_supplier === 1
                ? 'PO Send to Supplier'
                : val.po_approva_level_two === 1
                ? 'PO Managing Director Approved'
                : val.po_approva_level_one === 1
                ? 'PO Purchase Level Approved'
                : val.po_complete === 1
                ? 'PO Completed'
                : val.po_prepartion === 1
                ? 'PO Preparing'
...
export const PurchaseQuatanNegotain = getArray => {
  return new Promise((resolve, reject) => {
    try {
      const result = getArray
        ?.filter(val => val.quatation_calling_status === 1 && val.quatation_negotiation === 0)
        .map(val => {
          return {
            req_slno: val.req_slno,
            actual_requirement: val.actual_requirement,
...
export const potoSupp = getArray => {
  return new Promise((resolve, reject) => {
    try {
      const result = getArray
        ?.filter(val => val.po_complete === 1 && val.po_to_supplier === 0)
        .map(val => {
          return {
            req_slno: val.req_slno,
            actual_requirement: val.actual_requirement,
...
export const PurchDataCollPendingList = datacollPendng => {
  return new Promise((resolve, reject) => {
    try {
      const result = datacollPendng
        ?.filter(val => val.md_approve === 1 && val.ed_approve === 1)
        .map(val => {
          return {
            req_slno: val.req_slno,
            actual_requirement: val.actual_requirement,
