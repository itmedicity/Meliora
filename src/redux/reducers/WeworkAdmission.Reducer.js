...
const { FETCH_TOTAL_ADMISSION,
    FETCH_TOTAL_DAMA,
    FETCH_TOTAL_BHRC,
    FETCH_TOTAL_DOCVISIT,
    FETCH_TOTAL_DISAFTERNOON } = ActionTyps
//intial state
...
export const getTotalAdmission = (state = Dashboard, { type, payload, status }) => {
    switch (type) {
        ...
        case FETCH_TOTAL_DOCVISIT:
            return { ...state, 78: { slno: 78, name: "Doctor Rounds after 2 pm", count: payload, status: status } }
        case FETCH_TOTAL_DISAFTERNOON:
            return { ...state, 79: { slno: 79, name: "Discharge after 2 pm", count: payload, status: status } }
        default:
            return state
    }
