...
export const getAllDeptopenWithIn = async searchMonthlyOpenTimeRange => {
  return axioslogin.post('/Ticketdashboard/AllDeptopenWithIn', searchMonthlyOpenTimeRange).then(res => {
    const { success, data } = res.data
    if (success === 2) {
      return data
    }
...
export const getAllDeptcloseWithIn = async searchMonthlyOpenTimeRange => {
  return axioslogin.post('/Ticketdashboard/AllDeptcloseWithIn', searchMonthlyOpenTimeRange).then(res => {
    const { success, data } = res.data
    if (success === 2) {
      return data
    }
