...
const ScrapYardMastTable = ({ RowSelect }) => {

    const getCondemScrapYard = async () => {
        const { data } = await axioslogin.get('condemMasters/ScrapYardView')
        if (data.success === 2) {
            return data.data
        } else {
            return
                        ...
                        <tbody>
                            {tableData?.map((val, index) => (
                                <tr key={val.yard_slno || index}>
                                    ...
                                    <td style={{ width: 100, textAlign: 'center' }}>{val.yard_status === 1 ? 'Active' : 'Inactive'}</td>
                                </tr>
                            ))}
                        </tbody> :
                        <tbody>
...
export default ScrapYardMastTable
