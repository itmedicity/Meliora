...
const DepartmentMaping = () => {
    ...
    const { data: authDatakmc, isLoading: isAuthLoadingkmc, error: authErrorkmc } = useQuery({
        ...
        <CssVarsProvider>
            ...
            >
                ...
                <Box sx={{ px: 1, pt: 0.2, flex: 1.5, width: "50%", mt: 1 }}>
                    ...
                    >
                        <Option value={0} disabled  >Hod Name</Option>
                        {
                            authData && authData?.map((val, index) => (
                ...
                <Box sx={{ px: 1, pt: 0.2, flex: 1.5, width: "50%", mt: 1 }}>
                    <Select
                        ...
                        sx={{ height: 24, p: 0, m: 0, lineHeight: 1.200 }}
                    >
                        <Option value={0} disabled  >Incharge Name</Option>
                        {
                            authData && authData?.map((val, index) => (
                                <Option key={index} value={val.emp_id}>
                                    {val.em_name}
                ...
                <Box sx={{ px: 1, pt: 0.2, flex: 1.5, width: "50%", mt: 1 }}>
                    ...
                    >
                        <Option value={0} disabled  >Hod Name</Option>
                        {
                            authDatakmc && authDatakmc?.map((val, index) => (
                                <Option key={index} value={val.emp_id}>
                                    {val.em_name}
                                </Option>
                            ))
