...
const TaskDuedateCount = () => {
    ...
    const updateDuecount = useCallback((e) => {
    ...
    useEffect(() => {
        if (tm_duedate_count !== '') {
            const newCutoffPercentages = Array.from({ length: parseInt(tm_duedate_count) || 0 }, (_, i) => cutoffPercentages[i] || '');
                    ...
                    <Box sx={{ flex: 1, display: 'flex', m: 2 }}>
                        <Card sx={{ flex: 1.3, borderRadius: 10, mx: 5 }}>
                            <Box>
                                ...
                                <Box sx={{ flex: 1, mt: .8 }}>
                                    {tm_duedate_count !== '' ?
                                        <Typography sx={{ fontSize: 13, color: 'darkred' }}>CutOff Percentage(%) While Changing Duedates</Typography> :
                                        null}
                                        ...
                                        <Box sx={{ flex: 1, display: 'flex', mt: 1 }} key={index}>
                                            ...
                                            <Box sx={{ flex: 1 }}>
                                                <Box sx={{ flex: 1, display: 'flex', color: 'black' }}>
                                                    ...
                                                    />
                                                    <Typography sx={{ pt: .5, pl: .5, color: 'black' }}>%</Typography>
                                                </Box>
                                            </Box>
                                        </Box>
                                    ))}
                                ...
                                <Box sx={{ display: 'flex', pb: .5 }}>
                                    ...
                                    <Typography sx={{ fontWeight: 600, color: '#0B6BCB', cursor: 'pointer' }} onClick={submit}>Save</Typography>
                                </Box>
                            </Box>
                        </Card>
                        <Box sx={{ flex: 2 }}></Box>
