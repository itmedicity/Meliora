...
const CrfSMOApprovalMain = () => {
    ...
    const smoData = useMemo(() => smoDetails, [smoDetails]);
    useEffect(() => {
        if (radiovalue === '1') {
            if (smoData) {
                setcombinedData(smoData)
            }
            else {
                // setcombinedData([])
            }
        ...
        if (combinedData.length !== 0) {
            const datas = combinedData?.map((val) => {
                const obj = {
                    ...
                    senior_manage_remarks: val.senior_manage_remarks !== null ? val.senior_manage_remarks : "",
    ...
    if (isSmoLoading || isCompLoading) return <p>Loading...</p>;
    if (smoError || compError) return <p>Error occurred.</p>;
    return (
        <Fragment>
            {ApprovalFlag === 2 ?
            ...
            <Box sx={{ height: window.innerHeight - 80, flexWrap: 'wrap', bgcolor: 'white', }}>
                <Box sx={{ display: 'flex', backgroundColor: "#f0f3f5", border: '1px solid #B4F5F0' }}>
                    <Box sx={{ fontWeight: 550, flex: 1, pl: 1, pt: .5, color: '#385E72', }}>{company?.smo_status_name} Approval</Box>
                ...
                <Box>
                    <TopViewDesignForAll pendingData={smoData} radiovalue={radiovalue} setRadioValue={setRadioValue}
                        allData={allData} setDisData={setDisData} getPendingData={getPendingData}
...
export default memo(CrfSMOApprovalMain)