...
const CrfEDApprovalMain = () => {
    ...
    const { data: edDetails, isLoading: isEdLoading, error: edError } = useQuery({
        ...
        if (combinedData.length !== 0) {
            const datas = combinedData?.map((val) => {
                const obj = {
                    ...
                    higher: (val.ed_approve === null || val.ed_approve === 2 || val.ed_approve === 3
    ...
    if (isEdLoading) return <p>Loading...</p>;
    if (edError) return <p>Error occurred.</p>;
    return (
        <Fragment>
            {ApprovalFlag === 2 ?
            ...
            {cancelFlag === 1 ? <CrfDMSClose open={cancelModal} handleCloseCrfClose={handleCloseCrfClose} reqItems={reqItems}
                cancelData={cancelData} cancelledOne={'ED'} setCancelModal={setCancelModal} setCancelFlag={setCancelFlag}
            ...
            <Box sx={{ height: window.innerHeight - 80, flexWrap: 'wrap', bgcolor: 'white', }}>
                ...
                <Box>
                    <TopViewDesignForAll pendingData={edData} radiovalue={radiovalue} setRadioValue={setRadioValue}
