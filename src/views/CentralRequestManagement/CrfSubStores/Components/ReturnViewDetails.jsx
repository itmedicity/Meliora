    ...
    const returnReplyUpdate = useCallback(() => {
        const patchData = returnModalData?.map((val) => {
            return {
                ...
                store_return_received_remarks: val.received_status_return === 0 ? val.storeRemarks : val.store_return_received_remarks,
                received_user: val.received_status_return === 0 ? empid : val.received_user,
                received_date: val.received_status_return === 0 ? format(new Date(), 'yyyy-MM-dd HH:mm:ss') : format(new Date(val.received_date), 'yyyy-MM-dd HH:mm:ss'),
                        ...
                        <Box sx={{
                            ...
                            <Box sx={{ width: '100%', pt: 0.5, overflow: 'auto' }}>
                                <Box display="flex" justifyContent="space-between" sx={{
                                    ...
                                    <Typography sx={{ width: 70, textAlign: 'center', fontWeight: 550, fontSize: 13, my: 0.5, pl: 1 }}>Sl.No</Typography>
                                    <Typography sx={{ width: 250, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'left' }}>Item Description</Typography>
                                    <Typography sx={{ width: 250, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'left' }}>Reason</Typography>
                                    <Typography sx={{ width: 170, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'left' }}>User</Typography>
                                    <Typography sx={{ width: 200, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'left' }}>Return Date</Typography>

                                    {receivedStatus === true ?
                                        <>
                                            <Typography sx={{ width: 170, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'left' }}>Received User</Typography>
                                            <Typography sx={{ width: 200, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'left' }}>Received Date</Typography>
                                            <Typography sx={{ width: 250, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'lrft' }}>Received Remarks</Typography>
                                        </> : null}
                                    {receivedStatus === true > 0 ?
                                        <Typography sx={{ width: 300, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'center' }}>Replaced Remarks</Typography>
                                        :
                                        <Typography sx={{ width: 300, fontWeight: 550, fontSize: 13, my: 0.5, textAlign: 'center' }}>Received Remarks</Typography>
                                    }
                                    ...
                                    itemContent={(index, val) => (
                                        <React.Fragment key={val.item_return_slno}>
                                            <Box display="flex" justifyContent="space-between" sx={{ borderBottom: '1px solid lightgrey', flexWrap: 'nowrap', cursor: 'pointer' }}>
                                                <Typography sx={{ width: 70, textAlign: 'center', fontSize: 12, pt: 1.2 }}>{index + 1}</Typography>
                                                <Typography sx={{ width: 250, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{val.item_name}</Typography>
                                                <Typography sx={{ width: 250, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{val.return_remarks}</Typography>
                                                <Typography sx={{ width: 170, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{val.return_user} </Typography>
                                                <Typography sx={{ width: 200, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{format(new Date(val.return_date), 'dd-MM-yyyy hh:mm a')} </Typography>
                                                {val.received_status_return === 1 ?
                                                    <>
                                                        <Typography sx={{ width: 170, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{val.item_received_user} </Typography>
                                                        <Typography sx={{ width: 200, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{format(new Date(val.received_date), 'dd-MM-yyyy hh:mm a')} </Typography>
                                                        <Typography sx={{ width: 250, fontSize: 12, pt: 1.2, textAlign: 'left' }}>{val.store_return_received_remarks}</Typography>
                                                    </>
                                                    : null}
                                                <Box
                                                    sx={{
