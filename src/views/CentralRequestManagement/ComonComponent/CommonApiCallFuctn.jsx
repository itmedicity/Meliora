...
export const getAllApprovalPending = async (setDisData, setTableData, data) => {
    const newData = data?.map((val) => {
        const obj = {
            ...
            now_who: val.managing_director_approve !== null ? 'Managing Director' :
...
export const getApprovalKMCH = async (setcombinedData, pData) => {
    try {
        const result = await axioskmc.post('/newCRFRegister/getPendingList', pData);
        const { success, data } = result.data;
        setcombinedData(success === 1 ? data : []);
    } catch (error) {
        warningNotify("An error occurred during approval");
...
export const getOnholdRejectKMCH = async (setDisData, setAllData, pData) => {
    try {
        const result = await axioskmc.post('/newCRFRegister/getHoldRejectItems', pData);
        const { success, data } = result.data;
