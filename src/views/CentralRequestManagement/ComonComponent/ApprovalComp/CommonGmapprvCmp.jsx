...
const CommonGmapprvCmp = ({ DetailViewData, selectedCompany }) => {
    ...
    const ViewGMUploadImage = useCallback(() => {
        if (selectedCompany === '1') {
            ...
            const getImage = async (req_slno) => {
                const result = await axioskmc.get(`/newCRFRegisterImages/crfGMImageGet/${req_slno}`)
                const { success, data } = result.data
                if (success === 1) {
                    const fileNames = data;
                    const fileUrls = fileNames.map((fileName) => {
                        return `${PUBLIC_NAS_FOLDER_KMC}/CRF/crf_registration/${req_slno}/GMUpload/${fileName}`;
                    });
                    setImageArry(fileUrls);
                    setImageShowFlag(1)
                    setImageShow(true)
    ...
    return (
        <Fragment>
            ...
            <Paper variant="outlined" sx={{ overflow: 'auto', flexWrap: 'wrap' }}>
                <Box sx={{ display: 'flex', pt: 0.5, borderBottom: '1px solid lightgrey' }}>
                    ...
                    <Box sx={{ flex: 1, py: 0.4, ml: 2 }}>
                        <Chip size="md" variant="outlined" sx={{
                            color: (gm_approve === 1 ? '#2e7d32' : gm_approve === 2 ? '#bf360c' : gm_approve === 3 ? '#FF9800' : gm_approve === 4 ? '#009688' : '#607D8B'),
                ...
                {/* {(gm_approve === 1 || gm_approve === 2 || gm_approve === 3) ?
                    <Divider flexItem sx={{ borderBlockColor: 'grey' }}></Divider>
                    : null} */}
                <Box sx={{ pt: 0.1 }}>
                    {gm_approve === 1 && gm_approve_remarks !== null ?
                        <Box sx={{ pt: 0.5 }}>
                            <Box sx={{ display: 'flex' }}>
                                ...
                                <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, flex: 1, pr: 0.5, pt: 0.3 }}>
                                    {gm_approve_remarks === null ? 'Not Updated' : gm_approve_remarks}  </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', pt: 1 }}>
                                <Typography sx={{ pl: 1, fontSize: 14, flex: 0.4 }}>Detailed Analysis of Requirement</Typography>
                                <Typography sx={{ pl: 0.5 }} >  :&nbsp;</Typography>
                                <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, flex: 1, pr: 0.5, pt: 0.3 }}>
                                    {gm_detial_analysis === null ? 'Not Updated' : gm_detial_analysis}</Typography>
                            </Box>
                        </Box>
                        :
                        gm_approve === 2 && gm_approve_remarks !== null ?
                            ...
                            : gm_approve === 3 && gm_approve_remarks !== null ?
                                ...
                                : gm_approve === 4 && gm_approve_remarks !== null ?
                                    <Box sx={{ display: 'flex', pt: 0.5 }}>
                            ...
                            <Box sx={{ display: 'flex', py: 1 }}>
                                ...
                                <Box sx={{ display: 'flex', flex: 1 }}>
                                    ...
                                    <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, pt: 0.3, pl: 0.2 }}>
                                        {capitalizeWords(gm_user)}</Typography>
                                    <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, pl: 2, pt: 0.3 }}>
                                        {format(new Date(gm_approv_date), 'dd-MM-yyyy hh:mm:ss a')}</Typography>
                                    {gm_image === 1 ?
...
export default memo(CommonGmapprvCmp)