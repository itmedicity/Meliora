...
const UserReceivedItemDetails = ({ req_slno }) => {
    ...
    const { data: retunDetails, isLoading: isReturnLoading, error: returnError } = useQuery({
    ...
    return (
        <Fragment>
            {returnFlag === 1 ? <ItemReturnViewModal open={returnModal} handleClose={handleClose} modalData={modalData}
                itemName={itemName} /> : null}
            {combinedData && combinedData.length > 0 ?
                <Box sx={{ overflow: 'auto', flexWrap: 'wrap', px: 0.5 }}>
                    <Typography sx={{ fontWeight: 'bold', mx: 1, py: 0.5, color: '#145DA0', fontSize: 14 }}>
                        Received Items</Typography>
                    <Table aria-label="table with sticky header" borderAxis="both" padding={"none"} stickyHeader size='sm' >
                        <thead style={{ height: 4 }} size='small'>
                            <tr style={{ height: 4 }} size='small'>
                                ...
                                <th size='sm' style={{ borderRadius: 0, width: 70, textAlign: 'center', backgroundColor: '#e3f2fd', fontSize: 13 }}>Received Status(Store)</th>
                                <th size='sm' style={{ borderRadius: 0, width: 70, textAlign: 'center', backgroundColor: '#e3f2fd', fontSize: 13 }}>Receive/Return(User)</th>
                                {retunDetails.length > 0 ? <th size='sm' style={{ width: 40, textAlign: 'center', backgroundColor: '#e3f2fd', fontSize: 13 }}></th> : null}
                            </tr>
                        </thead>
                        <tbody>
                            {combinedData?.map((item, ind) => (
                                <tr key={item.po_itm_slno}>
                                    ...
                                    <td style={{ textAlign: 'center', color: (item.user_received_status === 1) ? '#59981A' : item.user_received_status === 0 ? '#e65100' : '#0288d1', }}>
                                        {item.user_received_status === 1 ? 'Received' : item.user_received_status === 0 ? 'Return' : 'Not Updated'}</td>
                                    <td style={{ textAlign: 'center', fontSize: 13 }}>
                                        {item.returnDetails && item.returnDetails.length > 0 ?
                                            <CustomToolTipForCRF title="Return Details" placement='top'>
                                                < AssignmentReturnedIcon
                                                    sx={{ color: (item.returnDetails.length > 0 ? '#ff8a80' : 'grey'), cursor: 'pointer' }}
