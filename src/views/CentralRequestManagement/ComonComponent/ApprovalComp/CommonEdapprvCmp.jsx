...
const CommonEdapprvCmp = ({ DetailViewData, company }) => {
  const { req_slno, ed_approve, ed_approve_remarks, ed_detial_analysis, ed_approve_date, ed_user, ed_image, ed } =
    DetailViewData
    ...
    <Fragment>
      ...
      <Paper variant="outlined" sx={{ overflow: 'auto', flexWrap: 'wrap' }}>
        ...
        <Box sx={{ pt: 0.1 }}>
            ...
            <Box sx={{ pt: 0.5 }}>
              <Box sx={{ display: 'flex' }}>
                ...
                <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, flex: 1, pr: 0.5, pt: 0.3 }}>
                  {ed_approve_remarks === null ? 'Not Updated' : ed_approve_remarks}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', pt: 1 }}>
                ...
                <Typography sx={{ pl: 0.5 }}> :&nbsp;</Typography>
                <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, flex: 1, pr: 0.5, pt: 0.3 }}>
                  {ed_detial_analysis === null ? 'Not Updated' : ed_detial_analysis}
                </Typography>
              </Box>
            </Box>
          ...
          ) : ed_approve === 4 && ed_approve_remarks !== null ? (
            <Box sx={{ display: 'flex', pt: 0.5 }}>
            ...
            <Box sx={{ display: 'flex', py: 1 }}>
              ...
              <Box sx={{ display: 'flex', flex: 1 }}>
                ...
                <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, pt: 0.3, pl: 0.2 }}>
                  {capitalizeWords(ed_user)}
                </Typography>
                <Typography sx={{ height: 'auto', fontSize: 13, fontWeight: 550, pl: 2, pt: 0.3 }}>
                  {format(new Date(ed_approve_date), 'dd-MM-yyyy hh:mm:ss a')}
                </Typography>
                {ed_image === 1 ? (
