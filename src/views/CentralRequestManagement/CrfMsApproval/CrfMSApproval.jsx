...
const CrfMSApproval = () => {
    ...
    const { data: msDetails, isLoading: isMsLoading, error: MsError } = useQuery({
        queryKey: ['getPendingAll', JSON.stringify(postData)],
        queryFn: () => getCRFPendingAboveHOD(postData),
        enabled: !!postData.level,
        staleTime: Infinity
    });
    const msData = useMemo(() => msDetails, [msDetails]);
    useEffect(() => {
        if (radiovalue === '1') {
            if (msData) {
                setcombinedData(msData)
            }
            else {
                setcombinedData([])
            }
        ...
        if (combinedData.length !== 0) {
            const datas = combinedData?.map((val) => {
                const obj = {
                    ...
                    ms_approve_remark: val.ms_approve_remark !== null ? val.ms_approve_remark : "",
    ...
    if (isMsLoading || isCompLoading) return <p>Loading...</p>;
    if (MsError || compError) return <p>Error occurred.</p>;
    return (
        <Fragment>
            {ApprovalFlag === 2 ?
                ...
                : ApprovalFlag === 1 ?
                    <CrfMSApprovalModal open={ApprovalModal} ApprovalData={ApprovalData} handleClose={handleClose}
                        ...
                        datacolflag={datacolflag} datacolData={datacolData} imagearray={imagearray} company={company} />
                    : null
            }
            ...
            <Box sx={{ height: window.innerHeight - 80, flexWrap: 'wrap', bgcolor: 'white', }}>
                <Box sx={{ display: 'flex', backgroundColor: "#f0f3f5", border: '1px solid #B4F5F0' }}>
                    <Box sx={{ fontWeight: 550, flex: 1, pl: 1, pt: .5, color: '#385E72', }}>{company?.ms_status_name} Approval</Box>
                ...
                <Box>
                    <TopViewDesignForAll pendingData={msData} radiovalue={radiovalue} setRadioValue={setRadioValue}
