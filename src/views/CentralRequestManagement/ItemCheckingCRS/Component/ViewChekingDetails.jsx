...
const ViewChekingDetails = ({ setViewFlag }) => {
    ...
    const { data: supplier, isLoading: isSupLoading, error: supError } = useQuery({
    ...
    const { data: checkData, isLoading: isItemCheckLoading, error: itemCheckError } = useQuery({
                            ...
                            <Box sx={{ flex: 2, display: 'flex' }}>
                                <Box sx={{ flex: 1, pl: 0.5, flexWrap: 'wrap', }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                        ...
                                        <Typography sx={{ fontSize: 13, color: '#1D617A', fontWeight: 550, pl: 0.3 }}>Item Check Date</Typography>
                                        <Box sx={{ pl: 1 }}>
                                            <CusCheckBox
                                                variant="outlined"
                                                size="md"
                    ...
                    {tableData && tableData.length > 0 ?
                        <Box sx={{ width: '100%' }}>
                            <Box display="flex" justifyContent="space-between" sx={{
                                ...
                                <Typography sx={{ width: 170, textAlign: 'left', fontWeight: 550, fontSize: 12 }}>Date & Time</Typography>
                                ...
                                <Typography sx={{ width: 120, textAlign: 'center', fontWeight: 550, fontSize: 12 }}>Delivered Qty</Typography>
                                ...
                                <Typography sx={{ width: 100, textAlign: 'center', fontWeight: 550, fontSize: 12, }}>Balance Qty</Typography>
                                <Typography sx={{ width: 150, textAlign: 'left', fontWeight: 550, fontSize: 12 }}>Goods Checker</Typography>
                                ...
                                itemContent={(index, val) => (
                                    <React.Fragment key={val.index}>
                                        <Box display="flex" justifyContent="space-between" sx={{ borderBottom: '1px solid lightgrey', flexWrap: 'nowrap' }}>
                                            ...
                                            <Typography sx={{ width: 170, textAlign: 'left', fontSize: 12, my: 1 }}>{format(new Date(val.checked_date), 'dd-MM-yyyy hh:mm:ss a')}</Typography>
                                            <Typography sx={{ width: 250, textAlign: 'left', fontSize: 12, my: 1 }}>{val.supplier_name}</Typography>
                                            <Typography sx={{ width: 80, textAlign: 'left', fontSize: 12, my: 1 }}>{val.item_code}</Typography>
                                            <Typography sx={{ width: 250, textAlign: 'left', fontSize: 12, my: 1 }}>{val.item_name}</Typography>
                                            {/* <Typography sx={{ width: 120, textAlign: 'center', fontSize: 12, my: 1 }}>{val.requested_qty}</Typography> */}
                                            <Typography sx={{ width: 120, textAlign: 'center', fontSize: 12, my: 1 }}>{val.delivered_qty} </Typography>
                                            <Typography sx={{ width: 100, textAlign: 'center', fontSize: 12, my: 1 }}>{val.excess_qty}</Typography>
                                            <Typography sx={{ width: 100, textAlign: 'center', fontSize: 12, my: 1 }}>{val.damage_qty}</Typography>
                                            <Typography sx={{ width: 100, textAlign: 'center', fontSize: 12, my: 1 }}>{val.balance_qty}</Typography>
                                            ...
                                            <Typography sx={{ width: 150, textAlign: 'left', fontSize: 12, my: 1 }}>{val.checking_user}</Typography>
