...
const CrfStoreProcessMain = () => {
  ...
  useEffect(() => {
    if (storeReceivePending.length !== 0) {
      const newArray = storeReceivePending.filter(
        (val, index, self) =>
          index ===
          self.findIndex(
            (value) =>
    ...
    if (stroreReceiveAll.length !== 0) {
      ...
      const fCount = stroreReceiveAll.filter(
        (val, index, self) =>
          index ===
          self.findIndex(
            (value) =>
  ...
  return (
    <Fragment>
      ...
      {modFlag === 1 ? (
        <GrnDetailsViewModal
          handleCloseGrn={handleCloseGrn}
          open={modalopen}
          grnData={grnDetailsView}
          poNumber={poNumber}
        />
      ) : null}
