...
import { getCompletedCRF, getDeliveryMarking, getItemChecking, getStoreAcknow, getUserAcknow } from 'src/api/CommonApiCRF'
...
const CRFStoreStatus = ({ storeData, companyData }) => {
    ...
    const { data: storeAck, isLoading: isStoreAckLoading, error: storeAckError } = useQuery({
    ...
    const { data: compCrf, isLoading: isComCRFoading, error: comCRFError } = useQuery({
    ...
    useEffect(() => {
        if (storeDetails.length !== 0) {
            ...
            setStrApprv({
                ...
                itemcheck: {
                    pending: itemCheckData ? `${new Set(itemCheckData?.map(item => item.checking_item_slno)).size}/${new Set(itemCheckData?.map(item => item.checking_slno)).size}` : 0,
    ...
    if (isDeliveryLoading || isItemCheckLoading || isStoreAckLoading || isUserAckLoading || isComCRFoading) return <p>Loading...</p>;
