...
const CrfReqstTableView = ({ rowSelect }) => {
  ...
  return (
    <Box sx={{ height: window.innerHeight - 150, bgcolor: 'white' }}>
      ...
      <Box sx={{}}>
        {radiovalue === '1' ? (
          <NotReceivedTable disData={disData} rowSelect={rowSelect} company={company} />
        ) : radiovalue === '2' ? (
          <ItemReceivedInStoreTable storeData={storeData} company={company} />
        ) : radiovalue === '3' ? (
          <ReceivedTable receivedData={receivedData} company={company} />
        ) : null}
      </Box>
    </Box>
  )
