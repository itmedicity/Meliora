...
const StoreReceivedItemList = ({ storeReceived, empId, req_slno }) => {
    ...
    return (
        <Fragment>
            ...
            {returnFlag === 1 ? <ReturnModal returnData={returnData} handleClose={handleCloseReturn} open={returnModal}
                empId={empId} /> : null}
            {notifyFlag === 1 ? <ItemReturnViewModal open={notifyModal} handleClose={handleCloseReturn} modalData={modalData}
                ...
                <Table aria-label="table with sticky header" borderAxis="both" padding={"none"} stickyHeader size='sm' >
                    <thead style={{ height: 4 }} size='small'>
                        <tr style={{ height: 4 }} size='small'>
                            ...
                            <th size='sm' style={{ borderRadius: 0, width: 70, textAlign: 'center', backgroundColor: '#e3f2fd', fontSize: 13 }}></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        {combinedData?.map((item, ind) => (
                            <tr key={item.po_itm_slno}>
                                ...
                                <td style={{ color: (item.user_received_status === 1) ? '#59981A' : item.user_received_status === 0 ? '#e65100' : '#0288d1', }}>
