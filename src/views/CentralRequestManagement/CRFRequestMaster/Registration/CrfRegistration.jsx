...
const CrfRegistration = ({ editRowData, setEditRowData, edit, setEdit, detailDataDis, setDetailDataDis, imagearray, setImageArry }) => {
    ...
    const { location, item_qty, item_desc, unitprice, approx_cost, actual_require, needed, levelOne,
        levelTwo, deptType, emergency, remarks, reqDetalSlno, startdate, reqSlno, imageshowFlag, imageshow,
        item_brand, item_spec } = crfRegister
    ...
    }, [item_desc, item_brand, item_qty, uom, uomName, item_spec, unitprice, approx_cost, editIndex,
        reqDetalSlno, detailDataDis, setDetailDataDis])
        ...
        <Fragment>
            ...
            <Paper variant='outlined' square sx={{ height: window.innerHeight - 140, bgcolor: 'white' }}>
                <Box sx={{ display: 'flex', m: 0.5 }}>
                    {edit === 1 ?
                        <Typography sx={{ fontWeight: 'bold', color: '#145DA0', fontSize: 14, flex: 1, pl: 1 }}>CRF/TMC/{reqSlno}</Typography> : null}
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                    <Box sx={{ flex: 1, pl: 0.5 }}>
                        ...
                        {authorizeDeptSec && authorizeDeptSec.length !== 0 && authorizeDeptSec.length !== 1 ?
                            ...
                            <Box sx={{ pl: 0.5, pt: 0.2 }}>
                                <Box sx={{
                                    width: '100%', height: 38, border: '1px solid lightgrey', borderRadius: 7,
                                    p: 0.9, textAlign: 'left', fontSize: 13,
                                    // bgcolor: '#FBFCFE'
                                }}>
                                    {deptsecName}
                                </Box>
                    ...
                    <Box sx={{ flex: 2, pl: 0.3 }}>
                        ...
                        <Box sx={{
                            mt: 0.2, height: 38, overflow: 'auto', border: '1px solid lightgrey',
                            borderRadius: 7
                        }} >
                ...
                <Box sx={{ bgcolor: 'white' }}>
                    <Typography sx={{ p: 1, color: 'grey', pl: 1.5, fontSize: 15, }} >Estimate/Approximate/Requirement Details</Typography>
                            ...
                            <Box sx={{ flex: 0.2, display: 'flex' }}>
                                ...
                                <Box sx={{ pt: 3.3, pl: 0.5 }}>
                                    <Tooltip title="Clear All" placement="bottom" sx={{ bgcolor: '#ffebee', color: '#d50000' }} >
                                        <ClearIcon
                                            sx={{
                                                height: 25, width: 25, color: '#ef9a9a', cursor: "pointer",
                                                ":hover": {
                                                    color: '#e57373'
                                                }
                                            }}
                    ...
                    {detailDataDis.length !== 0 ?
                        <Box variant="outlined" sx={{ maxHeight: '50vh', overflow: 'auto', '&::-webkit-scrollbar': { height: 8 }, px: 0.5, pt: 0.3 }}>
                            <CssVarsProvider>
                                <Table aria-label="table with sticky header" borderAxis='both' padding={"none"} stickyHeader size='sm'>
                                    <thead style={{ alignItems: 'center' }}>
                                        <tr style={{ height: 0.5 }}>
                                            <th size='sm' style={{ width: 50, fontSize: 14, textAlign: 'center' }}>&nbsp; Sl.No</th>
                                            <th size='sm' style={{ width: 150, fontSize: 14 }}>&nbsp;Description</th>
                                            <th size='sm' style={{ width: 100, fontSize: 14 }}>&nbsp;Brand</th>
                                            <th size='sm' style={{ width: 80, fontSize: 14, textAlign: 'center' }}>&nbsp;Quantity</th>
                                            <th size='sm' style={{ width: 80, fontSize: 14, textAlign: 'center' }}>&nbsp;Unit </th>
                                            <th size='sm' style={{ width: 220, fontSize: 14 }}>&nbsp;Specification</th>
                                            <th size='sm' style={{ width: 85, fontSize: 14, textAlign: 'center' }}>&nbsp;Unit Price</th>
                                            <th size='sm' style={{ width: 95, fontSize: 14, textAlign: 'center' }}>&nbsp;Approx.Cost</th>
                                    ...
                                    <tbody size='small'>
                                        {detailDataDis?.map((val, index) => (
                                            <tr key={index} size='small' style={{ maxHeight: 2, cursor: 'pointer' }}>
                                                ...
                                                <td size='sm' style={{ fontSize: 13, }}>&nbsp;{val.item_desc}</td>
                                                <td size='sm' style={{ fontSize: 13, }}>&nbsp;{val.item_brand}</td>
                                                ...
                                                <td size='sm' style={{ fontSize: 13 }}>&nbsp;{val.item_spec}</td>
                            ...
                            {emergency === true ?
                                <Box sx={{ display: 'flex', flex: 1, flexWrap: 'wrap' }}>
                                    ...
                                    <Box sx={{ flex: 1.5 }}>
                                        ...
                                        <Box sx={{ pt: 0.5 }} >
                                            <Textarea
                                                type="text" size="sm" sx={{ bgcolor: 'white', height: 36 }}
                                                minRows={1} maxRows={1}
                                                style={{ width: "100%", }} placeholder="type here ..."
                                                name='remarks'
                                                value={remarks || ''}
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                        ...
                        <Box sx={{ py: 0.5, pr: 2 }}>
                            <ModalButtomCmp
                                handleChange={refreshWindow}
                            > Clear</ModalButtomCmp>
                        </Box>
                    </Box>
                </Box>
            </Paper >
