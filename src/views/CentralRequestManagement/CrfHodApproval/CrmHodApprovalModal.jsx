...
const CrmHodApprovalModal = ({ open, ApprovalData, reqItems, handleClose, setApproveTableData, approveTableData,
    ...
    const [apprvlDetails, setApprvlDetails] = useState({
        approve: hod_approve === 1 ? true : false,
        reject: hod_approve === 2 ? true : false,
        pending: hod_approve === 3 ? true : false,
            ...
            if ((approve && detailAnalis && remark) || ((reject || pending) && remark)) {
                updateHODApproval(HodPatchData).then((val) => {
                    ...
                    const onSuccess = (fileUploadMessage) => {
                        ...
                        reset();
                        queryClient.invalidateQueries(['inchargeHodCrfList', JSON.stringify(deptsecArry)]);
                    };
    ...
    }, [approve, reject, pending, remark, HodPatchData, queryClient, reset, datacollFlag, datacolectremark,
        crfdept, id, req_slno, selectFile, detailAnalis, handleImageUpload, deptsecArry, editEnable]);
