...
import { getCRFPendingAboveHOD, getDefaultCompany } from 'src/api/CommonApiCRF'
...
const CrfMOApprovalMain = () => {
    ...
    const { data: moDetails, isLoading: isMoLoading, error: moError } = useQuery({
        ...
        if (combinedData.length !== 0) {
            ...
            if (radiovalue === '2') {
                const newData = datas?.filter((val) => val.now_who_status !== 2 && val.now_who_status !== 3 &&
                    (val.manag_operation_approv !== null || val.senior_manage_approv !== null || val.gm_approve !== null ||
    ...
    if (isMoLoading || isCompLoading) return <p>Loading...</p>;
    if (moError || compError) return <p>Error occurred.</p>;
    return (
        <Fragment>
            {ApprovalFlag === 2 ?
                ...
                : ApprovalFlag === 1 ?
                    <CrfMOApprovalModal open={ApprovalModal} ApprovalData={ApprovalData} handleClose={handleClose} reqItems={reqItems}
                        ...
                        datacolData={datacolData} imagearray={imagearray} company={company} />
                    : null
            }
            ...
            <Box sx={{ height: window.innerHeight - 80, flexWrap: 'wrap', bgcolor: 'white', }}>
                <Box sx={{ display: 'flex', backgroundColor: "#f0f3f5", border: '1px solid #B4F5F0' }}>
                    <Box sx={{ fontWeight: 550, flex: 1, pl: 1, pt: .5, color: '#385E72', }}> {company?.mo_status_name} Approval</Box>
                ...
                <Box>
                    <TopViewDesignForAll pendingData={moData} radiovalue={radiovalue} setRadioValue={setRadioValue}
                                ...
                                <Box key={index} sx={{
                                    ...
                                    {radiovalue === '8' ?
                                        ...
                                        <ApproveButtonsCompnt val={val} setApprovalFlag={setApprovalFlag}
                                            ...
                                            setCancelModal={setCancelModal} setApprovalData={setApprovalData} reqItems={reqItems}
                                            setCancelData={setCancelData} setReqItems={setReqItems} setPoDetails={setPoDetails}
                                            setApproveTableData={setApproveTableData} approveTableData={approveTableData}
