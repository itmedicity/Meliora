...
import CrfGMClose from './CrfGMClose'
...
const CrfGMApproval = () => {
    ...
    const updatedone = useCallback((e) => {
        if (e.target.checked === true) {
            ...
            const DoneList = DisData.filter((val) => {
                return val.crf_close !== 1 && val.gm_approve !== null || val.ed_approve !== null || val.md_approve !== null
            })
    ...
    useEffect(() => {
        if (tabledata.length !== 0) {
            const gmlist = tabledata.filter((val) => {
                return val.gm_approve_req === 1 && val.crf_close !== 1
            })

            const datas = gmlist.map((val) => {
                const obj = {
    ...
    return (
        <Fragment>
            ...
            /> : null}
            {cancelFlag === 1 ? <CrfGMClose open={cancelModal} setCancelData={setCancelData}
                setCancelFlag={setCancelFlag} setCancelModal={setCancelModal}
                count={count} setCount={setCount} cancelData={cancelData} /> : null}

            {ApprovalFlag === 1 ? <CrfGMApprovalModal open={ApprovalModal} ApprovalData={ApprovalData}
            ...
            <Box sx={{ height: 35, backgroundColor: "#f0f3f5", display: 'flex' }}>
                <Box sx={{ fontWeight: 550, flex: 1, pl: 1, pt: .5, color: '#385E72', }}>GM Operations Approval</Box>
                <Box>
...
export default memo(CrfGMApproval)