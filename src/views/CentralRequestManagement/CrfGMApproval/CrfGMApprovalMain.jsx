...
import CrfGMApprovalModal from './CrfGMApprovalModal'

const CrfGMApprovalMain = () => {
    ...
    const gmData = useMemo(() => gmDetails, [gmDetails]);
    useEffect(() => {
        if (radiovalue === '1') {
            if (gmData) {
                setcombinedData(gmData)
            }
            else {
                // setcombinedData([])
            }
        ...
        if (combinedData.length !== 0) {
            const datas = combinedData?.map((val) => {
                const obj = {
                    ...
                    gm_approve_remarks: val.gm_approve_remarks !== null ? val.gm_approve_remarks : "",
            ...
            if (radiovalue === '2') {
                const newData = datas?.filter((val) => val.now_who_status !== 2 && val.now_who_status !== 3 &&
                    (val.gm_approve !== null || val.ed_approve !== null || val.md_approve !== null || val?.internally_arranged_status === 1)
                )
                setDisData(newData)
                setAllData(newData)
            } else {
    ...
    if (isGmLoading || isCompLoading) return <p>Loading...</p>;
    if (gmError || compError) return <p>Error occurred.</p>;
    return (
        <Fragment>
            {ApprovalFlag === 2 ?
            ...
            <Box sx={{ height: window.innerHeight - 80, flexWrap: 'wrap', bgcolor: 'white', }}>
                <Box sx={{ display: 'flex', backgroundColor: "#f0f3f5", border: '1px solid #B4F5F0' }}>
                    <Box sx={{ fontWeight: 550, flex: 1, pl: 1, pt: .5, color: '#385E72', }}>{company?.gmo_status_name} Approval</Box>
                ...
                <Box>
                    <TopViewDesignForAll pendingData={gmData} radiovalue={radiovalue} setRadioValue={setRadioValue}
...
export default memo(CrfGMApprovalMain)