...
const PasswordManagement = () => {
  ...
  const rowSelect = useCallback((data) => {
    ...
    const getdetailtable = async (pswd_mast_slno) => {
      ...
      if (data.length !== 0) {
        if (success === 2) {
          const setDetailData = data && data.map((val) => {
            return {
  ...
  const { pswd_mast_location_name, pswd_mast_slno,
    pswd_mast_categry_no, pswd_mast_categry_name, pswd_mast_group_name,
    pswd_mast_item_name, pswd_mast_group_no, pswd_mast_item_no, pswd_mast_location, pswd_mast_description } = PasswordMast
  ...
  const searchAssetNo = useCallback((e) => {
    if (pswd_mast_asset_no === '') {
      ...
      const getAssetdata = async (postdata) => {
        ...
        if (data.length !== 0) {
          if (success === 1) {
            const { category_name, group_name, item_name, item_category_slno, item_creation_slno, item_group_slno,
              item_deptsec_slno, sec_name } = data[0]
            const formdata = {
  ...
  }, [arry, pswd_mast_item_no, credentialName, tableEdit, credential, description, pswd_detail_slno, user_name, password,
    port, remarks])
  ...
  }, [pswd_mast_categry_no, pswd_mast_group_no, pswd_mast_item_no, pswd_mast_asset_no, pswd_mast_location,
    pswd_mast_description, id])
  ...
  const { paswd_soft_slno, paswd_soft_webname, paswd_soft_linkname, paswd_soft_username, paswd_soft_password, paswd_soft_remarks } = SoftwarePswd
  ...
  }, [paswd_soft_slno, paswd_soft_webname, paswd_soft_linkname, paswd_soft_username, paswd_soft_password, paswd_soft_remarks, id])

  const rowSelectForSw = useCallback((data) => {
    setSwEdit(1)
    const {
      paswd_soft_slno,
      paswd_soft_webname,
  ...
  const submitPasswordData = useCallback(
        ...
        if (value === 0) {
          if (pswd_mast_categry_name !== '') {
            InsertPasswordMast(insertdata).then((value) => {
              ...
              if (success === 1) {
                const postForDetail = arry && arry.map((val) => {
                  return {
          ...
          UpdatePasswordMast(patchdataMast).then((value) => {
            ...
            succesNotify(message);
            if (success === 2) {
              const patchdataDetail = arry && arry.map((val) => {
                return {
                  pswd_detail_slno: val.pswd_detail_slno,
    ...
    [insertdata, reset, value, count, swTableCount, patchForSoftware, pswd_mast_categry_name, patchdataMast,
      postSoftwarePswd, flag, paswd_soft_username, addNewInUpdate, pswd_mast_slno, paswd_soft_password, paswd_soft_webname,
      resetSoftware, swEdit, arry, id],)
          ...
          flag === 1 ? (
            <Box sx={{ mt: 3 }}>
              <Box sx={{ display: 'flex', width: '70vw', margin: 'auto', }}>
                <Box sx={{
                  width: '50vw', display: 'flex', justifyContent: "flex-end"
                }}>
                  <Paper sx={{ height: 75, pt: .2, pl: 2, width: '33vw', }}>
                    <Box sx={{ pl: .5, color: '#055C9D', }}>
                      <Typography >
                        Asset number
                    ...
                    <Box sx={{ display: 'flex' }}>
                      ...
                      <Box sx={{ pl: 1, pt: .5 }}>
                        <Tooltip title="search" placement="top">
                          <SearchIcon sx={{ cursor: 'pointer', color: '#055C9D', fontSize: 26 }} onClick={searchAssetNo} />
                        </Tooltip>
                      </Box>
                    </Box>
                  </Paper>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', width: '18vw' }}>
                  <Box sx={{ width: 250, borderRadius: 2, border: .5, borderBlockColor: '#055C9D', }}>
                    ...
                    <Box sx={{ textAlign: 'center' }}>
                      <TableViewSharpIcon sx={{ color: '#145DA0', cursor: 'pointer', size: 'lg', width: 30, height: 30 }}
                        onClick={addModal} />
                    </Box>
                  </Box>
                </Box>
              ...
              <Box sx={{
                margin: 'auto', width: '47vw',
              }}>
                <Box sx={{ pt: 1, display: 'flex', margin: 'auto' }}>
                  <Box sx={{ textAlign: 'right', pr: 1, pt: .5, flex: .3, }} >
                    <Typography>
                      Category
                    </Typography>
                  </Box>
                ...
                <Box sx={{ pt: .5, display: 'flex', margin: 'auto' }}>
                  <Box sx={{ textAlign: 'right', pr: 1, pt: .5, flex: .3, }} >
                    <Typography>
                      Group
                    </Typography>
