...
const NextMonthTable = ({ nextmonthdata, setMonthflag }) => {
  ...
  const nextmonth = addDays(new Date(getEndofMonth), 1)
  return (
    <Fragment>
      <Paper>
                ...
                <tbody size="small" style={{ height: 8 }}>
                  {nextmonthdata?.map((val, index) => (
                    <tr key={val.time_slno} style={{ height: 8 }} size="small">
                      ...
                      <td>{val.backup_type_name}</td>\ <td>{val.backup_name}</td>
...
export default memo(NextMonthTable)
