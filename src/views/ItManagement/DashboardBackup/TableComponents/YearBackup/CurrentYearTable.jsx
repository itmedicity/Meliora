...
const CurrentYearTable = ({ yeartabData, setYeartabflag, yearflag, count, setCount }) => {
    ...
    const VerificationClick = useCallback((val) => {
        if (val.verify_status === 1) {
            ...
            if (yearflag === 1) {
                const getendOfYear = endOfYear(new Date(val.backup_yearly_date))
                if (getendOfYear === new Date()) {
                    setView(val)
                    setModalOpen(true)
                    setflag(1)
                }
                else {
                    infoNotify("Can't do it Now,It's Not the End of the Year")
                }
            }
            else {
                setView(val)
    ...
    return (
        <Fragment>
            <Paper>
                {flag === 1 ? < ModalYearVerification open={modalopen} handleClose={handleClose} rowSelect={view} count={count} setCount={setCount} /> :
                ...
                >
                    {yearflag === 1 ?
                        <Box sx={{ display: 'flex', flexDirection: 'row' }}>
                            <Box sx={{ flex: 1 }}>
                                <Typography sx={{ fontWeight: 10, fontSize: 17 }}>Current Year Details</Typography>
                            </Box>
                        ...
                        yearflag === 2 ?
                            <Paper>
                                <Typography sx={{ fontWeight: 10, fontSize: 18 }}>Yearly Backup Dues</Typography>
                            </Paper>
                            : yearflag === 3 ?
                                <Paper>
                                    <Typography sx={{ fontWeight: 10, fontSize: 18 }}>Yearly Backup Errors</Typography>
                                </Paper>
                                : null}
                            ...
                            <Table borderAxis="both" padding={"none"} stickyHeader >
                                <thead style={{ alignItems: 'center' }}>
                                    <tr>
                                        ...
                                        {yearflag === 1 ? <th rowSpan={2} style={{ width: 120, textAlign: 'center' }}>Backup Date</th> : null}
                                        {yearflag === 2 || yearflag === 3 ?
                                        ...
                                        <th rowSpan={2} style={{ width: 150, textAlign: 'center' }}>Backup Location</th>
                                        ...
                                        {yearflag === 3 ? <th rowSpan={2} style={{ width: 160, textAlign: 'center' }}>Date </th> : null}
                                        {yearflag === 3 ? <th rowSpan={2} style={{ width: 100, textAlign: 'center' }}>Employee </th> : null}
                                        {yearflag === 3 ? <th rowSpan={2} style={{ width: 100, textAlign: 'center' }}>Status</th> : null}
                                        {yearflag === 3 ? <th rowSpan={2} style={{ width: 150, textAlign: 'center' }}>Remarks</th> : null}
                                    </tr>
                                ...
                                <tbody size='small' style={{ height: 8 }}>
                                    {yeartabData?.map((val, index) => (
                                        <tr key={val.yearly_slno} size='small'
                                            ...
                                            {yearflag === 3 ? <td>{(val.verify_status === 1) ? 'Successfull' : (val.verify_status === 2) ? 'Not Successfull' : 'Not Updated'}</td> : null}
...
export default memo(CurrentYearTable)