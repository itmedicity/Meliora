...
import QuaterlyBillEditModal from '../DashBoardTariff/QuaterlyTarrif/QuaterlyBillEditModal';

const QuaterlyBillView = ({ setquarterly, setquarterlyBillTable,  billviewsl }) => {
 ...
 const [editFlag, setEditFalg] = useState(0) 
    ...
    const backtoSetting = useCallback(() => {     
      setquarterly(1)
      setquarterlyBillTable(0)
    }, [setquarterly, setquarterlyBillTable])
  
  useEffect(() => {
    const getCommunicationDevice = async (billviewsl) => {
      const result = await axioslogin.get(`/tarrifDetails/BillQuaterlyViewId/${billviewsl}`)
      ...
      if (success === 2) {
        ...
        const arr = data?.map((val) => {
          const obj = {
            ...
            sl_no_quaterly: val.sl_no_quaterly,
            device_slno_quaterly:val.device_slno_quaterly,
          }
          return obj
        })
...
export default memo(QuaterlyBillView)
