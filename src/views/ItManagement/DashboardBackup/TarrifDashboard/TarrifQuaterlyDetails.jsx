...
import QuaterlyBillAddModal from '../DashBoardTariff/QuaterlyTarrif/QuaterlyBillAddModal';
import QuaterlyBillView from './QuaterlyBillView';

const TarrifQuaterlyDetails = ({ setBackdashboard, setquarterly }) => {
    const [addModalOpen, setaddModalOpen] = useState(false)
  const [AddModalFlag, setAddModalFlag] = useState(0)
  const [quaterlyBillTable, setquarterlyBillTable] = useState(0);
  ...
  const QuaterlyBillTableView = useCallback((val) => {
    
    const {device_slno}=val
    setquarterlyBillTable(1);
  ...
  useEffect(() => {
    ...
    {quaterlyBillTable === 1 ? (
        <QuaterlyBillView
        setquarterly={setquarterly} setquarterlyBillTable={setquarterlyBillTable} billviewsl={billviewsl}
...
<Typography sx={{ fontWeight: 10, fontSize: 28, fontFamily: 'Anton',color:'#003060' }}>Quarterly Tarrif Details</Typography>
              ...
              >
                ...
                <td>
                      <ReceiptLongIcon sx={{ cursor: 'pointer' }} size={6}
                          onClick={() => QuaterlyBillTableView(val)}
                      />
                </td>
                <td> {index + 1}</td>
...
export default memo(TarrifQuaterlyDetails)