...
import YearlyBillEdits from '../DashBoardTariff/YearlyTarrif/YearlyBillEdits';

const YearlyBillView = ({ setyearly, setyearlyBillTable, billviewsl }) => {
 ...
 const [getarry,setgetarry]=useState([])
  ...
  const backtoSetting = useCallback(() => {
    ...
    setyearly(1)
    setyearlyBillTable(0)
  }, [setyearly, setyearlyBillTable])
  
  useEffect(() => {
    const getCommunicationDevice = async (billviewsl) => {
      const result = await axioslogin.get(`/tarrifDetails/BillYearlyViewId/${billviewsl}`)
  ...
  return (
    ...
    close={backtoSetting}
    >
      {AddModalFlag === 1 ? <YearlyBillEdits open={addModalOpen} handleClose={handleClose}
        setCount={setCount}
        count={count}
...
export default memo(YearlyBillView)