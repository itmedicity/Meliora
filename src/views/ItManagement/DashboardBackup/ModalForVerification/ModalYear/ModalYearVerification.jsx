...
const ModalYearVerification = ({ open, handleClose, rowSelect, count, setCount }) => {
  ...
  const SaveVerification = useCallback(
    e => {
      if (empname === 0) {
        ...
        const UpdateDetails = async patchdata => {
          const result = await axioslogin.patch('/verification/updateyear', patchdata)
    ...
    [patchdata, empname, count, setCount, verifyStatus, notverify, reset]
  )
  return (
    // <Fragment>
    //     <ToastContainer />
    ...
    //                                     slotProps={{
    //                                         input: {
    //                                             min: moment(startOfYear(new Date())).format('YYYY-MM-DD HH:mm:ss'),
    //                                             max: moment(endOfYear(new Date())).format('YYYY-MM-DD HH:mm:ss'),
    //                                         },
    //                                     }}
    //                                     style={{ width: 372 }}
    //                                     size="sm"
      ...
      >
        <ModalDialog variant="outlined" sx={{ p: 2, width: 800 }}>
          <Box sx={{ flex: 1, display: 'flex' }}>
            ...
            <Typography sx={{ fontSize: 19, pl: 0.5, flex: 1 }}> Backup Verification</Typography>
          ...
          >
            ...
            <Box sx={{ flex: 1, display: 'flex', pl: 0.5 }}>
              <Box sx={{ width: 220, color: '#274472', fontSize: 15 }}>Backup Name</Box>
              <Typography sx={{ fontSize: 14, color: '#274472' }}>:&nbsp;{backup_name} </Typography>
            </Box>
            <Box sx={{ flex: 1, display: 'flex', pl: 0.5 }}>
            ...
            <Typography sx={{ fontSize: 16, pl: 0.5, pt: 0.5 }}> Backup Device Details </Typography>
            ...
            <Box sx={{ flex: 1, pt: 1.5 }}>
              <Typography sx={{ pl: 0.5, fontWeight: 500, fontSize: 14 }}> Backup Path</Typography>
              <TextFieldCustom
                placeholder="Folder Path"
                type="text"
                size="sm"
...
export default memo(ModalYearVerification)
