...
const DaysVerificationModal = ({ open, handleClose, rowSelect, count, setCount }) => {
  const [backupslno, setBackupslno] = useState(0)
  ...
  const SaveVerification = useCallback(
    e => {
      if (empname === 0) {
        ...
        const UpdateDetails = async patchdata => {
          const result = await axioslogin.patch('/verification/updatedays', patchdata)
          return result.data
        }
    ...
    [patchdata, empname, count, insertdata, notverify, verifyStatus, setCount, reset]
  )
  return (
    <CssVarsProvider>
      <Modal
...
export default memo(DaysVerificationModal)
