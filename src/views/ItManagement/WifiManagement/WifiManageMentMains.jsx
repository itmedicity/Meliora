...
const WifiManageMentMains = () => {
  ...
  const onChangeIPnumber = useCallback((e) => {
  ...
  const deletewifiCode = useCallback((val) => {
    const { in_patient_no } = val
    const ipno = {
      it_wifi_ipno: in_patient_no
    }
        ...
        <Paper square variant='outlined' sx={{ pt: 0.4, flexWrap: 'wrap', }}>
          <Box sx={{ display: 'flex', justifyContent: 'center', flex: 1 }}>
                  ...
                  <Box sx={{ my: 0.5, ml: 1 }}>
                    <CssVarsProvider>
                      <Tooltip title="search" placement="top">
                        <SearchIcon sx={{ height: 30, width: 30, color: '#0063C5', cursor: 'pointer' }} onClick={() => searchIP()} />
                      </Tooltip>
                    </CssVarsProvider>
                  </Box>
                  <Box sx={{ my: 0.5, pl: 1 }}>
                    <CssVarsProvider>
                      <Tooltip title="refresh" placement="top">
                        <CachedIcon sx={{ height: 30, width: 30, color: '#0063C5', cursor: 'pointer' }} onClick={() => refreshWindow()} />
                      </Tooltip>
                    </CssVarsProvider>
                  </Box>
                </Box>
                    ...
                    <CssVarsProvider>
                      {disbleP === true && disbleB === true ?
                        <IconButton
                          ...
                          sx={{
                            px: 2,
                            fontSize: 12, minHeight: '30px', lineHeight: '1.2', minWidth: '100px',
                        ...
                        <IconButton
                          sx={{
                            px: 2,
                            fontSize: 12, height: '30px', minHeight: '30px', lineHeight: '1.2', minWidth: '100px',
                            bgcolor: '#0277bd', fontWeight: 650, color: 'white',
                            '&:hover': {
                              bgcolor: '#5CACEE', color: 'white',
                            },
            ...
            <Box sx={{ flex: 1, display: 'flex', justifyContent: 'flex-end', }}>
              ...
              <Box sx={{ pr: 4, pt: 0.6 }}>
                <CssVarsProvider>
                  <Tooltip placement="bottom" title='Last 5 Days Wi-Fi Usage Count' sx={{ bgcolor: '#e3f2fd', color: 'black', pr: 3 }}>
                    <Box
                      variant="contained"
                      sx={{
                        cursor: 'pointer', borderRadius: '50%', minWidth: 40, minHeight: 40, border: '1px solid lightgrey',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, bgcolor: '#e3f2fd',
                      }}
                    >{allowtted.length}
                    </Box>
                  </Tooltip>
                ...
                {tabledata.map((val, index) => {
                  ...
                  >
                    ...
                    {dateOver === 1 ?
                      ...
                      : <>
                        {val.patient_flag === 1 ? <td style={{ textAlign: 'center', width: 100, cursor: 'pointer' }}>
                    ...
                    {dateOver === 1 ?
                      ...
                      : <>
                        {val.bystander_flag === 1 ? <td style={{ textAlign: 'center', width: 100, cursor: 'pointer' }}>
                    ...
                    <td style={{ textAlign: 'center', width: 100 }}> {val.patient_flag === 1 ? format(new Date(createdDate), 'dd-MM-yyyy') : 'Not Issued'}</td>
                    {dateOver !== 0 ? <td style={{ textAlign: 'center', width: 100 }}>
                    ...
                    {dateOver === 1 ?
                      <td size='sm' style={{ textAlign: 'center', width: 120, display: 'flex' }}>
                        ...
                        <CssVarsProvider>
                          <Tooltip title="Renew" placement='right' sx={{ bgcolor: '#ffebee', color: 'black', width: 60, pl: 1 }}>
                            <AddBoxSharpIcon
                              sx={{
                                cursor: 'pointer',
                                color: '#1565c0',
        ...
        expiredDetails.length !== 0 && searchFlag === 1 ?
          <Paper sx={{ mt: 3 }}>
            <Paper variant="outlined" sx={{
              fontSize: 18, color: '#0d47a1', pl: 3, display: 'flex', fontWeight: 650,
              height: 40, pt: 1, bgcolor: '#eeeeee', justifyContent: 'center'
            }}>Wi-Fi Usage Statistics</Paper>
                ...
                <Table stickyHeader >
                  ...
                  <tbody>
                    {expiredDetails.map((val, index) => {
                      return <tr
                        key={index}
                        sx={{ '&:last-child td, &:last-child th': { border: 0 }, maxHeight: 50, minHeight: 5 }}
                      >
                        <td style={{ textAlign: 'center', width: 150 }}> {val.in_patient_no}</td>
                        <td style={{ textAlign: 'center', width: 100, cursor: 'pointer' }}>
                          <QrCode2Icon size={6} sx={{ color: 'lightgray' }} /></td>
                        <td style={{ textAlign: 'center', width: 100, cursor: 'pointer' }}>
                          <QrCode2Icon size={6} sx={{ color: 'lightgray' }} /></td>
                        <td style={{ textAlign: 'center', width: 100 }}>{format(new Date(val.updated_date), 'dd-MM-yyyy')}</td>
                        <td style={{ textAlign: 'center', width: 100 }}>{format(new Date(val.expired), 'dd-MM-yyyy')}</td>
                      </tr>
                    })}
                  </tbody>
                </Table>
