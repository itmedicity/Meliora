...
const OtherBills = ({ otherData, billCount, setbillCount }) => {
    ...
    return (
        <Box>
            <CssVarsProvider>
                {pendingModalFlag === 1 ? <UpdatePendingModal pendingModalOpen={pendingModalOpen} billData={billData} index_no={other_bill_slno} filezUrls={filezUrls}
                    setpendingModalFlag={setpendingModalFlag} setpendingModalOpen={setpendingModalOpen} billCount={billCount} setbillCount={setbillCount}
                    bill_description={bill_description} cateName={cateName}
                /> : null}
            </CssVarsProvider>
            {otherData.length !== 0 ?
                <Box sx={{ flex: 1, display: 'flex', bgcolor: '#868B8E', minHeight: 30, pt: .5 }}>
                    <Box sx={{ mx: 2.3, textAlign: 'center', fontWeight: 600, color: 'white' }}>#</Box>
                    <Box sx={{ flex: 3, fontWeight: 600, color: 'white', pl: 1 }}>Bill Name<ManageSearchIcon sx={{ color: 'white', height: 20, width: 30, cursor: 'pointer' }}
                    ...
                    <Box sx={{ flex: 1, maxHeight: '55vh', overflow: 'auto' }}>
                        {alphbasedData && alphbasedData.map((val) => {
                            return <Paper key={val.other_bill_slno}
                                ...
                                <Tooltip title="bill name" placement="bottom"><Box sx={{ flex: 3, pt: .5, cursor: 'grab', pl: .5 }}>
                                    {val.bill_name}
                                </Box></Tooltip>
                        ...
                        </Box> : <Box sx={{ flex: 1, maxHeight: '55vh', overflow: 'auto' }}>
                            {
                                otherData && otherData.map((val) => {
                                    return <Paper key={val.other_bill_slno}
                                        ...
                                        <Box sx={{ flex: 3, pt: .5, cursor: 'grab', pl: .5 }}>
                                            {val.bill_name}
                                        </Box>
