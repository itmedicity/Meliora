...
const MonthlyBill = ({ monthlydata, billCount, setbillCount }) => {
    ...
    return (
        <Box>
            <CssVarsProvider>
                {pendingModalFlag === 1 ? <UpdatePendingModal pendingModalOpen={pendingModalOpen} billData={billData} index_no={monthly_slno} filezUrls={filezUrls}
                    setpendingModalFlag={setpendingModalFlag} setpendingModalOpen={setpendingModalOpen} billCount={billCount} setbillCount={setbillCount} cateName={cateName}
                /> : null}
            </CssVarsProvider>
            {monthlydata.length !== 0 ?
                <Box sx={{ flex: 1, display: 'flex', bgcolor: '#868B8E', minHeight: 30, pt: .5 }}>
                    ...
                    <Box sx={{ flex: .8, textAlign: 'center', fontWeight: 600, color: 'white' }}></Box>
                        ...
                        <Box>
                            {alphbasedData && alphbasedData.map((val) => {
                                ...
                                return <Paper key={val.monthly_slno}
                                    ...
                                    <Box sx={{ flex: .8, pt: .5, cursor: 'grab', pl: 1 }}>
                            ...
                            <Box sx={{ flex: 1, maxHeight: '55vh', overflow: 'auto' }}>
                                {billcate && billcate.map((val) => {
                                    ...
                                    return <Paper key={val.monthly_slno}
                                        ...
                                        <Tooltip title="bill Category" placement="bottom"><Box sx={{ flex: 1, pt: .5, cursor: 'grab', pl: 2 }}>
                                            {val.it_bill_category_name}
                                        </Box></Tooltip>
                                    </Paper>
                                })
                                ...
                                {
                                    monthlydata && monthlydata.map((val) => {
                                        const Monthly = format(new Date(val.monthly_bill_generate), 'MMM yyyy')
                                        return <Paper key={val.monthly_slno}
                                            sx={{ minHeight: 33, maxHeight: 100, bgcolor: '#E4E5E8', borderRadius: 0, display: 'flex', mt: .5, color: 'black', }}>
                                            <Box sx={{
                                            ...
                                            <Box sx={{ flex: .8, pt: .5, cursor: 'grab', }}>
                                                <Chip sx={{ bgcolor: '#F1C83A', fontSize: 14, fontWeight: 700, color: '#670305', }}>
                                                    {Monthly}
                                                </Chip>
                                            </Box>
