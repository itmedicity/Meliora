...
const BillsChartsAndGraph = () => {
  ...
  useEffect(() => {
    ...
    if (monthINyearData.length !== 0) {
      const MonthNdYearChartData = billCategoryNames?.map(val => {
        const totBill = monthINyearData
          ?.filter(item => val.it_bill_category_slno === item.bill_category)
    ...
    const firstYearData = Xvalue?.map(val => {
      const toBillAmount = firstYear
        ?.filter(item => format(new Date(item.bill_paid_date), 'MMM') === val)
        .reduce((acc, curr) => acc + curr.bill_amount, 0)
      return {
        month: val,
  ...
  return (
    ...
    >
      <Box sx={{ flex: 1, fontWeight: 600, fontSize: 20, color: '#2F435A', p: 0.2 }}>
        <BarChartIcon sx={{ color: '#2F435A', height: 25, width: 20 }} />
        Bills Tracker
      </Box>
      <Box sx={{ px: 0.5, flex: 1, display: 'flex' }}>
        <Paper sx={{ flex: 2.1, borderRadius: 0, bgcolor: 'white', height: 300, mb: 1 }}>
          <Box sx={{ flex: 1, display: 'flex' }}>
            ...
            <Box sx={{ flex: 1, display: 'flex', justifyContent: 'flex-end', pt: 0.5 }}>
              <Box sx={{ textAlign: 'center', pt: 0.8 }}>
                ...
                <Typography sx={{ fontSize: 13, color: '#211625' }}>Paid Amount</Typography>
              </Box>
              <Box sx={{ textAlign: 'center', px: 2 }}>
                  ...
                  <Dropdown>
                    ...
                    >
                      <MenuItem sx={{ flex: 1, display: 'flex' }}>
                        <Box sx={{ flex: 1 }}>
                          <Typography>Monthly Tarrif</Typography>
                          <Typography sx={{ fontSize: 12, pl: 2 }}>({monthName})</Typography>
                        </Box>
                      ...
                      <MenuItem sx={{ flex: 1, display: 'flex' }}>
                        <Box sx={{ flex: 1 }}>
                          <Typography>Quaterly Tarrif</Typography>
                          <Typography sx={{ fontSize: 12, pl: 2 }}>({quarterRange})</Typography>
                        </Box>
                        <Typography sx={{ fontSize: 15, pl: 5, pb: 1 }}>
                          <CurrencyRupeeIcon sx={{ width: 15, color: '#6E5828' }} />
                      ...
                      <MenuItem sx={{ flex: 1, display: 'flex' }}>
                        <Box sx={{ flex: 1 }}>
                          <Typography>Other Bills</Typography>
                          <Typography sx={{ fontSize: 12, pl: 2 }}>({monthName})</Typography>
                        </Box>
                        <Typography sx={{ fontSize: 15, pl: 5, pb: 1 }}>
                          <CurrencyRupeeIcon sx={{ width: 15, color: '#6E5828' }} />
                ...
                <Typography sx={{ fontSize: 13, color: '#211625' }}>Billed Amount</Typography>
              </Box>
            </Box>
          </Box>
      ...
      <Paper sx={{ flex: 1.2, borderRadius: 0, bgcolor: 'white', height: 300, mb: 1, mx: 0.5 }}>
        ...
        <Box sx={{ width: 180, ml: 1, pl: 1, display: 'flex' }}>
          <Input
            variant="solid"
            sx={{ borderRadius: 20, pl: 3, mr: 0.5 }}
            name="CompareYearOne"
            type="number"
            size="sm"
            min="2000"
