...
// Sub-component for individual certificate approval cards (compact sizing)
const CertificateItem = ({ title, file, onFileChange, onRemoveFile, checked, onCheckedChange }) => {
    ...
    return (
        ...
        >
            ...
            <Checkbox
                ...
                label={checked ? "Approved" : "Review File"}
                sx={{
                    fontWeight: 700,
                    '--Checkbox-size': '16px',
                    ml: 1.5,
                    fontSize: '12px',
                    color: file ? (checked ? '#16A34A' : '#64748B') : '#CBD5E1',
                    '&.Mui-checked': {
                        color: '#16A34A'
                    }
                }}
...
const MedicalFilesMainModal = ({ open, onClose, onSuccess, selectedDivision, loginDetails, tokenCount, selectedDate }) => {
    ...
    // Form states matching the design pre-filled values
    ...
    // Dynamic row handlers for Content Details
    ...
    const handleSave = useCallback(async (e) => {
        ...
        const hasEmptyRow = contentRows.some(row => !row.name.trim() || !row.qty.trim())
        if (hasEmptyRow) {
        ...
        const postData = {
            ...
            medicalrep_id: loginDetails && loginDetails.length > 0 ? loginDetails[0].medicalrep_id : null,
            content_rows: contentRows.map(row => ({ name: row.name, qty: row.qty })),
            gmp_approved: gmpApproved ? 1 : 0,
            analysis_approved: analysisApproved ? 1 : 0,
            quotation_approved: quotationApproved ? 1 : 0,
            product_approved: productApproved ? 1 : 0,
            departmentId: loginDetails && loginDetails.length > 0 ? loginDetails[0].departmentId : null,
            companyId: loginDetails && loginDetails.length > 0 ? loginDetails[0].companyId : null,
            prefix: tokenCount?.prefix || null,
            tokenno: tokenCount?.token_number || null,
            selectedDate: selectedDate
        }
        console.log(postData);
        try {
            // 1. Insert Metadata
            const result = await axioslogin.post('/indent/insertMedicalDocs', postData)
            const { success, message, insertid } = result.data

            if (success === 1 || success === 2) {
                // 2. Upload Files if insert was successful
                const recordId = insertid || result.data.insertId

                if (recordId) {
                    const formData = new FormData()
                    formData.append('id', recordId)

                    if (gmpFile) formData.append('gmpFile', gmpFile, gmpFile.name)
                    if (analysisFile) formData.append('analysisFile', analysisFile, analysisFile.name)
                    if (quotationFile) formData.append('quotationFile', quotationFile, quotationFile.name)
                    if (productFile) formData.append('productFile', productFile, productFile.name)
                    ...
                    if (uploadData.success === 1) {
                        succesNotify("Medical Documentation saved and files uploaded successfully")
                        if (onSuccess) {
                            onSuccess()
                        } else {
                            onClose()
                        ...
                        warningNotify(uploadData.message || "Metadata saved but file upload failed")
                        onClose()
                    }
                } else {
                    succesNotify(message || "Medical Documentation saved successfully")
                    if (onSuccess) {
                        onSuccess()
                    } else {
                        onClose()
                ...
                warningNotify(message || "Failed to save Medical Documentation")
            }
        } catch (error) {
            errorNotify(error.message || "An error occurred while saving documentation")
            console.error(error)
        }
    }, [
        medicineName,
...
export default memo(MedicalFilesMainModal)
