...
const ComplaintDashboard = ({ widgetName, count, status, slno }) => {
    ...
    const TotalComplaintsDept = () => {
        history.push('/Home/TotalDeptWiseList')
    }
    const PendingComplaintsDept = () => {
        history.push('/Home/PendingDeptWiseList')
    }
    const AssignedComplaintsDept = () => {
        history.push('/Home/AssignDeptWiseList')
    }
    const VerifyComplaintsDept = () => {
        history.push('/Home/VerifyDeptWiseList')
    }

    const PendingOnholdDept = () => {
        history.push('/Home/OnHoldPendingDeptWiseList')
    }
    const AssignedComplaintsEmp = () => {
        history.push('/Home/AssignEmpWiseList')
    }
    const VerifyComplaintsEmp = () => {
        history.push('/Home/VerifyEmpWiseList')
    }

    const PendingOnholdEmp = () => {
        history.push('/Home/OnHoldPendingEmpWiseList')
    }

    const TotalComplaintAll = () => {
        history.push('/Home/ComplaintList')
    }

    const DashboardClick = (slno) => {
        return slno === 1 && TotalComplaintsDept() ||
