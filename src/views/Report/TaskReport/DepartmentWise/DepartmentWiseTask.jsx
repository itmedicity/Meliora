...
const DepartmentWiseTask = () => {
  ...
  const DeptSearch = useCallback(() => {
    const getDeptTable = async () => {
      const result = await axioslogin.post(`/tmReport/searchDeptAndSec`, searchData)
  ...
  const onExportClick = () => {
    if (TableData.length === 0) {
      warningNotify('No Data For Download, Please select Department and section')
      setexport(0)
    } else {
      setexport(1)
    }
  ...
  return (
    <CardMasterClose title="DEPARMENT'S TASK REPORT" close={backToSetting}>
      <Box sx={{ display: 'flex', mt: 1 }}>
        ...
        <Box sx={{ flex: 2, ml: 1, pt: 2.8 }}>
          <CusIconButton size="sm" variant="outlined" clickable="true" onClick={DeptSearch}>
...
export default memo(DepartmentWiseTask)
