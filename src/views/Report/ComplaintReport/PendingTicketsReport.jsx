...
const PendingTicketsReport = () => {
    ...
    const SearchTickets = useCallback(async () => {
        if (ticketOption === 0 || ticketOption === null) {
            ...
            setLoading(true);
            try {
                const result = await axioslogin.post('/getTatReports/getPendingTicketsReport', postDept);
    ...
    const onExportClick = useCallback(() => {
        ...
        const excelData = ticketList.map((val, index) => ({
            ...
            "Ticket Status":
                val.compalint_status === 0 ? "Not Assigned" :
                    val.compalint_status === 1 &&
                        val.cm_rectify_status === 'O' ? "On Hold" : '',
        ...
        saveAs(new Blob([wbout], { type: "application/octet-stream" }), "PendingTickets.xlsx");
    }, [ticketList]);
                                    ...
                                    <Table padding={"none"} stickyHeader size='sm' borderAxis='both' >
                                        <thead>
                                            <tr style={{ backgroundColor: '#c1c8e4' }}>
                                                ...
                                                <th style={{ width: 150, minHeight: 200, whiteSpace: "normal", fontSize: 13 }}>Ticket Status</th>
                                            ...
                                            {ticketList?.map((val, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td style={{ width: "auto", }}>
                                                            {val.ASSINGED_DATE === null ? null :
                                                                format(new Date(val.ASSINGED_DATE), 'dd-MM-yyyy hh:mm:ss a')}
                                                        </td>
                                                        <td style={{ width: "auto", }}>{val.ASSINGED_EMP || null}</td>
                                                        <td style={{ width: "auto", }}>{val.HOLD_USER || null}</td>
                                                        <td style={{ width: "auto", }}>
                                                            {val.HOLDED_ON === null ? null :
                                                                format(new Date(val.HOLDED_ON), 'dd-MM-yyyy hh:mm:ss a')}
                                                        </td>
                                                        <td style={{ width: "auto", }}>
...
export default memo(PendingTicketsReport)