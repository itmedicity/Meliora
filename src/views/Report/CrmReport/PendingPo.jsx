    ...
    const searchCRFDetails = useCallback(
        async (e) => {
            ...
            if (startDate === '' || endDate === '') {
                warningNotify('Select Start date and End date before clicking the Search button');
                setOpen(false);
                return;
            }
            ...
            try {
                const result = await axioslogin.post('/CrfReports/getAllPurchasereport', postdata);
                const { success, data } = result.data;

                if (success === 1) {
                    const updatedData = data.map(item => {
                        const edDate = item.md_approve_date ? new Date(item.md_approve_date) : null;
                        const poDate = item.po_complete_date ? new Date(item.po_complete_date) : null;
                        const now = new Date();

                        let timeDiff = null;

                        if (edDate) {
                            const end = poDate || now;
                            const diffMs = end - edDate;
                ...
                console.error('Error fetching CRF details:', error);
                warningNotify('Something went wrong while fetching the report');
                setTableData([]);
            } finally {
                setOpen(false);
            }
        ...
        <Fragment>
            ...
            <Box sx={{ height: window.innerHeight - 80, flexWrap: 'wrap', bgcolor: 'white', width: '100%', }}>
                <Box sx={{ border: '1px solid #B4F5F0' }}>
                    <Box sx={{ display: 'flex' }}>
                        <Box sx={{ fontWeight: 550, flex: 1, pl: 1, pt: 0.5, color: '#385E72' }}>CRF PO And PO Pending</Box>
