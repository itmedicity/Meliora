...
const HallBookModal = ({ open, setOpen, count, setCount, setModal, mddata }) => {
  ...
  useEffect(() => {
    if (mddata.length !== 0) {
      ...
      const cateringfoods = JSON.parse(h_book_catering)
      const { ac, dias, podium, mikeset, projector, conferencetable } = hallitems
      const { tea, snacks, breakfast, lunch, dinner } = cateringfoods
      const formData = {
      ...
      const items = {
        ...
        ac: ac === 2 ? true : false,
        projector: projector === 3 ? true : false,
        podium: podium === 4 ? true : false,
        dias: dias === 5 ? true : false,
        conferencetable: conferencetable === 6 ? true : false
      }
      setItems(items)
      const foods = {
        tea: tea === 1 ? true : false,
        snacks: snacks === 2 ? true : false,
        breakfast: breakfast === 3 ? true : false,
        lunch: lunch === 4 ? true : false,
        dinner: dinner === 5 ? true : false
      }
      setCatering(foods)
      setSlno(h_book_slno)
    }
  ...
  const EventSubmit = useCallback(
    e => {
      e.preventDefault()
      const EventSubmit = async patchData => {
        const result = await axioslogin.patch('/hallBooking', patchData)
      ...
      if (dept !== 0) {
        EventSubmit(patchData)
      } else {
        infoNotify('Please Select Department')
      }
    },
    [patchData, count, dept, resetForm, setCount, reset]
  )
  return (
    <Fragment>
      {/* <ToastContainer /> */}
      ...
      >
        ...
        >
          {' '}
          <DialogContentText id="alert-dialog-slide-descriptiona">Event Booking</DialogContentText>
        ...
        <DialogActions>
          <Button color="secondary" onClick={EventSubmit}>
            Save
          </Button>
