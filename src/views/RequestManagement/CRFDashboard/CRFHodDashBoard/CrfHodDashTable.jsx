...
const CrfHodDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
    ...
    return (
        <Fragment>
            {HodModalFlag === 1 ? <CrfHodDashModal
                open={HodModalOpen} setOpen={setHodModalOpen}
                datas={HodModaldata} count={count} setCount={setCount}
            /> : null}
            {
                subDaFlag === 1 ?
                    <Box sx={{ pt: 1 }}>
                        <Typography sx={{ fontSize: 15, pl: 2, color: "#D0A2F7", fontWeight: 700 }}>
                            Table For HOD Pending :</Typography>
                        <CusAgGridForMain
