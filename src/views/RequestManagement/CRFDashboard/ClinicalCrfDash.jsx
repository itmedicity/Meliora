...
import CrfDMSDashTable from './CRFDMSDashBoard/CrfDMSDashTable'
import CrfMSDashTable from './CRFMSDashBoard/CrfMSDashTable'
...
const ClinicalCrfDash = ({ setClinicalCrfFlag, subDaFlag, data, count, setCount }) => {
  ...
  const DMSPending = data && data.filter(val => val.dms_approve === null)

  const MSPending = data && data.filter(val => val.ms_approve === null)
  ...
  const crfPendingClinicalDMS = useCallback(() => {
    if (DMSPending.length !== 0) {
      setWherePending(3)
    } else {
      warningNotify('No CRF For DMS Approval Pending')
      setWherePending(0)
    }
  }, [DMSPending])

  const crfPendingClinicalMS = useCallback(() => {
    if (MSPending.length !== 0) {
      setWherePending(4)
    } else {
      warningNotify('No CRF For MS Approval Pending')
      setWherePending(0)
    }
  }, [MSPending])
  ...
  const crfPendingClinicalMD = useCallback(() => {
    if (MDPending.length !== 0) {
      ...
      warningNotify('No CRF For MD Approval Pending')
      setWherePending(0)
    }
  }, [MDPending])
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Senior Manager Operations Pending
            </Box>
            <Box
              sx={{
                display: 'flex',
      ...
      {wherePending === 1 ? (
        ...
        <Box>
          <CrfDMSDashTable subDaFlag={subDaFlag} tabledata={DMSPending} count={count} setCount={setCount} />
        </Box>
      ) : wherePending === 4 ? (
        <Box>
          <CrfMSDashTable subDaFlag={subDaFlag} tabledata={MSPending} count={count} setCount={setCount} />
        </Box>
      ) : wherePending === 5 ? (
        <Box>
