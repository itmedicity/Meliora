...
const CrfInchargeDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
    ...
    return (
        <Fragment>
            {InchModalFlag === 1 ? <CrfInchargeDshModal
                open={InchModalOpen} setOpen={setInchModalOpen}
                datas={InchModaldata} count={count} setCount={setCount}
            /> : null}
            {
                subDaFlag === 1 ?
                    <Box sx={{ pt: 1 }}>
                        <Typography sx={{ fontSize: 15, pl: 2, color: "#D0A2F7", fontWeight: 700 }}>
                            Table For Incharge Pending :</Typography>
                        <CusAgGridForMain
