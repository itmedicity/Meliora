...
import CrfInchargeDshModal from './CrfInchargeDshModal'
import { Box } from '@mui/material'
import { Typography } from '@mui/joy'

const CrfInchargeDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
  ...
  const [InchModalFlag, setInchModalFlag] = useState(0)
  const [InchModaldata, setInchModalData] = useState([])

  //Data set for edit
  const rowSelect = useCallback(params => {
    const data = params.api.getSelectedRows()
    setInchModalData(data)
    setInchModalOpen(true)
    setInchModalFlag(1)
  }, [])
      ...
      {subDaFlag === 1 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
            Table For Incharge Pending :
          </Typography>
          <CusAgGridForMain columnDefs={columnClinical} tableData={InchargeData} />
        </Box>
      ) : subDaFlag === 3 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
          ...
          <CusAgGridForMain columnDefs={columnNonClinical} tableData={InchargeData} />{' '}
        </Box>
      ) : null}
    </Fragment>
  )
...
export default memo(CrfInchargeDashTable)
