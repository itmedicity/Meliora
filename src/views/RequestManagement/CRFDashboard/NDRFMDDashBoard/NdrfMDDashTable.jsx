...
const NdrfMDDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
    ...
    return (
        <Fragment>
            {mdApproveModalFlag === 1 ? <NdrfMDDashModal
                open={mdApproveModal} setOpen={setMDapprovModall}
                datas={mdApproveData} count={count} setCount={setCount}
            /> : null}
            {
                subDaFlag === 2 ?
                    <Box sx={{ pt: 1 }}>
                        <Typography sx={{ fontSize: 15, pl: 2, color: "#D0A2F7", fontWeight: 700 }}>
                            Table For MD Pending :</Typography>
                        <CusAgGridForMain
