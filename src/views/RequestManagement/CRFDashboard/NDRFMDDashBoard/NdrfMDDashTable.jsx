...
import NdrfMDDashModal from './NdrfMDDashModal'
import { Typography } from '@mui/joy'

const NdrfMDDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
  ...
  const [columnClinical] = useState([
    ...
    {
      headerName: 'NDRF CAOO/COO.Status',
      field: 'ndrfCOO',
      autoHeight: true,
      wrapText: true,
      minWidth: 250,
      ...
      {subDaFlag === 2 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
            Table For MD Pending :
          </Typography>
          <CusAgGridForMain columnDefs={columnClinical} tableData={mdData} />{' '}
        </Box>
      ) : subDaFlag === 4 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
          ...
          <CusAgGridForMain columnDefs={columnNonClinical} tableData={mdData} />
        </Box>
      ) : null}
    </Fragment>
  )
...
export default memo(NdrfMDDashTable)
