...
const CrfMSDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
    ...
    return (
        <Fragment>
            {msApproveModalFlag === 1 ? <CrfMSDashModal
                open={msApproveModal} setOpen={setmsapprovModall}
                datas={msApproveData} count={count} setCount={setCount}
            /> : null}
            {
                subDaFlag === 1 ?
                    <Box sx={{ pt: 1 }}>
                        <Typography sx={{ fontSize: 15, pl: 2, color: "#D0A2F7", fontWeight: 700 }}>
                            Table For MS Pending :</Typography>
                        <CusAgGridForMain
