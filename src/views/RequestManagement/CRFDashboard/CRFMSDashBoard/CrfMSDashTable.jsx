...
import CrfMSDashModal from './CrfMSDashModal'
import { Box } from '@mui/material'
import { Typography } from '@mui/joy'

const CrfMSDashTable = ({ subDaFlag, tabledata, count, setCount }) => {
  ...
  const [msApproveModal, setmsapprovModall] = useState(false)
  const [msApproveModalFlag, setmsApproveModalFlag] = useState(0)
  const [msApproveData, setmsApproveData] = useState([])

  //Data set for edit
  const rowSelect = useCallback(params => {
    const data = params.api.getSelectedRows()
    setmsApproveData(data)
    setmsapprovModall(true)
    setmsApproveModalFlag(1)
  }, [])
  return (
    <Fragment>
      {msApproveModalFlag === 1 ? (
        <CrfMSDashModal
          open={msApproveModal}
          setOpen={setmsapprovModall}
          datas={msApproveData}
          count={count}
          setCount={setCount}
        />
      ...
      {subDaFlag === 1 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
            Table For MS Pending :
          </Typography>
          <CusAgGridForMain columnDefs={columnClinical} tableData={msData} />{' '}
        </Box>
      ) : subDaFlag === 3 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
          ...
          <CusAgGridForMain columnDefs={columnNonClinical} tableData={msData} />{' '}
        </Box>
      ) : null}
    </Fragment>
  )
...
export default memo(CrfMSDashTable)
