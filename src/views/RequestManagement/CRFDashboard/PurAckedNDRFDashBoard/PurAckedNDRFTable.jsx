...
const PurAckedNDRFTable = ({ subDaFlag, tabledata, count, setCount }) => {
  ...
  const [columnClinical] = useState([
    ...
    { headerName: 'NDRF GM.Remark', field: 'ndrfCAORemark', minWidth: 250, wrapText: true },
    {
  ...
  return (
    <Fragment>
      {acknowlgedModalFlag === 1 ? (
        <PurAckedNdrfModal
          open={acknowlgedModal}
          setOpen={setacknowlgedModall}
          datas={acknowlgedData}
          count={count}
          setCount={setCount}
        />
      ) : null}
      {subDaFlag === 2 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
            Table For Purchase Acknowledgement Done :
          </Typography>
          <CusAgGridForMain columnDefs={columnClinical} tableData={omData} />{' '}
        </Box>
      ) : subDaFlag === 4 ? (
        <Box sx={{ pt: 1 }}>
          <Typography sx={{ fontSize: 15, pl: 2, color: '#D0A2F7', fontWeight: 700 }}>
          ...
          <CusAgGridForMain columnDefs={columnNonClinical} tableData={omData} />
        </Box>
      ) : null}
    </Fragment>
  )
