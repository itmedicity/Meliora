  ...
  const [column] = useState([
    {
      ...
      cellRenderer: params => {
        if (params.data.crf_dept_status === 1) {
          ...
          return (
            <IconButton onClick={() => DataEntry(params)} sx={{ color: editicon, paddingY: 0.5 }}>
    ...
    <CardCloseOnly title="Data Collection For CRF" close={backtoSetting}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', fontWeight: 400, pl: 1, pt: 1.2 }}>
          <Typography>Data already Given</Typography>
        </Box>
        <Box sx={{ display: 'flex' }}>
          <IconButton>
        ...
        <Box sx={{ display: 'flex', fontWeight: 400, pl: 1, pt: 1.2 }}>
          <Typography>Data Not Given</Typography>
        </Box>
      </Box>
    </CardCloseOnly>
  )
