...
const MDApprovalTable = () => {
  ...
  const [mdApproveModal, setMDapprovModall] = useState(false)
  const [mdApproveModalFlag, setMDApproveModalFlag] = useState(0)
  const [mdApproveData, setMDApproveData] = useState([])
  ...
  const [mdNdrfAppModal, setMDNdrfAppModal] = useState(false)
  const [mdNdrfAppModalFlag, setMDNdrfAppModalFlag] = useState(0)
  const [mdNdrfAppModalData, setMDNdrfAppModalData] = useState([])

  const MDApproval = useCallback((params) => {
    const data = params.api.getSelectedRows()
    setMDApproveData(data)
    setMDapprovModall(true)
    setMDApproveModalFlag(1)
  }, [])
  ...
  const ndrfSelect = useCallback((params) => {
    const data = params.api.getSelectedRows()
    setMDNdrfAppModal(true)
    setMDNdrfAppModalData(data)
    setMDNdrfAppModalFlag(1)
  }, [])
  ...
  return (
    <CardCloseOnly title="Medical Director Approval" close={backtoSetting}>
      {CloseModalFlag === 1 ? (
        <CloseDetailsModal
          open={CloseModal}
      ...
      {mdApproveModalFlag === 1 ? (
        <MDApproveModal
          open={mdApproveModal}
          setOpen={setMDapprovModall}
          datas={mdApproveData}
          count={count}
          setCount={setCount}
        />
      ...
      {mdNdrfAppModalFlag === 1 ? (
        <MDNdrfModal
          open={mdNdrfAppModal}
          setOpen={setMDNdrfAppModal}
          datas={mdNdrfAppModalData}
          count={count}
          setCount={setCount}
        />
      ) : null}
      ...
      {reqNdrf === 2 ? (
        <Box sx={{ p: 1 }}>
          <CusAgGridForMain columnDefs={column} tableData={mdData} getRowStyle={getRowStyle} />
        </Box>
      ) : (
        <Box sx={{ p: 1 }}>
          <CusAgGridForMain
