...
import CAOApprovalModel from './CAOApprovalModel'
...
const CEOApproval = () => {
    ...
    if (ndrftable.length !== 0) {
      ...
      const datas = COOPending.map(val => {
        const obj = {
          ...
          ndrfOM:
            val.ndrf_om_approv === 1
              ? 'Approved'
              : val.ndrf_om_approv === 2
              ? 'Reject'
              : val.ndrf_om_approv === 3
              ? 'On-Hold'
              : 'Not Updated',
          ...
          ndrfSMO:
            val.ndrf_smo_approv === 1
              ? 'Approved'
              : val.ndrf_smo_approv === 2
              ? 'Reject'
              : val.ndrf_smo_approv === 3
              ? 'On-Hold'
              : 'Not Updated',
  ...
  const [column] = useState([
    {
      ...
      cellRenderer: params => {
        ...
        } else if (params.data.ed_approve !== null && params.data.md_approve !== null) {
          return (
  ...
  const [cooApproveModal, setCOOapprovModall] = useState(false)
  const [cooApproveModalFlag, setCOOApproveModalFlag] = useState(0)
  const [cooApproveData, setCOOApproveData] = useState([])
  ...
  const [cooNdrfAppModal, setCOONdrfAppModal] = useState(false)
  const [cooNdrfAppModalFlag, setCOONdrfAppModalFlag] = useState(0)
  const [cooNdrfAppModalData, setCOONdrfAppModalData] = useState([])
  ...
  return (
    <CardCloseOnly title="GM Operations Approval" close={backtoSetting}>
      {CloseModalFlag === 1 ? (
        <CloseDetailsModal
          open={CloseModal}
      ...
      {cooApproveModalFlag === 1 ? (
        <CAOApprovalModel
          open={cooApproveModal}
          setOpen={setCOOapprovModall}
          datas={cooApproveData}
          count={count}
          setCount={setCount}
        />
      ) : null}
      ...
      {cooNdrfAppModalFlag === 1 ? (
        <NdrfModelCao
          open={cooNdrfAppModal}
          setOpen={setCOONdrfAppModal}
          datas={cooNdrfAppModalData}
          count={count}
          setCount={setCount}
        />
      ) : null}
      ...
      {reqNdrf === 2 ? (
        <Box sx={{ p: 1 }}>
          <CusAgGridForMain columnDefs={column} tableData={cooData} getRowStyle={getRowStyle} />
        </Box>
      ) : (
        <Box sx={{ p: 1 }}>
          <CusAgGridForMain columnDefs={columnndrf} tableData={ndrfData} />
        </Box>
      )}
