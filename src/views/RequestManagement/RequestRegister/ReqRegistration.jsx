...
const ReqRegistration = () => {
    ...
    const [alReadyItemNo, setAlreadyItem] = useState(0)
    const [editAddArry, setEditAddArry] = useState([])
    const [editAddArryFlag, setEditAddArryFlag] = useState(0)
    // Estimate add button click data store into array
    //When page is taken by users except it,maintenence an d biomedical
    const AddItemUser = useCallback(() => {
        if (item_desc === '') {
            ...
            if (value === 0) {
                ...
                if (arrayupdate === 0) {
                    const newdata = {
                        id: Math.ceil(Math.random() * 1000),
                        item_slno: parseInt(alReadyItemNo),
                        item_desc: item_desc,
                        item_brand: item_brand !== '' ? item_brand : "Not Given",
                        item_qnty: item_qty !== '' ? parseInt(item_qty) : 0,
                        item_status: 1
    ...
    }, [setArryUpdate, dataPost, arrayupdate, item_brand, item_desc, approx_cost,
        item_unit, item_qty, item_spec, editdata, item_slno, editAddArry,
        value, setAlreadyItem, alReadyItemNo])
    ...
    const AddItem = useCallback(() => {
        if (item_desc === '') {
            ...
            if (value === 0) {
                ...
                //Edit Details then add new details editAddArry, setEditAddArry
                setTableDis(1)
                if (arrayupdate === 0) {
                    const newdata = {
                        id: Math.ceil(Math.random() * 1000),
        ...
        item_unit, item_qty, item_spec, editdata, item_slno, editAddArry, value, setAlreadyItem, alReadyItemNo])
    ...
    const submitComplaint = useCallback((e) => {
        ...
        const PatchInsertFundetl = async (insertid) => {
            if (dataPost.length !== 0) {
                const postdataDetl = editAddArry && editAddArry.map((val) => {
                    return {
                        req_slno: insertid,
                        item_slno: val.item_slno,
                        item_desc: val.item_desc,
