...
const RectifyCompalint = () => {
  ...
  const [column] = useState([
    {
      ...
      cellRenderer: params => {
        if (params.data.compalint_status === 2 || params.data.compalint_status === 3) {
          ...
          return (
            <IconButton sx={{ color: editicon, paddingY: 0.5 }} onClick={() => Rectifycomplaintdept(params)}>
              <CustomeToolTip title="Rectify Complaint ">
                <AssignmentTurnedInIcon />
              </CustomeToolTip>
            </IconButton>
    ...
    {
      headerName: 'Rectified Status',
      field: 'cm_rectify_status1',
      autoHeight: true,
      wrapText: true,
      minWidth: 150
    },
    {
      headerName: 'Reason',
      field: 'rectify_pending_hold_remarks1',
      autoHeight: true,
      wrapText: true,
      minWidth: 200
    }
  ...
  return (
    <Fragment>
      ...
      <CardMaster close={backtoSetting} title="Rectify complaint">
        <Box
          sx={{
            width: '100%',
            pl: 1,
        ...
        <Box sx={{ p: 1 }}>
          {check === 0 ? (
            <ComplistAgGridcmp columnDefs={column} tableData={getdata} count={count} />
          ) : (
            <ComplistAgGridcmp columnDefs={column} tableData={tabledata} count={count} />
          )}
        </Box>
      </CardMaster>
    </Fragment>
