...
const RectifiedInAllList = ({ count, setCount }) => {
    ...
    const rectifiedListCall = useCallback(() => {
        if (searchDate) {
            const getAllPendingHoldComplaints = async () => {
                const result = await axioslogin.post('/Rectifycomplit/getDepartmentRectfiedList', searchDate);
            ...
            <Box sx={{ border: 1, borderColor: 'lightgrey' }}>
                ...
                {queryflag === 1 ?
                    <QueryModalview open={queryOpen} setqueryOpen={setqueryOpen} valuee={valuee}
                        setqueryflag={setqueryflag} setCount={setCount} count={count}
                    /> : null}
                {detailsFlag === 1 ?
                    <RectifyDetailsModal
                        open={deatilsOpen}
                ...
                {rectifedList.length !== 0 ?
                    ...
                    />
                    :
                    <Box sx={{ flex: 1, height: '58vh', textAlign: 'center', fontWeight: 700, fontSize: 22, color: 'lightgray', pt: 10, }}>
                        {flagz === 1 ?
                            <>
                                Empty list Under Selected Date Range
                            </> :
                            <>
                                Search Date Range to View Rectified Tickets Under Department
                            </>}
...
export default memo(RectifiedInAllList)