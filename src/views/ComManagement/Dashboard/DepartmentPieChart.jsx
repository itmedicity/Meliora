...
import { getmonthlyticketChart } from 'src/api/TicketApi'
import TextComponent from 'src/views/Components/TextComponent';
import SquareIcon from '@mui/icons-material/Square';

const DepartmentPieChart = ({ empdept }) => {
    ...
    const pieChartData = useMemo(() => {
        if (!monthlyChart || monthlyChart.length === 0) return [];
        const chartData = monthlyChart[0];
    ...
    function PieCenterLabel({ children }) {
        ...
        return (
            <StyledText x={left + width / 1} y={top + height / 1.9}>
                {children}
            </StyledText>
        );
    }
    function CenterLabel({ children }) {
        const { width, height, left, top } = useDrawingArea();
        return (
            <StyledCenterText x={left + width / 1.1} y={top + height / 2.4}>
                {children}
            </StyledCenterText>
        );
    }
    ...
    return (
        <Box sx={{ width: 370, border: 1, borderColor: 'lightgrey', p: 1, my: .5, bgcolor: 'white', borderRadius: 5 }}>
            ...
            <Box sx={{ flex: 1, display: 'flex' }}>
                ...
                <Box sx={{ pl: 1, pt: 2.5 }}>
                    ...
                    <Box sx={{ flex: 1, pt: .5, pl: 2, display: 'flex', gap: 1 }}>
