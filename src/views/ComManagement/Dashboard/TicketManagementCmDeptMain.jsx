...
const TicketManagementCmDeptMain = () => {
    ...
    const menuList = useMemo(() => {
        if (loading) return [];
        return [
            { slno: 235, name: 'Ticket Dashboard All Dept Ticket Tile', component: <AllDeptticketTile /> },
            { slno: 236, name: 'Ticket Dashboard Dept Ticket Tile', component: <DeptTicketTile /> },
            { slno: 237, name: 'Ticket Dashboard Dept Ticket Type Progress Bar', component: <TicketTypeListProgressChart empdept={empdept} /> },
            { slno: 238, name: 'Ticket Dashboard Dept Pie Chart', component: <DepartmentPieChart empdept={empdept} /> },
            { slno: 239, name: 'Ticket Dashboard Dept Response Tat', component: <DeptOpenTicketProgressBar empdept={empdept} /> },
            { slno: 240, name: 'Ticket Dashboard Dept Closed Tat', component: <DeptClosedTicketProgressBar empdept={empdept} /> },
            { slno: 241, name: 'Ticket Dashboard Status Across Dept', component: <AllDeptTicketProgressList PostDataa={PostDataa} /> },
            { slno: 242, name: 'Ticket Dashboard All  Dept Pie Chart', component: <AllDeptPieChart /> },
            { slno: 244, name: 'Ticket Dashboard All  Dept Response Tat', component: <AllDeptOpenTicketProgressBar /> },
            { slno: 245, name: 'Ticket Dashboard All  Dept Closed Tat', component: <AllDeptClosedProgressBar /> },
            { slno: 246, name: 'Ticket Dashboard All  Dept Top contributors', component: <TopPerformerList PostDate={PostDate} /> },
            { slno: 247, name: 'Ticket Dashboard  Dept Employee Stat', component: <EmployeeTicketList empdept={empdept} /> },
        ];
    ...
    return (
        <Box sx={{
            ...
            <CssVarsProvider>
                ...
                <Box sx={{ flex: 1, display: 'flex', p: .5, gap: .5 }}>
                    <Box sx={{ flex: 1, }}>
                        {
                        ...
                        <Box sx={{ flex: 1, display: 'flex', gap: .5 }}>
                            ...
                            <Box>
                                {
                                    menurights.find((menu) => menu.slno === 238)?.component || null
                                }
                            </Box>
                        </Box>
                        <Box sx={{ flex: 1, display: 'flex', gap: .5 }}>
                        ...
                        <Box sx={{ flex: 1, display: 'flex', gap: .5 }}>
                            ...
                            <Box>
                                {
                                    menurights.find((menu) => menu.slno === 242)?.component || null
                                }
                            </Box>
                        </Box>
                        <Box sx={{ flex: 1, display: 'flex', gap: .5 }}>
                    ...
                    <Box>
                        {
                            menurights.find((menu) => menu.slno === 247)?.component || null
                        }
                    </Box>
                </Box>
            </CssVarsProvider >
