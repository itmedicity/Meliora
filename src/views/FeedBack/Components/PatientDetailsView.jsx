...
const PatientDetailsView = ({ nsName, setListFlag }) => {
    ...
    const ipList = [
        { ipno: 1, ptid: 'K-00004752', ptname: 'ADHYA', bed: 'A017 AC', doctor: 'ANANTHAKRISHNAN S', room: 1, complnt: 1, incident: 0, feedback: 0 },
        { ipno: 2, ptid: 'L-00047321', ptname: 'DILEEP S', bed: 'A026 NR', doctor: 'AFFIN A', room: 0, complnt: 0, incident: 0, feedback: 0 },
        { ipno: 3, ptid: 'M-00000143', ptname: 'APPU', bed: 'A015 AC', doctor: 'ASWIN M NAIR', room: 0, complnt: 1, incident: 1, feedback: 0 },
        { ipno: 4, ptid: 'N-00045120', ptname: 'JIBIN WILLAM', bed: 'A016 NR', doctor: 'JUMALY GEORGE', room: 1, complnt: 1, incident: 1, feedback: 0 },
        { ipno: 5, ptid: 'N-00008541', ptname: 'JAYANANDA .K  ', bed: 'A026 AC', doctor: 'ARUN KUMAR A S', room: 1, complnt: 0, incident: 1, feedback: 0 },
        { ipno: 6, ptid: '', ptname: '', bed: 'A036 AC', doctor: 'ARUN KUMAR A S', room: 0, complnt: 0, incident: 0, feedback: 0 },

        { ipno: 1, ptid: 'K-00004752', ptname: 'ADHYA', bed: 'A017 AC', doctor: 'ANANTHAKRISHNAN S', room: 1, complnt: 1, incident: 0, feedback: 0 },
        { ipno: 2, ptid: 'L-00047321', ptname: 'DILEEP S', bed: 'A026 NR', doctor: 'AFFIN A', room: 0, complnt: 0, incident: 0, feedback: 0 },
        { ipno: 3, ptid: 'M-00000143', ptname: 'APPU', bed: 'A015 AC', doctor: 'ASWIN M NAIR', room: 0, complnt: 1, incident: 1, feedback: 0 },
        { ipno: 4, ptid: 'N-00045120', ptname: 'JIBIN WILLAM', bed: 'A016 NR', doctor: 'JUMALY GEORGE', room: 1, complnt: 1, incident: 0, feedback: 0 },
        { ipno: 5, ptid: 'N-00008541', ptname: 'JAYANANDA .K  ', bed: 'A026 AC', doctor: 'ARUN KUMAR A S', room: 1, complnt: 0, incident: 1, feedback: 0 },
        { ipno: 6, ptid: '', ptname: '', bed: 'A036 AC', doctor: 'ARUN KUMAR A S', room: 0, complnt: 0, incident: 0, feedback: 0 },
    ...
    return (
        <Fragment>
            {formFlag === 1 ? <ListMenuForIP setformFlag={setformFlag} ptDetails={ptDetails} regFormFlag={regFormFlag}
                nsName={nsName} open={modalopen} handleClose={handleClose} detailFlag={detailFlag} setDetailFlag={setDetailFlag} /> :
                <Box sx={{ maxHeight: '79vh' }}>
                    <Box sx={{ display: 'flex', height: 42 }}>
                        <Box sx={{ flex: 1, fontSize: 19, pt: 1, pl: 2, }}>
                            {nsName}
                        </Box>
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', flex: 1, fontSize: 20, pt: 0.8, pr: 0.2 }}>
                            <Tooltip title="Close" placement="bottom" >
                                <Avatar size="sm" variant="plain" sx={{ bgcolor: '#F5F5F5' }}>
                                    <CloseIcon sx={{
                                        cursor: 'pointer', size: 'lg', fontSize: 20, color: '#424242',
                                        '&:hover': { color: 'red' }
                                    }} onClick={backtoHome} />
                                </Avatar>
                            </Tooltip>
                                        ...
                                        <Paper sx={{ bgcolor: '#F0F4F8', borderRadius: 2, boxShadow: 3, pb: 1 }}>
                                            ...
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', bgcolor: '#fff' }}>
                                                {val.ptid === '' ? (
                                                    ...
                                                    : (<Box sx={{ display: 'flex', pl: 1, pt: 1 }}>
                                                        <Tooltip title="Room Checklist" placement="bottom">
                                                            <ChecklistIcon sx={val.room === 1 ? iconStyleAfterSave : iconStyle} onClick={() => RoomChecklistDetails(val)} />
                                                        </Tooltip>
                                                        <Tooltip title="Reg. Complaint" placement="bottom">
                                                            <EngineeringIcon sx={val.complnt === 1 ? iconStyleIncident : iconStyle} onClick={() => RegComplaintDetails(val)} />
                                                        </Tooltip>
                                                        <Tooltip title="Incident Reg." placement="bottom">
                                                            <EditNoteIcon sx={val.incident === 1 ? iconStyleIncident : iconStyle} onClick={() => RegIncidentDetails(val)} />
                                                        </Tooltip>
                                                        <Tooltip title="Feedback" placement="bottom">
                                                    ...
                                                    <Dropdown>
                                                        <MenuButton sx={{ p: 0, m: 0.5, border: 0, pr: 1 }}>
                                                            <Tooltip title="Add" placement="right">
                                                                <AddBoxIcon sx={{ color: '#757575', height: 28, width: 28 }} onClick={() => AddDetails(val)} />
                                                            </Tooltip>
                                                        </MenuButton>
                                                        {addFlag === 1 ?
                    ...
                    >
                        <Box sx={{
                            border: '1px solid lightgrey', bgcolor: '#388E3C', borderRadius: 8, boxShadow: 5,
                            p: 0.5, fontSize: 10, fontWeight: 550, color: 'white'
                        }}>CheckList Added/Completed</Box>
                        <Box sx={{
                            border: '1px solid lightgrey', bgcolor: '#d32f2f', borderRadius: 8, boxShadow: 5,
                            p: 0.5, fontSize: 10, fontWeight: 550, color: 'white'
                        }}>Complaint Registered</Box>
                        <Box sx={{
                            border: '1px solid lightgrey', borderRadius: 8, boxShadow: 5,
                            p: 0.5, fontSize: 10, fontWeight: 550
                        }}>No Entries Reported</Box>
                    </Box>
                </Box >
