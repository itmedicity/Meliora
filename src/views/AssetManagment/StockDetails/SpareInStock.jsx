...
const SpareInStock = () => {
    ...
    const filteredSpareList = useMemo(() => {
        return spareList.filter((item) => {
            const fullSpareNo = `${item?.spare_asset_no || ''}/${(item?.spare_asset_no_only || 0).toString().padStart(6, '0')}`;
            return (
                fullSpareNo.toLowerCase().includes(filters.spareNo.toLowerCase()) &&
                (item?.category_name || '').toLowerCase().includes(filters.category.toLowerCase()) &&
                (item?.item_name || '').toLowerCase().includes(filters.itemName.toLowerCase())
    ...
    return (
        <Box sx={{ flex: 1, p: 1 }}>
            <Box sx={{
                height: 40, display: 'flex', borderBottom: 1, borderTop: 1, borderColor: 'lightgray',
                bgcolor: 'white', flex: 1
            }}>
                <Box sx={{ width: 50, fontWeight: 600, color: 'black', fontSize: 12, pt: 1, pl: 1 }}>
                    #
                </Box>
                <Box sx={{ width: 140, display: 'flex', }}>
                    <IconButton
                        onClick={(e) => handleMenuOpen(e, 'spareNo')}
                    >
                ...
                <Box sx={{ flex: 4, display: 'flex', }}>
                    <IconButton
                        onClick={(e) => handleMenuOpen(e, 'itemName')}
                    ...
                    itemContent={(index) => {
                        ...
                        return (
                            <Box
                                ...
                                <Box sx={{ flex: 2, pl: .5, fontWeight: 400, color: 'black', fontSize: 14 }}>
                                    {val.category_name || "-"}
                                </Box>
