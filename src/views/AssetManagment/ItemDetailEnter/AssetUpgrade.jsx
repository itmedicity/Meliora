    ...
    return (
        <Box
            sx={{
                ...
                m: 0,
                pt: 1,
                border: 1, borderColor: '#E0E1E3', py: 1, pl: 2
            }}>
            {spareDetails.length !== 0 ?
                <Box sx={{ flex: 1, mb: 3 }}>
                    ...
                    {spareDetails.map((val, index) => {
                        ...
                        return (
                            <Box key={index} sx={{ flex: 1, display: 'flex', borderBottom: 1, borderColor: 'lightgrey', pt: .8, mr: 2 }}>
                                <Box sx={{ width: 50, textAlign: 'center', fontSize: 13, fontWeight: 500 }}>{index + 1}</Box>
                                <Box sx={{ width: 105, fontSize: 13, fontWeight: 500 }}>{val.spare_asset_no}/{formattedSlno}</Box>
                                <Box sx={{ flex: 1, fontSize: 13, fontWeight: 500 }}>{val.item_name}</Box>

                                <Tooltip title={'By clicking, the spare will be removed from the asset and add back to the stock'}
                                    placement='top'
                                    sx={{ width: 200, }}>
                                    <Box sx={{ width: 95, textAlign: 'center', fontSize: 13, pr: .5, cursor: 'pointer' }}>
                                        <ChangeCircleSharpIcon sx={{
                ...
                {spareData.length !== 0 ?
                    <>
                        ...
                        {spareData.map((val, index) => {
                            return (
                                <Box key={index} sx={{
                                    flex: 1, mr: 1.1, display: 'flex', borderBottom: 1, py: .5, borderLeft: 1, borderRight: 1, ml: .1,
                                    borderColor: 'lightgrey'
                                }}>
                                    <Box sx={{ flex: .5, textAlign: 'center', fontSize: 13 }}>{index + 1}</Box>
                                    <Box sx={{ flex: 10, fontSize: 13 }}>{val.name}</Box>
                                    <Tooltip placement='left' title={'Spare will be removed  by clicking'}
                                        color='neutral'>
