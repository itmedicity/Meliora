...
const LeaseAddMast = ({ setLeaseFlg }) => {
  ...
  const updateLeaseMAst = useCallback(
    e => {
  ...
  const ViewAmcCmcImage = async () => {
    setImageShowFlag(1)
    setImageShow(true)
    const images = await getFilesFromZip('/AssetFileUpload/LeaseMasterImageView', am_lease_mastslno);
    setImageArry(images);
  };
    ...
    >
      ...
      <Box sx={{ flex: 1, display: 'flex', gap: 1 }}>
        <Box sx={{ width: 420 }}>
          ...
          {oracleFlag !== 1 ? (
            <Box sx={{ display: 'flex', pt: 1, gap: 0.5 }}>
              <Box>
                <CusIconButton
                  size="sm"
                  variant="outlined"
