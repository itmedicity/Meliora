...
const AmcCmcAddedTable = ({ count, rowSelect }) => {
    ...
    return (
        <Box sx={{
            minHeight: 225, maxHeight: 250,
            overflow: 'auto', border: 1, borderColor: 'lightgrey'
        }} >
            <CssVarsProvider>
                                    ...
                                    >
                                        <Box sx={{ display: 'flex', cursor: 'pointer', px: 0.5 }}>
                                            <Typography sx={{ flex: 1 }}>
                                                {field === 'suppplier' ? 'Supplier' : field === 'amcmc' ? 'AMC/CMC' : field === 'frmDate' ? 'From Date' : 'To Date'}
                                            </Typography>
                                            <CloseIcon onClick={() => handleMenuClose(field)} />
                                        </Box>
                                        <MenuItem sx={{ padding: 0, border: 0, borderColor: 'white' }}>
                                            <Input
                                                placeholder={`Search ${field === 'suppplier' ? 'Supplier' : field === 'amcmc' ? 'AMC/CMC' : field === 'frmDate' ? 'From Date' : 'To Date'}`}
                                                value={filters[field]}
