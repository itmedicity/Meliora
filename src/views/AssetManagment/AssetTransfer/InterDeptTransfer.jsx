...
import AmAssetUnderSelectdDeptAndSec from 'src/views/CommonSelectCode/AmAssetUnderSelectdDeptAndSec'
...
const InterDeptTransfer = () => {
    ...
    const updateDeptTransfer = useCallback(() => {
        if (custAsset !== 0) {
            infoNotify(
                <>
                    Click on <AddCircleIcon /> to add the asset to the Transfer List.
                </>
            );
            return;
        }
        const patchDeptTrans = async (patchData) => {
            try {
                const { data } = await axioslogin.patch('/assetDeptTransfer/AssetTransfer', patchData);
                ...
                infoNotify("Error during department transfer:", error);
            }
        };
    ...
    return (
        <Box>
            <Box sx={{ flex: 1, border: 1, borderColor: 'lightgray', m: 1, px: 1, pt: 2, pb: 1 }}>
                <Box sx={{ flex: 1, display: 'flex', gap: .9, }}>
                    <Box sx={{ flex: 1, }}>
                        <TextComponent
                ...
                <Box sx={{ flex: 1, display: 'flex', gap: .9, mt: 2 }}>
                    <Box sx={{ flex: 1, }}>
                        <TextComponent
                ...
                {selectedAssets.length !== 0 ?
                    <Box sx={{
                        ...
                        <Box sx={{ flex: 1, display: 'flex', bgcolor: '#BDC4C9' }}>
                            <Box sx={{ flex: .5, pl: 3, color: 'black', fontWeight: 600 }}>
                                #
                            </Box>
                            <Box sx={{ flex: 2, color: 'black', fontWeight: 600 }}>
                                Asset No.
                            </Box>
                            <Box sx={{ flex: 8, color: 'black', fontWeight: 600 }}>
                                Asset Name
                            </Box>
            ...
            <Box sx={{ flex: 1, border: 1, borderColor: 'lightgray', m: 1, p: .5, }}>
                ...
                <Box sx={{ flex: 1, overflow: 'auto' }}>
                    {isLoading ? (
                        ...
                        <Box sx={{ felx: 1, border: 1, py: 5, borderColor: '#D3D3D3', display: 'flex', justifyContent: 'center' }}>
                            No Data Available
                        </Box>
                    ) : (
                            ...
                            >
                                <thead>
                                    <tr>
                                        ...
                                        <th rowSpan={2} style={{ width: '130px' }}>Transfer ID</th>
                                        <th rowSpan={2} style={{ width: '140px' }}>Transferred Date</th>
                                        <th rowSpan={2} style={{ width: '200px' }}>Transferred User</th>
                                        <th rowSpan={2} style={{ width: '120px' }}>Transferred Items</th>
                                                        ...
                                                        <td colSpan={12} style={{ height: 0, padding: 0 }}>
                                                            ...
                                                            >
                                                                <TextComponent
                                                                    ...
                                                                    sx={{ fontWeight: 500, color: 'black', pb: .5 }} />
