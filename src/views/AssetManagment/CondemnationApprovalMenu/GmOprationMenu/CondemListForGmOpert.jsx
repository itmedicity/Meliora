...
const CondemListForGmOpert = ({ empId, menurights }) => {
  ...
  const filteredPendingCondemAllDeptGmOpr = useMemo(() => {
    return AllDeptCondemPendingGmOpr?.filter(row => !(row.store_approve_status === 2)) || []
  ...
  return (
    <Box>
      ...
      {selectedRValue === '1' ? (
        <Box>
          <AllPendingsGmOpertions
            filteredPendingCondemAllDeptGmOpr={filteredPendingCondemAllDeptGmOpr}
            editForm={editForm}
            viewForm={viewForm}
          />
        </Box>
