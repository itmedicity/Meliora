...
import AllPendingsGmOpertions from './AllPendingsGmOpertions';
...
const CondemListForGmOpert = ({ empId, menurights }) => {

    const condemStatusFrom = 7
    const condemstatusTo = 3
    ...
    const { data: AllDeptCondemPendingGmOpr } = useQuery({
        queryKey: ['getAllDeptCondemPendingGmOpr', formCount],
        queryFn: () => getAllDeptCondemPendingDatas(postCondemAllDept),
    })

    const filteredPendingCondemAllDeptGmOpr = useMemo(() => {
        return AllDeptCondemPendingGmOpr?.filter(
            row => !(row.store_approve_status === 2)
        ) || [];
    }, [AllDeptCondemPendingGmOpr]);
    ...
    return (
        <Box>
            ...
            {selectedRValue === "1" ?
                <Box>
                    <AllPendingsGmOpertions filteredPendingCondemAllDeptGmOpr={filteredPendingCondemAllDeptGmOpr} editForm={editForm} viewForm={viewForm} />
                </Box>
                :
                selectedRValue === "2" ?
                    <Box>
...
export default memo(CondemListForGmOpert)