...
const AllPendingsGmOpertions = ({ filteredPendingCondemAllDeptGmOpr, editForm, viewForm }) => {
    return (
        <Box>
            <CssVarsProvider>
                {filteredPendingCondemAllDeptGmOpr && filteredPendingCondemAllDeptGmOpr.length > 0 ? (
                    <Table stickyHeader size='sm'
                        ...
                        <tbody >
                            {filteredPendingCondemAllDeptGmOpr
                                ?.slice()
                                .sort((a, b) => a.condem_status - b.condem_status)
                                .map((val, index) => (
                                    <tr key={index}>
                                        <td style={{ textAlign: 'center', }}>{index + 1}</td>
                                        <td >{val.condem_status <= 4 ?
                                            <Box sx={{
...
export default memo(AllPendingsGmOpertions)