...
const AllPendingsGenStore = ({ AllDeptCondemPendingStore, editForm, viewForm }) => {
    return (
        <Box>
            <CssVarsProvider>
                {AllDeptCondemPendingStore && AllDeptCondemPendingStore.length > 0 ? (
                    <Table stickyHeader size='sm'
                        ...
                        <tbody >
                            {AllDeptCondemPendingStore
                                ?.slice()
                                .sort((a, b) => a.condem_status - b.condem_status)
                                .map((val, index) => (
                                    <tr key={index}>
                                        <td style={{ textAlign: 'center', }}>{index + 1}</td>
                                        <td >{val.condem_status <= 6 ?
                                            <Box sx={{
                                                                    ...
                                                                    val.condem_status === 4 && val.gm_approve_status === 2 ? "GM Operations Rejected" :
                                                                        val.condem_status === 5 && val.acc_approve_status === 1 ? "Accounts Approved" :
                                                                            val.condem_status === 5 && val.acc_approve_status === 2 ? "Accounts Rejected" :
                                                                                val.condem_status === 6 && val.store_approve_status === 1 ? "General Store Approved" :
                                                                                    val.condem_status === 6 && val.store_approve_status === 2 ? "General Store Rejected" :
...
export default memo(AllPendingsGenStore)