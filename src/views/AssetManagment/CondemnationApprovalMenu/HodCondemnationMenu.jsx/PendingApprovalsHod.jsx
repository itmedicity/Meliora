...
const PendingApprovalsHod = ({ empId, empdept, menurights }) => {
  ...
  const PendingApproveHodData = useMemo(
    () => (CondemHodData ? CondemHodData.filter(row => row.incharge_approve_status !== 2) : []),
    [CondemHodData]
  )
  ...
  return (
    <Box sx={{ py: 1, px: 0.5 }}>
      ...
      <CssVarsProvider>
        {PendingApproveHodData && PendingApproveHodData.length > 0 ? (
          <Table stickyHeader size="sm" sx={{ borderRadius: 2 }} borderAxis="both">
            ...
            <tbody>
              {PendingApproveHodData?.map((val, index) => (
                <tr key={index}>
...
export default memo(PendingApprovalsHod)
