...
import InchargeCondemApprovalModal from './InchargeCondemApprovalModal';
...
const PendingApprovalIncharge = ({ empdept, empid }) => {
    ...
    const [inchargePendingApprovalList, setinchargePendingApprovalList] = useState([]);
    ...
    const { isLoading } = useQuery(
        ['CondemnInchargePendingApproval', postDept],
        async () => {
            const result = await axioslogin.post(
                'AssetCondemnation/getCondemnInchargePendingApproval',
                postDept
            );
        ...
        },
        {
            ...
            onSuccess: (data) => {
                // setinchargePendingApprovalList(data);
                // Filter out items with hod_level_acknowledge = 1
                const filteredData = data.filter(item => item.hod_level_acknowledge !== 1);
                setinchargePendingApprovalList(filteredData);
            },
        }
    );
    ...
    return (
        <Box sx={{ py: 1, px: .5 }}>
            ...
            <CssVarsProvider>
                {isLoading ? (
                ...
                ) : inchargePendingApprovalList.length > 0 ? (
                    <Table stickyHeader size='sm'
                        ...
                        <tbody >
                            {inchargePendingApprovalList?.map((val, index) => {
...
export default PendingApprovalIncharge