...
const CondemnApproveModal = ({ modalApproveOpen, setmodalApproveOpen, setmodalApproveFlag, empId, formDetails, EmployeeCondemnApprovalLevelData,
    queryClient, checkActiveLevel }) => {

    const { condem_mast_slno, condem_form_prefix, condem_form_no, reg_date, inch_emp, inch_level_state, inch_review_date, inch_review,
        hod_emp, hod_level_state, hod_review, hod_review_date, req_dpt_name
    } = formDetails

    const { level_name, level_no } = EmployeeCondemnApprovalLevelData[0] || {};
    ...
    const ApproveData = useMemo(() => {
        return {
            ...
            condemn_rejected: 0,
            level_review_slno: null,
        };
    }, [level_no, remarks, empId, condem_mast_slno, checkActiveLevel]);
    ...
    const { data: allVerifiedList = [], } = useQuery(['PendingCondemApprovalList'], async () => {
        const result = await axioslogin.get(`AssetCondemnation/getCondemnLlevelsApproved/${condem_mast_slno}`);
...
export default memo(CondemnApproveModal)