...
const EditCondemnAtLevel = ({ modalApproveOpen, setmodalApproveOpen, setmodalApproveFlag, empId, formDetails, setformDetails, queryClient, level_no }) => {

    const { condem_mast_slno, condem_form_prefix, condem_form_no, reg_date, inch_level_state, inch_review, inch_review_date, inch_emp, hod_emp, hod_level_state,
        hod_review, hod_review_date, checkActiveLevel, req_dpt_name } = formDetails
    ...
    }, [level_no, condem_mast_slno])

    const { data: levelDetails = [], } = useQuery(['getlevelDetails'], async () => {
        const result = await axioslogin.post('AssetCondemnation/getcondemlevelDetails', postLevel);
        return result.data?.data || [];
    },
        { enabled: !!level_no }
    );
    const { level_review_state, level_review, level_name, level_review_slno } = levelDetails[0] || {};

    const [condemCount, setcondemCount] = useState(0)
    const [remarks, setremarks] = useState(level_review || '')
    const [count, setcount] = useState(0)
    const [allVerifiedList, setAllVerifiedList] = useState([]);
    ...
    const { data: verifiedList = [], isSuccess } = useQuery(
        ['PendingCondemApprovalList', condem_mast_slno],
        async () => {
            const result = await axioslogin.get(
                `AssetCondemnation/getCondemnLlevelsApproved/${condem_mast_slno}`
            );
            return result.data?.data || [];
        },
        { enabled: !!condem_mast_slno }
    );

    useEffect(() => {
        if (isSuccess) {
            setAllVerifiedList(verifiedList);
        } else {
            setAllVerifiedList([]);
        }
    }, [isSuccess, verifiedList]);
    ...
    const ApproveData = useMemo(() => {
        return {
            condem_level: level_no,
            condemn_level_state: 'A',
            level_review: remarks,
            level_employee: empId,
            level_review_status: 1,
            condem_mast_slno: condem_mast_slno,
            condemn_all_approved: checkActiveLevel === 1 ? 1 : 0,
            condemn_rejected: 0,
            level_review_slno: level_review_slno
        };
    }, [level_no, remarks, empId, condem_mast_slno, checkActiveLevel, level_review_slno]);

    const Approve = useCallback(
        (e) => {
            e.preventDefault()
            const FormUpdate = async (ApproveData) => {
                const result = await axioslogin.patch('/AssetCondemnation/CondemnUpdateLevel', ApproveData)
                const { message, success } = result.data
                if (success === 2) {
                    succesNotify(message)
                    queryClient.invalidateQueries('PendingCondemApprovalList')
                    CloseModal()
                } else {
                    infoNotify(message)
                }
    ...
    const RejectData = useMemo(() => {
        return {
            condem_level: level_no,
            condemn_level_state: 'R',
            level_review: remarks,
            level_employee: empId,
            level_review_status: 1,
            condem_mast_slno: condem_mast_slno,
            condemn_all_approved: 0,
            condemn_rejected: 1,
            level_review_slno: level_review_slno
        };
    }, [level_no, remarks, empId, condem_mast_slno, checkActiveLevel, level_review_slno]);
    ...
    const Reject = useCallback(
        (e) => {
            ...
            const FormUpdate = async (RejectData) => {
                const result = await axioslogin.patch('/AssetCondemnation/CondemnUpdateLevel', RejectData)
                const { message, success } = result.data
                if (success === 2) {
                    succesNotify(message)
                    queryClient.invalidateQueries('PendingCondemApprovalList')
                                        ...
                                        return (
                                            <tr key={index} style={{
                                                ...
                                                <td style={{ width: 150, textAlign: 'center' }}>
                                                    <Checkbox
                                                        checked={(val.keep_inscarp_status === 1)}
                                                        readOnly
                                                    />
                                                </td>
                            ...
                            >
                                ...
                                <tbody>
                                    ...
                                    {hod_review_date !== null ? < tr >
                                        ...
                                        : null}
                                    {allVerifiedList.length > 0 && (
                                        allVerifiedList.map((val, index) => (
                                            <tr key={index}>
                                                <td style={{ textAlign: 'center', width: 10 }}>
                                                    {val.level_name}
                                                </td>
                                                ...
                                                <td style={{ textAlign: 'center', width: 10 }}>
                                                    {val.level_review_date
                                                        ? format(new Date(val.level_review_date), 'dd MMM yyyy,  hh:mm a')
                                                        : 'Invalid Date'}
                                                </td>
                                            </tr>
                                        ))
...
export default memo(EditCondemnAtLevel)