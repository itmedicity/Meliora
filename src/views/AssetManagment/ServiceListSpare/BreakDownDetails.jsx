      ...
      {alldetailsService.length !== 0 ? (
        <Box>
          {alldetailsService?.map((val, index) => {
            return (
              <Box key={index} sx={{ ml: 0.5, my: 0.8, border: 1, py: 2, mr: 1 }}>
                <Box sx={{ display: 'flex' }}>
                  ...
                  <Box sx={{ flex: 1 }}>
                    ...
                    <Box sx={{ flex: 1, display: 'flex' }}>
                      ...
                      <Box sx={{ flex: 4, fontWeight: 600, color: 'Black', fontSize: 13, pt: 0.5 }}>
                        <Box
                    ...
                    <Box sx={{ mr: 2, my: 1 }}>
                      <Table stickyHeader size="sm" sx={{ borderRadius: 2 }} borderAxis="x">
                        <thead>
                          <tr style={{ backgroundColor: '#EBEFFB' }}>
                            <th style={{ width: 5, fontWeight: 700 }}>Slno</th>
                            <th style={{ width: 15, fontWeight: 700 }}>Attended by</th>
                            <th style={{ width: 15, fontWeight: 700 }}>Serviced Date</th>
                            <th style={{ width: 30, fontWeight: 700 }}>Issues Identified</th>
                            <th style={{ width: 30, fontWeight: 700 }}>Remarks</th>
                          </tr>
                        </thead>
                        <tbody>
                          {deptServiceempList[val.serviced_emp_details_slno]?.map((emp, index) => (
                            <tr key={index}>
                              ...
                              <td>{emp.service_issues_identified}</td>
                              <td>{emp.serviced_issue_remarks}</td>
                            </tr>
                          ))}
                        </tbody>
                      </Table>
                    ...
                    <Typography sx={{ fontWeight: 700, mt: 1.5 }}>Serviced Details</Typography>
                    <Box sx={{ flex: 1, display: 'flex' }}>
                    ...
                    <Box sx={{ flex: 1, display: 'flex' }}>
                      <Typography sx={{ flex: 1, fontSize: 15 }}>Supplier</Typography>
                      <Typography sx={{ flex: 4, fontWeight: 600, color: 'Black', fontSize: 13 }}>
                        : {val.serviced_supplier || 'Not updated'}
                      </Typography>
                    </Box>
                    <Box sx={{ flex: 1, display: 'flex' }}>
                    ...
                    <Box sx={{ flex: 1, display: 'flex' }}>
                      <Typography sx={{ flex: 1, fontSize: 15 }}>Marked Employee</Typography>
                      <Typography sx={{ flex: 4, fontWeight: 600, color: 'Black', fontSize: 13 }}>
