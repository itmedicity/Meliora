    ...
    return (
        <Paper sx={{ borderRadius: 0, height: '90vh' }}>
            <CssVarsProvider>
                ...
                <Box sx={{ m: .5, border: 1, borderColor: '#D0D0D0', p: 1, borderRadius: 2, flex: 1, }}>
                    <Box sx={{ flex: 1, display: 'flex', flexWrap: 'wrap', gap: .5 }}>
                        {CustDept.map((dept) => {
                            const icon = departmentIcons[dept.am_custodian_slno] || <HiveIcon sx={{ height: '100%', width: '100%', color: '#2C2743' }} />;
                            const isSelected = selectedDept === dept.am_custodian_slno;
                            return (
                                <Box key={dept.am_custodian_slno} sx={{
                                    p: 1, minWidth: 180, borderRadius: 8,
                                    ...
                                    <Box sx={{ flex: 1, display: 'flex', height: 30, gap: 1 }}>
                                        <TextComponent sx={{ flex: 1, fontSize: 13, fontWeight: 800, pt: 1, color: 'white' }} text={dept.am_custodian_name} />
                                        <Box sx={{}}>
                                            {icon}
                                        </Box>
                                    </Box>
                ...
                <Box sx={{ flex: 1, m: .5, border: 1, p: 1, borderColor: '#D0D0D0' }}>

                    <Box sx={{ flex: 1, }}>
                        <TextComponent
                            text={`Total Count : ${AssetList.length}`}
                            sx={{ fontWeight: 600, color: '#2C2743', pb: .5 }} />
                    </Box>
                    <Box sx={{
                        display: 'flex', borderBottom: 1, borderTop: 1, borderColor: 'lightgray', bgcolor: 'white', flex: 1,
                    }}>
