...
const MyPerformance = () => {
  ...
  const [completedTasksWithoutchangingDuedate, setcompletedTasksWithoutchangingDuedate] =
    useState(0)
  ...
  const [empTotComplWithoutHold, setEmpTotComplWithoutHold] = useState(0)
  const TotalPerformance =
    EmpTotalWithouthold + empTotComplWithoutHold === 0
      ? 0
      : ((AdjustedCompleteion + completedcomplaints) /
          (EmpTotalWithouthold + empTotComplWithoutHold)) *
        100
  ...
  const [empMonthOnHold, setEmpMonthOnHold] = useState(0)
  ...
  useEffect(() => {
    const getAllTask = async () => {
      ...
      if (success === 2) {
        ...
        const cmptlWOneTimeDuedate = data.filter(
          item => item.tm_task_status === 1 && item.tm_mast_duedate_count === 1
        )
        const cmptlWTwoTimeDuedate = data.filter(
          item => item.tm_task_status === 1 && item.tm_mast_duedate_count === 2
        )
        const cmptlWThreeTimeDuedate = data.filter(
          item => item.tm_task_status === 1 && item.tm_mast_duedate_count === 3
        )
        const cmptlWMoreThreeTimeDuedate = data.filter(
          item => item.tm_task_status === 1 && item.tm_mast_duedate_count > 3
        )
        const totalWithOutHold = totalTasks - onHoldTasks
        const valueProgress =
          completedTasks + totalTasks === 0 ? 0 : (completedTasks / totalTasks) * 100
        const penaltyCounts = {}
        data.forEach(item => {
    ...
    const getAllComplaints = async () => {
      ...
      if (success === 2) {
        ...
        const Completedcomplaints =
          RectifiedComplints + VerifiedComplints === 0 ? 0 : RectifiedComplints + VerifiedComplints
        ...
        const Pending = data.filter(
          item =>
            (item.cm_rectify_status !== 'R' &&
              item.cm_rectify_status !== 'O' &&
              item.cm_rectify_status !== 'V') ||
            item.cm_rectify_status === null
        )
        const totallWithoutHold = totalComplaints - OnholdComplaints
        const ComplaintEmpProgress =
          Completedcomplaints + totalComplaints === 0
            ? 0
        ...
        const ComplaintPerformnce =
          Completedcomplaints + totallWithoutHold === 0
            ? 0
            : (Completedcomplaints / totallWithoutHold) * 100
        ...
        setEmpTotComplWithoutHold(totallWithoutHold)
      } else {
  ...
  useEffect(() => {
    const getPendingComplaints = async () => {
      ...
      if (success === 2) {
        ...
        setMypendingToday(pendingToday.length)
      } else {
  ...
  return (
    <Box sx={{ flex: 1 }}>
      <Box sx={{ flex: 1, justifyContent: 'flex-end', pt: 0.6 }}>
        <Box
          sx={{
            width: 180,
            margin: 'auto',
        ...
        <Box sx={{ flex: 1 }}>
          <Box sx={{ flex: 1 }}>
            ...
            >
              ...
              <Box sx={{ flex: 1 }}>
                <Typography sx={{ fontSize: 29, fontWeight: 600, color: 'grey', pl: 0.8 }}>
                  My Performance
                </Typography>
                <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'grey', pl: 6 }}>
                  Task & complaints
                </Typography>
                <Box sx={{ display: 'flex', pt: 1, pl: 2 }}>
                  ...
                  <Box>
                    <Typography
                      sx={{ fontSize: 12, fontWeight: 600, color: '#12467B', pl: 0.5, pr: 1 }}
                    >
                      Task
                    </Typography>
                    <Typography
                  ...
                  <Box>
                    <Typography
                      sx={{ fontSize: 12, fontWeight: 600, color: '#492B08', pl: 0.5, pr: 1 }}
                    >
                      Complaints
                    </Typography>
                    <Typography
          ...
          <Box sx={{ flex: 1, mx: 0.5, mt: 1.5, minHeight: 80 }}>
            ...
            {empProject.length !== 0 ? (
                ...
                {empProject?.map(val => {
                  ...
                  return (
                    <Box
                      ...
                      sx={{ flex: 1, mb: 1, mx: 0.5, borderColor: 'lightgray', pb: 1 }}
                    >
                      <Box
                        sx={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}
                        onClick={() => toggleProjectOpen(val.tm_project_slno)}
                      >
                        {isOpen ? (
              ...
              <Box>
                <Typography sx={{ fontWeight: 600, color: 'lightgrey', pl: 5, pt: 3 }}>
                  No Projects Due in {monthName}
                </Typography>
              </Box>
            )}
          </Box>
          <Box sx={{ flex: 1, mx: 0.5, mt: 1, minHeight: 120 }}>
            <Typography sx={{ fontWeight: 600, pt: 0.5, pl: 2 }}>Tasks</Typography>
            {mainTasks.length !== 0 ? (
              <Box sx={{ mt: 1, overflow: 'auto', maxHeight: 150 }}>
                {mainTasks?.map(val => {
                  const isOpen = openTask[val.tm_task_slno]
                  return (
                    <Box key={val.tm_task_slno} sx={{ flex: 1, mb: 1, mx: 0.5, pb: 1 }}>
                      <Box
                        sx={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}
                        onClick={() => toggleTaskOpen(val.tm_task_slno)}
                      >
                        {isOpen ? (
                          <ArrowDropDownIcon sx={{ color: '#92443A', bgcolor: '#F7F7F7' }} />
                        ) : (
        ...
        <Box sx={{ flex: 1.7, mr: 1 }}>
          <Box sx={{ flex: 1 }}>
            <Box sx={{ flex: 1, mx: 0.5 }}>
              <Typography sx={{ pl: 1.5, fontWeight: 600, color: '#12467B', fontSize: 15 }}>
                Task
              </Typography>
              <Typography sx={{ pl: 2, pt: 0.5, fontSize: 15, color: '#12467B' }}>
                {completedTaskCount}/{empTotalTask}
              </Typography>
              <Box sx={{ px: 1.5 }}>
                <CssVarsProvider>
                  <LinearProgress determinate value={taskProogress} />
                </CssVarsProvider>
              </Box>

              <Box sx={{ flex: 1, display: 'flex' }}>
                <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: 1, ml: 1.5, mr: 0.5, height: 120 }}>
                  ...
                  <Box sx={{ flex: 1, display: 'flex' }}>
                    <Typography sx={{ pl: 1.5, pt: 1, fontSize: 12, flex: 1 }}>
                      Completed{' '}
                    </Typography>
                    <Box
                      sx={{
                ...
                <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: 1, mx: 0.5 }}>
                  <Typography sx={{ pt: 0.5, fontWeight: 600, pl: 1.5 }}>
                    <u>{monthName}</u>
                  </Typography>
                  <Box sx={{ flex: 1, display: 'flex' }}>
                  ...
                  <Box sx={{ flex: 1 }}>
                    <Box sx={{ display: 'flex' }}>
                      <Typography sx={{ pl: 1.2, pt: 1, fontSize: 12, flex: 1 }}>
                        ...
                        {completedTaskDetailsOpen ? (
                          <ArrowDropUpIcon
                            sx={{ cursor: 'pointer' }}
                            onClick={toggleCompletedTaskDetails}
                          />
                        ) : (
                          <ArrowDropDownIcon
                            sx={{ cursor: 'pointer' }}
                    ...
                    {completedTaskDetailsOpen && (
                      <Box sx={{ flex: 1, pl: 3, pr: 3, pt: 1, mb: 2, bgcolor: '#E9EEF7' }}>
                        <Box
                          sx={{ flex: 1, display: 'flex', borderBottom: 1, borderColor: '#DFE3ED' }}
                        >
                          <Typography sx={{ flex: 1, fontSize: 12, pl: 0.5, color: '#3A230A' }}>
                            On Schedule date
                          </Typography>
                          <Typography sx={{ pr: 1.5, color: '#3A230A' }}>
                            <u>{completedTasksWithoutchangingDuedate}</u>
                          </Typography>
                        </Box>
                        <Box
                          sx={{ flex: 1, display: 'flex', borderBottom: 1, borderColor: '#DFE3ED' }}
                        >
                          <Typography sx={{ flex: 1, fontSize: 12, pl: 0.5, color: '#3A230A' }}>
                            {' '}
                            One-time duedate extension{' '}
                          </Typography>
                          <Typography sx={{ pr: 1.5, color: '#3A230A' }}>
                            <u>{cmptlWithOneTimeDuedate}</u>
                          </Typography>
                        </Box>
                        <Box
                          sx={{ flex: 1, display: 'flex', borderBottom: 1, borderColor: '#DFE3ED' }}
                        >
                          <Typography sx={{ flex: 1, fontSize: 12, pl: 0.5, color: '#3A230A' }}>
                            Two-time duedate extension
                          </Typography>
                          <Typography sx={{ pr: 1.5, color: '#3A230A' }}>
                            <u>{cmptlWithTwoTimeDuedate}</u>
                          </Typography>
                        </Box>
                        <Box
                          sx={{ flex: 1, display: 'flex', borderBottom: 1, borderColor: '#DFE3ED' }}
                        >
                          <Typography sx={{ flex: 1, fontSize: 12, pl: 0.5, color: '#3A230A' }}>
                            Three-time duedate extension
                          </Typography>
                          <Typography sx={{ pr: 1.5, color: '#3A230A' }}>
                            <u>{cmptlWithThreeTimeDuedate}</u>
                          </Typography>
                        </Box>
                        <Box
                          sx={{ flex: 1, display: 'flex', borderBottom: 1, borderColor: '#DFE3ED' }}
                        >
                          <Typography sx={{ flex: 1, fontSize: 12, pl: 0.5, color: '#3A230A' }}>
                            More than 3 time duedate extension
                          </Typography>
                          <Typography sx={{ pr: 1.5, color: '#3A230A' }}>
                            <u>{cmptlWithMoreThreeTimeDuedate}</u>
                          </Typography>
                        </Box>
                      </Box>
                    )}
            ...
            <Box sx={{ flex: 1, m: 0.5 }}>
              <Typography
                sx={{ pl: 1.5, pt: 0.5, fontWeight: 600, color: '#492B08', fontSize: 18 }}
              >
                Complaints
              </Typography>
              <Typography sx={{ pl: 2, pt: 0.3, fontSize: 15, color: '#492B08' }}>
                {completedcomplaints}/{empTotalComplaints}
              </Typography>
              <Box sx={{ px: 1.5 }}>
                <CssVarsProvider>
                  <LinearProgress color="warning" determinate value={empLinearComptProg} />
                </CssVarsProvider>
              </Box>

              <Box sx={{ flex: 1, display: 'flex', mx: 0.5 }}>
