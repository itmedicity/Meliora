...
const EditSubtaskEmp = ({
  ...
  const [completedSub, setCompletedSub] = useState(
    tm_task_status === 1
      ? true
      : tm_task_status === 2
        ? false
  ...
  const InsertProgressSub = useCallback(
    e => {
      e.preventDefault()
      if (tm_progres_date !== '') {
  ...
  const UpdateProgressSub = useCallback(
    e => {
      e.preventDefault()
      if (tm_progres_date !== '') {
            ...
            if (employeeSubTask !== 0) {
              Inactiveemp(inactive).then(value => {
                ...
                if (succes === 1) {
                  ...
                  if (success === 1) {
                    if (
                      (completedSub === true && completedremarksSub === null) ||
                      (onHoldSub === true && onholdremarksSub === null) ||
      ...
      <Box>
        {dueDateModalFlag === 1 ? (
          <DueDateModal
            ...
            tm_task_due_date={subTaskDueDate}
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        <Box sx={{ flex: 1, mr: 1 }}>
          ...
          {changeAssignee === 0 ? (
            <Box sx={{ display: 'flex' }}>
              <Box sx={{ flex: 1 }}>
                <Textarea type="text" name="em_name" value={em_name} disabled style={{ minHeight: 57 }}></Textarea>
              </Box>
              <Box sx={{ pt: 2 }}>
        ...
        <Box sx={{ flex: 1.5, mr: 0.5 }}>
          <Box sx={{ color: '#000C66', fontFamily: 'Georgia', pl: 0.5 }}>Description</Box>
          <Textarea
            type="text"
            size="sm"
