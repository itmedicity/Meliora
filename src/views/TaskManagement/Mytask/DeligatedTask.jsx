...
const DeligatedTask = ({ projectcount, setprojectcount }) => {
    ...
    const fetchTasks = async (id) => {
        if (!id) return [];
        const result = await axioslogin.get(`/TmTableView/empDelegatedTasks/${id}`);
    ...
    const filteredData = AllTaskData.filter((row) =>
        Object.values(row)
            ...
            .toLowerCase()
            .includes(filterText.toLowerCase()) // match against input
    );
        ...
        <Box sx={{ flexGrow: 1 }}>
            {editModalFlag === 1 ?
                <ModalEditTask open={editModalOpen} masterData={masterData} setEditModalOpen={setEditModalOpen}
                    setEditModalFlag={setEditModalFlag} projectcount={projectcount}
            ...
            <Box sx={{ px: 1, height: '64vh', overflow: 'auto', }}>
                {isLoading ? (
...
export default memo(DeligatedTask)