...
const PerformanceMain = () => {
  ...
  let capdeeptName = deeptName.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
  let capsecName = secName.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
  ...
  const TotalPerformance = (EmpTotalWithouthold + empTotalComplWithouthold) === 0 ? 0 : (((AdjustedCompleteion + emptotCompltedComplaints) / (EmpTotalWithouthold + empTotalComplWithouthold)) * 100)
  ...
  useEffect(() => {
    const getDeptMasterTable = async () => {
      ...
      if (data.length !== 0) {
        if (success === 2) {
          const deptcompletedToday = data.filter(item => item.tm_task_status === 1 && isToday(item.tm_complete_date)).length;
  ...
  return (
    <Paper sx={{ p: .5, }} >
      <Box sx={{ bgcolor: ' #DFE3ED', height: '95%', p: .5 }}>
        <Box sx={{ bgcolor: 'white', height: '100%', flex: 1, pb: 1 }}>
          ...
          <Box sx={{ flex: 1, display: 'flex', height: '100%' }}>
                  ...
                  <Box sx={{ flex: 1.2, }}>
                    <Box sx={{ flex: 1, display: 'flex', }}>
                      <Box sx={{ width: 250, height: 250, ml: 3, mt: 3, }}>
                        <CssVarsProvider>
                          <CircularProgress determinate
                            ...
                            sx={{
                              '--CircularProgress-size': '250px', p: 2,
                      ...
                      <Box sx={{ flex: 1, pt: 5, pl: 2 }}>
                        <Typography sx={{ color: '#848484', fontWeight: 600, fontSize: 30 }}>{employeeNamee}</Typography>
                        <Typography sx={{ color: '#4C4C4C', fontWeight: 200, fontSize: 18, }}>{designation}</Typography>
                        <Typography sx={{ color: '#4C5270', fontWeight: 800, fontSize: 18, }}>Monthly Performance</Typography>
                        <Typography sx={{
                          color: MonthlyPerformance < 30 ? 'darkRed' : MonthlyPerformance < 70 ? '#32383E' : '#104210',
                          fontWeight: 800, fontSize: 28, pl: 8, py: 1
                        }}>{MonthlyPerformance}%</Typography>

                        <Box sx={{ display: 'flex', mt: .5 }}>
                          <Box sx={{ width: 7, height: 48, bgcolor: 'lightgrey', }}></Box>
                          <Box>
                            <Typography sx={{ fontSize: 14, fontWeight: 600, color: '#12467B', pl: .5, pr: 1 }}>Task</Typography>
                            <Typography sx={{
                              fontSize: 18, fontWeight: 600,
                              color: taskPerformance < 30 ? 'darkRed' : taskPerformance < 70 ? '#32383E' : '#104210', pl: .5, pr: 1
                            }}>{taskPerformance}%</Typography>
                          </Box>
                          <Box sx={{ width: 7, height: 48, bgcolor: 'lightgrey', ml: 5 }}></Box>
                          <Box>
                            <Typography sx={{ fontSize: 14, fontWeight: 600, color: '#492B08', pl: .5, pr: 1 }}>Complaints</Typography>
                            <Typography sx={{
                              fontSize: 18, fontWeight: 600,
                              color: complPerfm < 30 ? 'darkRed' : complPerfm < 70 ? '#32383E' : '#104210',
                              pl: .5, pr: 1
                            }}>{complPerfm}%</Typography>
                          </Box>
                        </Box>
                        ...
                        <Box sx={{ minHeight: 50, maxHeight: 200, overflow: 'auto', }}>
                          {
                            empProject && empProject.map((val) => {
                              return <Box key={val.tm_project_slno}
                                sx={{ display: 'flex' }}>
                                <Box>
                                  <AccountTreeIcon sx={{ color: '#492B08', p: .5 }} />
                                </Box>
                                <Box sx={{ flex: 1 }}>
                                  {val.tm_project_name}
                                </Box>
                    ...
                    <Box sx={{ flex: 1, mr: 1, }}>
                      ...
                      {emplmainTasks.length !== 0 ?
                        <Box sx={{ minHeight: 100, maxHeight: 250, overflow: 'auto' }}>
                          {
                            emplmainTasks && emplmainTasks.map((val) => {
                              return <Box key={val.tm_task_slno}
                                sx={{ display: 'flex' }}>
                                <Box>
                                  <DeviceHubSharpIcon sx={{ color: '#12467B', p: .5 }} />
                                </Box>
                                <Box sx={{ flex: 1 }}>
                                  {val.tm_task_name}
                                </Box>
                        ...
                        <Box>
                          <Typography sx={{ fontWeight: 600, color: 'lightgrey', pl: 5, pt: 3 }}>No Task Due Under Month </Typography>
                        </Box>}
                    </Box>
                  </Box>
                  <Box sx={{ flex: .9, m: 1, }}>
                    <Box sx={{ flex: 1, pt: 1, }}>
                      <Box sx={{ flex: 1, }}>
                        ...
                        <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: .5, mx: 1, py: .5 }}>
                          ...
                          <Box sx={{ flex: 1, display: 'flex', }}>
                            ...
                            <Box sx={{ width: 50, height: 25, border: 1, mx: 2, mt: .5, textAlign: 'center', borderRadius: 2, borderColor: '#12467B', color: '#12467B', }}>
                              {empOerdueToday}
                            </Box>
                          </Box>
                          <Box sx={{ flex: 1, display: 'flex' }}>
                      ...
                      <Box sx={{ flex: 1, }}>
                        <Typography sx={{ pl: 1.5, pt: 1.5, fontWeight: 600, color: '#492B08', fontSize: 18 }}>Complaints</Typography>
                        <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: .5, mx: 1, py: .5 }}>
                          <Typography sx={{ pl: 1, pt: .5, fontWeight: 600 }}><u>Today&apos;s</u></Typography>
                          <Box sx={{ flex: 1, display: 'flex', }}>
            ...
            <Box sx={{ flex: .8, mx: 1, border: 1, borderColor: '#DFE3ED', pb: 1 }}>
              <Typography sx={{ flex: 1, display: 'flex', justifyContent: 'center', fontWeight: 600, fontSize: 18, pt: 1, color: '#32383E' }}>
                {capsecName} </Typography>
              <Typography sx={{ flex: 1, display: 'flex', justifyContent: 'center', fontWeight: 600, fontSize: 12, color: '#32383E' }}>
                ({capdeeptName})</Typography>
