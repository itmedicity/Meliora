  ...
  const TotalPerformance =
    EmpTotalWithouthold + empTotalComplWithouthold === 0
      ...
      : ((AdjustedCompleteion + emptotCompltedComplaints) /
          (EmpTotalWithouthold + empTotalComplWithouthold)) *
        100
  ...
  useEffect(() => {
    const getAllComplaints = async () => {
      const result = await axioslogin.post(
        '/TmGraph/getDeptComplaintsfromtodate',
        searchComplaintData
      )
      const { success, data } = result.data
      if (success === 2) {
        ...
        const verifiedToday = data.filter(
          item => item.cm_rectify_status === 'V' && isToday(item.cm_verfy_time)
        )
    ...
    const getEmployeeTask = async () => {
      ...
      if (success === 2) {
        ...
        const completedTasks = data.filter(
          item =>
            item.tm_task_status === 1 &&
            isSameMonth(parseISO(item.tm_complete_date), new Date(searchMonthAndYear))
        ).length
        const onHoldTasks = data.filter(
          item => isPastDue(item.tm_task_due_date) && item.tm_task_status === 3
        ).length
        ...
        <Box sx={{ bgcolor: 'white', height: '100%', flex: 1, pb: 1 }}>
          ...
          <Box sx={{ flex: 1, display: 'flex', height: '100%' }}>
                ...
                <Box sx={{ flex: 1, display: 'flex', height: '96%' }}>
                  <Box sx={{ flex: 1.2 }}>
                    <Box sx={{ flex: 1, display: 'flex' }}>
                      ...
                      <Box sx={{ flex: 1, pt: 5, pl: 2 }}>
                        ...
                        <Typography sx={{ color: '#4C4C4C', fontWeight: 200, fontSize: 18 }}>
                          {designation}
                        </Typography>
                  ...
                  <Box sx={{ flex: 0.9, m: 1 }}>
                    <Box sx={{ flex: 1, pt: 1 }}>
                      <Box sx={{ flex: 1 }}>
                        <Typography
                          sx={{ pl: 1.5, fontWeight: 600, color: '#12467B', fontSize: 18 }}
                        >
                          Task
                        </Typography>
                        <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: 0.5, mx: 1, py: 0.5 }}>
                          ...
                          <Box sx={{ flex: 1, display: 'flex' }}>
                            <Typography sx={{ pl: 1.5, pt: 1, fontSize: 15, flex: 1 }}>
                              Overdue{' '}
                            </Typography>
                            <Box
                              sx={{
                      ...
                      <Box sx={{ flex: 1 }}>
                        <Typography
                          sx={{ pl: 1.5, pt: 1.5, fontWeight: 600, color: '#492B08', fontSize: 18 }}
                        >
                          Complaints
                        </Typography>
                        <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: 0.5, mx: 1, py: 0.5 }}>
                ...
                <Box sx={{ flex: 1, display: 'flex', mb: 1 }}>
                  <Box sx={{ flex: 1, display: 'flex', pl: 0.5 }}>
                    <ArrowBackIosIcon
                      ...
                      onClick={handlePrev}
                    />
                    <Typography sx={{ fontSize: 12, pt: 0.5, pl: 0, color: 'grey' }}>
                      Prev
                    </Typography>
                  </Box>
            ...
            <Box sx={{ flex: 0.8, mx: 1, border: 1, borderColor: '#DFE3ED', pb: 1 }}>
              ...
              <Typography
                sx={{ pl: 1.5, pt: 1.5, fontWeight: 600, color: '#12467B', fontSize: 18 }}
              >
                Task
              </Typography>
              <Box sx={{ flex: 1, bgcolor: '#E9EEF7', mt: 0.5, mx: 1, py: 0.5 }}>
