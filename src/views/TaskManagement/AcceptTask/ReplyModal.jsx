...
const ReplyModal = ({ open, setReplyOpen, setReplyflag, valuee, setTableCount, tableCount }) => {
  ...
  const replyUpdate = useCallback(e => {
          ...
          >
            <Box>
              <Box sx={{ flex: 1, display: 'flex', mt: 1, p: 1 }}>
                <Box sx={{ flex: 1, color: 'grey' }}>replies</Box>
                <Box sx={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
                  <CancelIcon sx={{ color: 'darkred', cursor: 'pointer' }} onClick={QueryClose} />
                </Box>
              </Box>
              <Box sx={{ flex: 1, display: 'flex', bgcolor: '#D9DADE', py: 0.5 }}>
                ...
                <Box sx={{ flex: 1, textAlign: 'right' }}>
                  <Typography sx={{ pr: 5, fontWeight: 600, color: 'Black' }}>
                    <InsertInvitationSharpIcon sx={{ p: 0.3 }} />
                    Duedate
                  </Typography>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'flex-start', mt: 2 }}>
                        ...
                        <Box sx={{ ml: 0.5, mr: 10, minWidth: 200 }}>
                          ...
                          <Box sx={{ display: 'flex', fontSize: 11, mt: 0.5 }}>
                            <Box sx={{ pl: 0.5, flex: 1 }}>{capitalizeWords(val.query_user)}</Box>
