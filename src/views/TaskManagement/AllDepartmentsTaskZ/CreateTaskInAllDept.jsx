...
import { Avatar, Box, Checkbox, Chip, CssVarsProvider, Divider, Modal, ModalDialog, Textarea, Typography, } from '@mui/joy'
...
const CreateTaskInAllDept = ({ open, setAddModalFlag, setaddModalOpen, tableCount, setTableCount }) => {
    ...
    const EmployeeCheckbox = useCallback((e) => {
    ...
    const { tm_task_slno, tm_task_name, tm_task_due_date, tm_task_description, tm_onhold_remarks, tm_pending_remark, tm_completed_remarks, tm_complete_date, main_task_slno
    } = taskCreation
    ...
    const addTask = useCallback(() => {
        ...
        if (tm_task_due_date !== '' && tm_task_name !== '' && tm_task_description !== '' && department !== '' && deptSection !== '') {
    ...
    }, [arry, postArray, tm_task_name, deptIds, department, tm_task_due_date, tm_task_description, deptSection, assignees, projectz,
        goalz, id, tm_complete_date, tm_completed_remarks, tm_onhold_remarks, tm_pending_remark, value, main_task_slno, selectFile
    ]);
    ...
    const rowSelectFromTempTable = useCallback((val, index) => {
        setvalue(1);
        const { tm_task_name, tm_task_description, tm_task_due_date, tm_goals_slno, tm_onhold_remarks, tm_pending_remark, tm_completed_remarks,
            tm_complete_date, tm_project_slno, tm_task_dept, tm_task_dept_sec, employee, main_task_slno } = val;
    ...
    const SubmitTask = useCallback((e) => {
        const saveArray = postArray?.map((val) => {
            const sectiondata = val.tm_task_dept_sec?.map((value) => {
                const empdetails = val.employee?.filter((valemp) => valemp.dept_section === value.sec_id ? valemp.dept_section : 0)
                const obj = {
    ...
    const changeDepSec = useCallback((e) => {
        dispatch(getDeptSections(deptIds));
        setchangeDeptSec(1)
        setchangeEmp(1)
    }, [deptIds, dispatch])

    const changeAssignee = useCallback((e) => {
        setchangeEmp(1)
    ...
    return (
        <Box>
            ...
            {addProjectFlag === 1 ? <ProjectCreation open={addProjectModalOpen} setTableCount={setTableCount} tableCount={tableCount}
                setAddProjectFlag={setAddProjectFlag} setaddProjectlModalOpen={setaddProjectlModalOpen} dueDateGoal={dueDateGoal}
            /> : null}
            <CssVarsProvider>
                <Modal
                    ...
                    <ModalDialog variant="outlined" sx={{ width: '95vw', height: '100vh', p: 0, overflow: 'auto' }}>
                        <Box sx={{ flex: 1, }}>
                            <Box sx={{ flex: 1, display: 'flex', borderColor: 'lightgray', mt: 1, mx: 2 }}>
                                ...
                                <HighlightOffIcon sx={{ height: 25, width: 25, cursor: 'pointer', color: '#05445E', '&:hover': { color: '#BA0F30' } }} onClick={handleClose} />
                            </Box>
                            <Box sx={{ flex: 1, height: 30, bgcolor: '#0C4160', mt: 1 }}>
                            </Box>
                            <Box style={{
                                marginLeft: 55,
                                marginTop: "-0.99em",
                            ...
                            <Box sx={{
                                ...
                                <Box sx={{ flex: 2, display: 'flex' }}>
                                    <Box sx={{ flex: 1 }}>
                                        <Typography sx={{ pl: 1, fontWeight: 500, fontSize: 13, color: '#1D5183' }}>
                                            <RadarIcon sx={{ p: .3, color: '#4C5270' }} />
                                            Goal</Typography>
                                        <TmAllGoalsList
                                            goalz={goalz} setgoalz={setgoalz} setdueDateGoal={setdueDateGoal} />
                                    </Box>
                                    <Box sx={{ ml: .5, mt: 4.5 }} onClick={CreateGoal}>
                                ...
                                <Box sx={{ flex: 2, ml: 2.5, display: 'flex' }}>
                                    ...
                                    <Box sx={{ ml: .5, mt: 4.5 }} onClick={CreateProject}>
                            ...
                            <Box sx={{ display: 'flex', mx: 3, p: 1, mt: 1 }}>
                                <Box sx={{ flex: 1.5, }}>
                                    <Typography sx={{ pl: 1, fontWeight: 500, fontSize: 13, color: '#1D5183' }}>
                                        <GroupsIcon sx={{ p: .2, color: '#4C5270' }} />
                                                    ...
                                                    <Box sx={{ display: 'flex', mt: .5, }}>
                                                        ...
                                                        <Box sx={{ pt: .5 }}>
                                                            <Tooltip title="Change Department">
                                                                <ChangeCircleRoundedIcon sx={{ cursor: 'pointer' }}
                                                                    onClick={changeDepmt} />
                                                            </Tooltip>
                                                        </Box>
                                                    </Box>
                                ...
                                <Box sx={{ flex: 1.5, ml: 3 }}>
                                    <Typography sx={{ pl: 1, fontWeight: 500, fontSize: 13, color: '#1D5183' }}>
                                        <Groups3Icon sx={{ p: .2, color: '#4C5270' }} />
                                ...
                                <Box sx={{ flex: 1.8, ml: 3, display: 'flex', }}>
                                    ...
                                    <Typography sx={{
                                        fontSize: 10, pt: .5, pl: 1,
                                        width: '80px',

                                    }}
                                    >Select All HODs/Incharges
                                        Under Selected Section
                                    </Typography>
                            ...
                            <Box sx={{ flex: 1, display: 'flex', mx: 10, my: 5, }}>
                                <Box sx={{ flex: 18, }}>
                                    ...
                                    <Box sx={{
                                        flex: 1, border: 1, mx: 1, mt: 1, borderColor: '#8E8A92', py: 1.5, borderStyle: 'dashed',
                                        fontWeight: 500, display: 'flex'
                                    }}>
                                        <Box sx={{
                                            color: '#0000FF', cursor: 'pointer', '&:hover': { color: '#000C66' }, textAlign: 'center', width: 165, border: .1,
                                        ...
                                        <Box sx={{ display: 'flex', flex: 1, overflowX: 'auto', mx: .5 }}>
                                ...
                                <Box sx={{ flex: .1, pt: 4, px: 1, mx: .5, mt: 3, borderRadius: 5 }}>
                                    <Tooltip title={'Add More Task'}>
                                        <Avatar sx={{ bgcolor: '#E4D4C8' }} >
                                            <AddCircleIcon sx={{ color: '#523A28', cursor: 'pointer', '&:hover': { color: '#5C4E4E' }, height: 30, width: 30 }}
                                                onClick={addTask} />
                                        </Avatar>
                                    </Tooltip>
                                </Box>
                            ...
                            <Box sx={{ mt: 4, mx: 3, }}>
                                <Divider sx={{ '--Divider-childPosition': `5%`, fontWeight: 600, fontSize: 18, color: '#003B73', mb: 1, mt: 2, mx: 1 }}>Task List</Divider>
                            </Box>
                            ...
                            <Box sx={{
                                ...
                                <Box sx={{ width: 100, textAlign: 'center', fontWeight: 600, color: '#444444', fontSize: 12 }}>Action</Box>
                                ...
                                <Box sx={{ width: 400, fontWeight: 600, color: '#444444', fontSize: 12, pl: .8 }}>Department Section</Box>
                            ...
                            <Box>
                                {arry?.map((val, index) => {
                                    return <Box
                                        ...
                                        <Box sx={{ width: 100, textAlign: 'center', fontWeight: 600, color: 'grey', fontSize: 12 }}>
                                            <EditIcon
                                                sx={{ cursor: 'pointer', '&:hover': { color: '#003060' } }} size={6}
                                                onClick={() => rowSelectFromTempTable(val, index)}
                                            />
                                        </Box>
                                        <Box sx={{ width: 800, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1 }}>
                                            <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12, mb: .2, }}>{val.tm_task_name}</Chip>
                                        </Box>
                                        <Box sx={{ width: 250, fontWeight: 600, color: 'grey', fontSize: 12, textTransform: 'capitalize', pl: 1 }}>
                                            <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12 }}>{format(new Date(val.tm_task_due_date), "dd-MM-yyyy HH:mm:ss")}</Chip>
                                        </Box>
                                        <Box sx={{ width: 400, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1, }}>
                                            {
                                                val.tm_task_dept && val.tm_task_dept.map((value) => {
                                                    return <Box key={value.dept_id}><Chip sx={{ bgcolor: '#EFEDC6', mb: .2, fontSize: 12 }}> {value.dept_name}</Chip></Box>
                                                })
                                            }
                                        </Box>
                                        <Box sx={{ width: 400, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1 }}>                                            {
                                            val.tm_task_dept_sec && val.tm_task_dept_sec.map((value) => {
                                                return <Box key={value.sec_id}><Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12, mb: .2, }}>{value.sec_name}</Chip></Box>
                                            })
                                        }
                                        </Box>
                                        <Box sx={{ width: 500, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1, }}>
                                            {
                                                val.employee && val.employee.map((value) => {
                                                    return <Box key={value.emp_id} > <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12, mb: .2, }}>{value.em_name}</Chip></Box>
                                                })
                                            }
                                        </Box>
                                        <Box sx={{ width: 800, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1 }}>
                                            <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12, mb: .2, }}>{val.tm_task_description}</Chip>
                                        </Box>
                                    </Box>
                                })}
                            </Box>
                        </Box>
                        <Box sx={{ display: 'flex', justifyContent: "flex-end", height: 60, borderTop: 1, borderColor: 'lightgrey', pt: 2 }}>
