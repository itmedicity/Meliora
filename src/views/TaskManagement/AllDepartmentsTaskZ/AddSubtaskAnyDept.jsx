...
const AddSubtaskAnyDept = ({ tm_task_slno, tm_project_slno, setflag, tableRendering, setTableRendering, tm_task_due_date, setTableCount, tableCount }) => {
    ...
    const { tm_subtask_name, tm_subtask_duedate, tm_subtask_description, tm_onhold_remarkSubtask, tm_complete_dateSubtask, tm_pending_remarkSubtask,
        tm_completed_remarkSubtask
    } = subTaskMast
    ...
    const addTask = useCallback(() => {
        ...
        if (tm_subtask_duedate !== '' && tm_subtask_name !== '' && tm_subtask_description !== '' && subtaskdepartment !== '' && subtaskdeptSection !== '') {
        ...
        arry, postArray, tm_subtask_name, deptIds, subtaskdepartment, tm_subtask_duedate, tm_subtask_description, subtaskdeptSection, subtaskAssignees, tm_project_slno,
        id, tm_complete_dateSubtask, tm_task_slno,
        tm_completed_remarkSubtask,
        tm_onhold_remarkSubtask,
        tm_pending_remarkSubtask,
        value
    ...
    const rowSelectFromTempTable = useCallback((val, index) => {
        setvalue(1);
        const { tm_subtask_name, tm_subtask_description, tm_subtask_duedate, tm_onhold_remarkSubtask, tm_pending_remarkSubtask, tm_completed_remarkSubtask,
            tm_complete_dateSubtask, tm_project_slno, tm_subtask_dept, tm_subtask_dept_sec, employee, Subtask_create_id, main_task_slno, tm_subtask_status } = val;
    ...
    return (
        <Box>
            ...
            <Box sx={{ flex: 1, display: 'flex', mx: 7, mt: 3, mb: 5 }}>
                ...
                <Box sx={{ flex: .1, pt: 3, px: 1, mx: .5, mt: 1, borderRadius: 5 }}>
            ...
            {arry.length !== 0 ?
                <>
                    <Box sx={{ mx: 3, mt: 3, flex: 1, }}>
                        <Divider sx={{ '--Divider-childPosition': `5%`, fontWeight: 600, fontSize: 18, color: '#523A28', mb: 1, mt: 2, mx: 3, }}>Subtask creation List</Divider>
                    </Box>
                    <Box sx={{
                        flex: 1,
                        height: 45, mt: .5, display: 'flex', borderBottom: 1, borderTop: 1, borderColor: 'lightgray', pt: 1.5, mx: 6,
                        bgcolor: 'white'
                    }}>
                    ...
                    <Box>
                        {arry?.map((val, index) => {
                            return <Box key={index} sx={{
                                flex: 1, mx: 6,
                                display: 'flex', mt: .3, borderBottom: 2, borderColor: 'lightgrey', minHeight: 30,
                                background: 'white',
                                pt: .5,
                            }}>
                                ...
                                <Box sx={{ width: 800, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1 }}>
                                    <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12, mb: .2, }}>{val.tm_subtask_name}</Chip>
                                </Box>
                                <Box sx={{ width: 250, fontWeight: 600, color: 'grey', fontSize: 12, textTransform: 'capitalize', pl: 1 }}>
                                    <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12 }}>{format(new Date(val.tm_subtask_duedate), "dd-MM-yyyy HH:mm:ss")}</Chip>
                                </Box>
                                <Box sx={{ width: 400, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1, }}>
                                    {
                                        val.tm_subtask_dept && val.tm_subtask_dept.map((value) => {
                                            return <Box key={value.dept_id}><Chip sx={{ bgcolor: '#EFEDC6', mb: .2, fontSize: 12 }}> {value.dept_name}</Chip></Box>
                                        })
                                    }
                                </Box>
                                <Box sx={{ width: 400, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1 }}>                                            {
                                    val.tm_subtask_dept_sec && val.tm_subtask_dept_sec.map((value) => {
                                ...
                                <Box sx={{ width: 800, fontWeight: 600, color: 'grey', textTransform: 'capitalize', pl: 1 }}>
                                    <Chip sx={{ bgcolor: '#EFEDC6', fontSize: 12, mb: .2, }}>{val.tm_subtask_description}</Chip>
                                </Box>
                            </Box>
                        })}
                    </Box>
                    <Box sx={{ mr: .5, fontSize: 20, cursor: 'pointer', display: 'flex', justifyContent: "center", my: 1.5 }}>
                        <CssVarsProvider>
                            <Button variant="solid"
                                onClick={SubmitTask}
                                sx={{ fontSize: 16, color: 'white', bgcolor: '#6C3F3F' }} > + Add Subtask/s </Button>
                        </CssVarsProvider>
                    </Box>
                </> : null}
        </Box>
