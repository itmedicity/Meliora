            ...
            <Box sx={{ height: '60vh', overflow: 'auto' }}>
                {allEmpTask?.map((val, index) => {
                    return <Box key={index} sx={{
                        ...
                        <Box sx={{ flex: .5, pl: 1.7, fontWeight: 600, color: 'grey', fontSize: 12 }}>{index + 1}</Box>
                        <Box sx={{ flex: 1, fontWeight: 600, color: 'grey', fontSize: 12, }}>
                            {val.emslno}
                        </Box>
                        <Box sx={{ flex: 5, fontWeight: 600, color: 'grey', fontSize: 12, cursor: 'pointer' }}>&nbsp;
                            <PersonIcon sx={{ color: '#6A4973' }} /> {val.empname}
                        </Box>
                        <Box sx={{ flex: 1, color: 'green', pl: 4 }}>
                            [{val.TC || '0'}]
                        </Box>
