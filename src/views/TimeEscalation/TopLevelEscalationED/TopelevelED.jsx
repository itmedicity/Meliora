...
import { getescalationtoplvl } from 'src/redux/actions/TopLevelEscalation.action';
import ToplevelTable from './ToplevelTable'
import { getEscalationMapping } from 'src/redux/actions/EscalationMapping.action'
import { getEscalationMappingToplvlmaintenenace } from 'src/redux/actions/EscalationMappingToplvlMain.action';
import { getEscalationMappingToplvlIt } from 'src/redux/actions/EsclationMappingToplvlIt.action';
const TopelevelED = () => {
    ...
    // }, [dispatch])
    // const LevelED = useSelector((state) => {
    ...
    useEffect(() => {
        if (bc !== 0) {
            dispatch(getescalationtoplvl(timeescalationData)).then(() => {
                setCheck(2)
            })
        }
    ...
    const complaints = useSelector((state) => {
        return state.getescalationtoplvl.toplvlscalation
    })
    ...
    useEffect(() => {
        if (escmap.length !== 0) {
            dispatch(getEscalationMappingToplvlmaintenenace(postdata2))
            dispatch(getEscalationMappingToplvlIt(postdata))
        }
                                            ...
                                            newarray && newarray.map((val) => (
                                                <TableRow key={val.esc_slno} sx={{
                                                    ...
                                                    <TableCell
                                                        ...
                                                        sx={{
                                                            width: '20%', border: '1px solid',
                                                            borderColor: '#F0F3F5', p: 1,
                                                        }}>
