...
import { getescalationlvl2 } from 'src/redux/actions/Level2Escalation.action'
import Level2Table from './Level2Table'
import { getEscalationMapping } from 'src/redux/actions/EscalationMapping.action'
import { getEscalationMappingLvl2Main } from 'src/redux/actions/EscalationMappingLvl2Main.action'
import { getEscalationMappingLvl2It } from 'src/redux/actions/EscalationMappingLvl2It.action'
const Level2escalation = () => {
    ...
    // }, [dispatch])
    // const level2 = useSelector((state) => {
    ...
    useEffect(() => {
        if (bc !== 0) {
            dispatch(getescalationlvl2(timeescalationData)).then(() => {
                setCheck(2)
            })
        }
    ...
    const complaints = useSelector((state) => {
        return state.getEscalationlvl2.lvl2scalation
    })
    ...
    useEffect(() => {
        if (escmap.length !== 0) {
            dispatch(getEscalationMappingLvl2Main(postdata2))
            dispatch(getEscalationMappingLvl2It(postdata))
        }
    }, [escmap, dispatch, id, postdata2, postdata])
    // counts values
    const Counts = useSelector((state) => {
        return {
            countmain: state.getEscalationMappingLvl2Main.escalationMappingLvl2Main || 0,
            countit: state.getEscalationMappingLvl2It.escalationMappingLvl2It || 0
        }
    })
                                        ...
                                        <TableRow >
                                            ...
                                            <TableCell rowSpan='70' sx={{
                                                ...
                                                <Box sx={{
                                                    ...
                                                    {
                                                        ab === 1 && complaints.length !== 0 ? <Level2Table complaints={complaints} /> : null
                                                    }
                                        ...
                                        </TableRow>
                                        {
                                            newarray && newarray.map((val) => (
                                                <TableRow key={val.esc_slno} sx={{
                                                    ...
                                                    <TableCell
                                                        padding='none'
                                                        size='medium' variant='head'
                                                        sx={{
                                                            width: '20%', border: '1px solid', borderColor: '#F0F3F5', p: 1
                                                        }}>
                                                        <Grid container spacing={2} >
                                                            <Grid item xl={8} xs={8} sm={8} md={8} lg={8}
                                                                sx={{
                                                                    p: 0,
                                                                    mt: 0.5,
                                                                    height: 20
...
export default Level2escalation