...
import { CssVarsProvider, Modal, ModalClose, ModalDialog, Box, Typography, Checkbox, Button, Textarea, Chip } from '@mui/joy'
...
const ModalIncidentMarking = ({ modalData, open, handleClose, SearchReport }) => {
    ...
    useEffect(() => {
        if (modalData.length !== 0) {
            const { incident_flag, incident_reason, incident_details, incident_mark_remarks, final_incident_type,
                verified_date } = modalData
                ...
                >
                    ...
                    >
                            ...
                            <Box sx={{ flex: 1 }}>
                                <Box sx={{ display: 'flex' }}>
                                    <Box sx={{ flex: 1 }}>
                                        ...
                                        <Box sx={{ pl: 0.5 }}>
                                            <Typography sx={{ fontSize: 17 }}>{qi_dept_desc.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</Typography>
                                        </Box>
                                    </Box>
                                    <Box sx={{ flex: 1 }}>
                                        <Box>
                                        ...
                                        <Box sx={{ pl: 0.5 }}>
                                            <Typography sx={{ fontSize: 17 }}>{moment(new Date(incident_date)).format('DD-MM-yyyy hh:mm:ss A')}</Typography>
                                        </Box>
                                    </Box>
                                </Box>
                                    ...
                                    <Box sx={{ flex: 1, pt: 1.5 }}>
                                        ...
                                        <Box sx={{ pl: 0.5, pt: 0.5 }}>
                                            <select
                                                variant="outlined"
                                                style={{
                                                    height: 35, width: 250, paddingLeft: 7, borderRadius: 6,
                                    ...
                                    <Box sx={{ flex: 1, pt: 1.5 }}>
                                        ...
                                        <Box sx={{ pl: 0.5, pt: 0.5 }}>
                                            <CssVarsProvider>
                                                <Chip size="md"
                                                    variant="outlined"
                                                    sx={{ color: '#bf360c', height: 30 }}
                                                >{initial_incident_type === 1 ? 'GENERAL' : initial_incident_type === 2 ? 'NEAR MISSESS' :
                                                    initial_incident_type === 3 ? 'HARMFUL' : initial_incident_type === 4 ? 'SENTINEL' : 'Nil'}</Chip>
                                            </CssVarsProvider>
                                        </Box>
                                    </Box>
                                </Box>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', pb: 1 }}>
                            <CssVarsProvider>
                                <Button variant='plain' onClick={UpdateincidetData}
                                    style={{
                                        fontSize: 17,
                                        color: '#bf360c',
                                        cursor: 'pointer',
                                    }}>Save</Button>
                                <Button variant='plain' onClick={Reset} style={{
                                    fontSize: 17,
                                    color: '#bf360c',
                                    cursor: 'pointer',
                                }}>Cancel</Button>
