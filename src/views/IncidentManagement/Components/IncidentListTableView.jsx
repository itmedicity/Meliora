...
const IncidentListTableView = ({ tableData, SearchReport }) => {
    ...
    return (
        <Fragment>
            {incFlag === 1 ? < ModalIncidentMarking modalData={modalData} open={modalopen} handleClose={handleClose} SearchReport={SearchReport} /> : null}
            < Paper sx={{ overflow: 'auto', flex: 1 }}>
                <Box variant="outlined" sx={{ maxHeight: window.innerHeight - 240, }}>
                    <CssVarsProvider>
                                ...
                                {tableData?.map((val, index) => {
                                    return (
                                        < tr key={index} size='small'
                                            ...
                                            <td size='sm' style={{ fontSize: 13, height: 15 }}>{val.incident_mark_remarks}</td>
