                                    ...
                                    <Table aria-label="table with sticky header" borderAxis="both" padding={"none"} stickyHeader size='sm' stickyFooter hoverRow >
                                        <thead style={{ alignItems: 'center' }}>
                                            <tr style={{ height: 0.5 }}>
                                                ...
                                                <th size='sm' style={{ width: 200, borderRight: '1px solid white', textAlign: 'center', backgroundColor: '#cfd8dc', fontSize: 15 }}>Arrived Time</th>
                                                <th size='sm' style={{ width: 200, borderRight: '1px solid white', textAlign: 'center', backgroundColor: '#cfd8dc', fontSize: 15 }}>Assessment Start</th>
                                                <th size='sm' style={{ width: 200, borderRight: '1px solid white', textAlign: 'center', backgroundColor: '#cfd8dc', fontSize: 15 }}>Assessment End</th>
                                            ...
                                            {patList?.map((val, index) => {
                                                return (
                                                    < tr key={index} size='small' style={{ maxHeight: 2, cursor: 'pointer' }}>
                                                        ...
                                                        <td size='sm' style={{ fontSize: 13 }}>&nbsp;{format(new Date(val.patient_arrived_date), 'dd-MM-yyyy hh:mm a')}</td>
                                                        <td size='sm' style={{ fontSize: 13 }}>&nbsp;{format(new Date(val.assessment_start), 'dd-MM-yyyy hh:mm a')}</td>
                                                        <td size='sm' style={{ fontSize: 13 }}>&nbsp;{format(new Date(val.assessment_end), 'dd-MM-yyyy hh:mm a')}</td>
