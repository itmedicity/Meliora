        ...
        if (success === 1) {
          const newArray = data?.map(val => {
            ...
            return {
              ...
              ptname: val.ptname,
              qi_dept_no: qidept,
              patient_arrived_date: patientArrivedDate
                ? format(new Date(patientArrivedDate), 'dd-MM-yyyy hh:mm:ss a')
                : null,
              assessment_start: assessmentStartDate
                ? format(new Date(assessmentStartDate), 'dd-MM-yyyy hh:mm:ss a')
                : 'Not Updated',
              assessment_end: assessmentEndDate
                ? format(new Date(assessmentEndDate), 'dd-MM-yyyy hh:mm:ss a')
                : 'Not Updated',
              consult_start_date: consultStartDate
                ? format(new Date(consultStartDate), 'dd-MM-yyyy hh:mm:ss a')
                : 'Not Updated',
              consult_end_date: endTimeofConsult
                ? format(new Date(endTimeofConsult), 'dd-MM-yyyy hh:mm:ss a')
                : 'Not Updated',
    ...
    <Paper sx={{ height: '90vh' }}>
      ...
      <Paper square variant="outlined" sx={{ display: 'flex', pt: 0.4, flexWrap: 'wrap' }}>
        ...
        <Box sx={{ display: 'flex', m: 2, flex: 1 }}>
          ...
          <Box sx={{}}>
            <Box sx={{ pl: 2, fontSize: 12, color: '#0d47a1' }}>
      ...
      <>{searchFlag === 1 ? <OPConsultationWaitingReport tableData={tableData} /> : null}</>
    </Paper>
  )
}
