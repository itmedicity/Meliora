...
export const RefreshPatientList = async (qidept, count, setCount, qitype, depCode, id, dailyDate) => {
    const patchdata = {
    ...
    const InsertDialysisData = async (dialysisArray) => {
        const result = await axioslogin.post('/qidialysis/insertData', dialysisArray);
        return result.data
    }
            ...
            GetElliderData(elliderSearch).then((value) => {
                ...
                if (success === 1) {
                    ...
                    if (qitype === 1) {
                        ...
                        const newArray = data?.filter(val => val.DO_CODE === 'R027')
                        const dialysisArray = newArray?.map((val) => {
                            return {
                                patient_arrived_date: format(new Date(val.VSD_DATE), 'yyyy-MM-dd HH:mm:ss'),
                                ptno: val.PT_NO,
                                ptname: val.PTC_PTNAME,
                        ...
                        InsertDialysisData(dialysisArray).then((val) => {
                            const { success } = val
                            if (success === 1) {
                                setCount(count + 1)
                            }
