...
const DayWisePatientsListTable = ({ viewData, ipViewReport, qitype }) => {
    ...
    useEffect(() => {
        if (viewData.length !== 0) {
            ...
            if (ipViewReport.length !== 0) {
                ...
                const finalData = [...opdata, ...ipdata].sort((a, b) => new Date(a.endo_arrival_time) - new Date(b.endo_arrival_time));
    ...
    return (
        <Box>
            {qiflag === 1 ? <ModalDayWiseQIView open={modalopen} handleClose={handleClose} rowSelect={rowSelect} /> :
                qiflag === 2 ? <IPDayWiseModalView open={modalopen} handleClose={handleClose} rowSelect={rowSelect} /> :
                    null}
            {tableData.length !== 0 ?
                        ...
                        <Table aria-label="table with sticky header" borderAxis="both" padding={"none"} stickyHeader size='sm' stickyFooter hoverRow >
                            <thead style={{ alignItems: 'center' }}>
                                <tr style={{ height: 0.5 }}>
                                    ...
                                    <th size='sm' style={{ width: 50, fontWeight: 650, fontSize: 14 }}>&nbsp;Type</th>
                            ...
                            <tbody size='small'>
                                {tableData?.map((val, index) => {
                                    return (< tr key={index} size='small'
                                        ...
                                        <td size='sm' style={{ fontSize: 12, height: 5 }}>&nbsp;{val.type}</td>
