...
const QiApprovalView = ({ viewData, qidept, searchDate, setsearchFlag, ipViewReport, testCount, equipmentlist, endoDays }) => {
    ...
    const [inchrgeStatus, setInchrgeStatus] = useState(0)

    const [monthReport, setMonthReport] = useState({
        totalTest: 0, totalPat: 0, totalError: 0, totalRedos: 0, totalTime: 0, totalIdentifctn: 0, totalFalls: 0, totalSentinel: 0,
        totalSentinelAnalysed: 0, totalNearMisses: 0, errorResult: 0, redosResult: 0, timeResult: 0, idetifctionResult: 0,
        fallsResult: 0, sentinelResult: 0, nearMissessResult: 0, totIncidentReported: 0, totEquipTime: 0, totEquipAvailable: 0,
        equipResult: 0
    })
    const { totalTest, totalPat, totalError, totalRedos, totalTime, totalIdentifctn, totalFalls, totalSentinel, totalSentinelAnalysed,
        totalNearMisses, errorResult, redosResult, timeResult, idetifctionResult, fallsResult, sentinelResult,
        nearMissessResult, totIncidentReported, totEquipTime, totEquipAvailable, equipResult } = monthReport

    const [headerNames, setHeaderNames] = useState({
        header1: '', header2: '',
    })
    const [hodDetails, setHodDetails] = useState({
        hodStatus: 0, hodName: '', hodDate: ''
    })
    ...
    useEffect(() => {
        if (viewData.length !== 0 || ipViewReport.length !== 0) {
            ...
            const optime = (viewData?.filter(val => val.qi_save_status === 1).reduce((acc, curr) => acc + (curr.sumof_service_time), 0));
            const iptime = (ipViewReport?.filter(val => val.qi_save_status === 1).reduce((acc, curr) => acc + (curr.sumof_service_time), 0));
            ...
            const analysedSentineltot = ((viewData?.filter((val) => val.sentinel_analysed === 1)).length) +
            ...
            // equipServiceTime
            const opequipTime = (viewData?.filter(val => val.qi_save_status === 1).reduce((acc, curr) => acc + (curr.equip_service_time), 0));
            const ipequipTime = (ipViewReport?.filter(val => val.qi_save_status === 1).reduce((acc, curr) => acc + (curr.equip_service_time), 0));
    ...
    useEffect(() => {
        if (viewData.length !== 0) {
            ...
            ExistApprvView(postdata).then((val) => {
                ...
                if (success === 1) {
                    const { endo_incharge_apprv_status, endo_incharge_remarks, endo_Incharge_apprv_date, inchrge,
                        inchargeno, endo_hod_apprv_date, hod, hodno, endo_hod_apprv_status } = data[0]
                    setInchrgeStatus(endo_incharge_apprv_status)
                    setRemarks(endo_incharge_apprv_status === 1 ? endo_incharge_remarks : 'nil')
                    setLastApprvdDate(format(new Date(endo_Incharge_apprv_date), 'dd-MM-yyyy hh:mm:ss a'))
                    setempName(inchrge + '  (' + inchargeno + ')')
                    const fromdata = {
                        hodName: hod + '  (' + hodno + ')',
    ...
    }, [searchDate, qidept, viewData, count])
    ...
    const ViewErrorDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...errorData, ...ipErrorData].sort((a, b) => new Date(a.incident_error_date) - new Date(b.incident_error_date));
    ...
    const ViewRedosDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...errorData, ...ipErrorData].sort((a, b) => new Date(a.incident_redos_date) - new Date(b.incident_redos_date));
                setTableData(finalData)
            } else {
                setTableData(errorData)
            }
    ...
    const ViewWaitingTimeDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...AssesmentData, ...ipAssesmentData].sort((a, b) => new Date(a.endo_arrival_time) - new Date(b.endo_arrival_time));
                setTableData(finalData)
            } else {
    ...
    const ViewIdentifictionErrorDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...errorData, ...ipErrorData].sort((a, b) => new Date(a.incidence_ident_date) - new Date(b.incidence_ident_date));
                setTableData(finalData)
            } else {
                setTableData(errorData)
            }
    ...
    const ViewFallsDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...errorData, ...ipErrorData].sort((a, b) => new Date(a.incident_falls_date) - new Date(b.incident_falls_date));
                setTableData(finalData)
            } else {
                setTableData(errorData)
            }
    ...
    const ViewSentinelDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...errorData, ...ipErrorData].sort((a, b) => new Date(a.incident_sentinel_date) - new Date(b.incident_sentinel_date));
                setTableData(finalData)
            } else {
                setTableData(errorData)
            }
    ...
    const ViewNearMissessDetails = useCallback(() => {
        ...
        if (opData.length !== 0) {
            ...
            if (ipdata.length !== 0) {
                ...
                const finalData = [...errorData, ...ipErrorData].sort((a, b) => new Date(a.incident_nearmisses_date) - new Date(b.incident_nearmisses_date));
                setTableData(finalData)
            } else {
                setTableData(errorData)
            }
    ...
    const SaveDetails = useCallback(() => {
        ...
        else if (hodStatus === 1 && inchrgeStatus === 0) {
            infoNotify("Level II Approval Done, so You Can't Approve")
        } else {
            const saveData = async (postdata) => {
                const result = await axioslogin.post('/qiendoscopy/inchrgeapprv', postdata);
                return result.data
            }
    ...
    }, [remarks, postdata, reset, count, hodStatus, inchrgeStatus])
    ...
    return (
        <Box>
            ...
            {viewFlag === 1 ? <MonthlyReportModal open={modalopen} handleClose={handleClose} tableData={tableData} headerNames={headerNames} />
                : null}

            <Box variant="outlined" sx={{
                overflow: 'auto', maxHeight: window.innerHeight - 270, padding: 'none',
                border: '1px solid lightgrey', px: 0.5
            }}>
                {endoSearch === 1 ?
                    <Grid container spacing={0.5} sx={{ flexGrow: 1, py: 0.5 }}>
                        <Grid xs={5}>
                            <Paper sx={{ bgcolor: 'white' }}>
                                <Box sx={{
                                    fontSize: 12, textTransform: 'uppercase', py: 0.5, fontWeight: 650, pl: 1,
                                    color: '#555830', bgcolor: '#E4E5E8'
                                }}>
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                        <Box sx={{ flex: 1 }}>
                                            <Box sx={{ display: 'flex', pt: 0.5 }}>
                                                ...
                                                <Box sx={{ flex: 0.1, p: 0.2, fontWeight: 650, display: 'flex', justifyContent: 'flex-end' }}>
                                                    :
                                                </Box>
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 1 }} >
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                    startDecorator={<ErrorOutlineOutlinedIcon sx={{ color: '#E55B13', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 1 }} >
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                    startDecorator={<VaccinesIcon sx={{ color: '#778A35', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 1 }} >
                                            <CssVarsProvider>
                                                <Tooltip title="Time Exceedence Patients Report" placement='bottom'>
                                                    <Button variant="outlined" color="neutral" size="sm"
                                                        sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                        startDecorator={<TimerOutlinedIcon sx={{ color: '#827717', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 1 }} >
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                    startDecorator={<ReportGmailerrorredOutlinedIcon sx={{ color: '#E55B13', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 1 }} >
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                    startDecorator={<FmdBadOutlinedIcon sx={{ color: '#E55B13', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 1 }} >
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                    startDecorator={<NearbyErrorIcon sx={{ color: '#E55B13', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 2 }} >
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                    startDecorator={<RunningWithErrorsOutlinedIcon sx={{ color: '#E55B13', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                        ...
                                        <Box sx={{ flex: 0.1, pt: 4, pr: 2 }} >
                                            <CssVarsProvider>
                                                <Button disabled color="danger" variant="plain"
                                                    sx={{ height: 30, width: 100, display: 'flex', justifyContent: 'flex-start' }}>
                                                </Button>
                                            </CssVarsProvider>
                                        </Box>
                                    </Box>
                        ...
                        <Grid xs={12} sx={{}}>
                            {(hodStatus === 1 && inchrgeStatus === 0) ?
                                <Paper sx={{ bgcolor: 'white', height: 110 }}>
                                    ...
                                    <Box sx={{ display: 'flex', pb: 2, pl: 2, flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ pr: 1.5, pt: 2 }}>
                                            <Typography sx={{ fontSize: 13, color: '#555830' }}>{hodName}</Typography>
                                        </Box>
                                        <Box sx={{ fontSize: 16, pl: 1, color: '#555830', fontWeight: 550, pr: 1.5, pt: 1.8 }}>
                                            Approved Date :
                                        </Box>
                                    ...
                                    <Box sx={{ display: 'flex', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                                        ...
                                        <Box sx={{ py: 1, pr: 2 }}>
                                            <CssVarsProvider>
                                                {inchrgeStatus === 0 ?
                                                    <Button variant="outlined" color="neutral" size="sm"
                                                        sx={{ height: 30, width: 120, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                        startDecorator={<SaveIcon sx={{ color: '#0E86D4', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                                        onClick={SaveDetails}
                                                    >Approve
                                                    </Button>
                                                    :
                                                    <Button variant="outlined" color="neutral" size="sm" disabled
                                                        sx={{ height: 30, width: 120, display: 'flex', justifyContent: 'flex-start', borderRadius: 7 }}
                                                        startDecorator={<SaveIcon sx={{ color: '#bbdefb', cursor: 'pointer', height: 20, width: 20 }} fontSize='large' />}
                                                    >Approved
                                                    </Button>}
                                            </CssVarsProvider>
                                        </Box>
...
export default memo(QiApprovalView)