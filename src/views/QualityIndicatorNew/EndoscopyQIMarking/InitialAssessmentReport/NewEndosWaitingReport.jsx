...
const NewEndosWaitingReport = ({ viewData, ipViewReport, opCheck, ipCheck }) => {
    ...
    return (
        <Box sx={{ pt: 1 }}>
            {tableData.length !== 0 ?
                <Box variant="plain" sx={{ overflow: 'auto', maxHeight: window.innerHeight - 220, padding: 'none', '&::-webkit-scrollbar': { height: 8 } }}>
                    <CssVarsProvider>
                        <Table aria-label="table with sticky header" padding={"none"} borderAxis='none' stickyHeader size='sm' stickyFooter hoverRow >
                            <thead style={{ alignItems: 'center' }}>
                                <tr style={{ height: 0.5, borderRadius: 0, }}>
                                    <th size='sm' style={{ width: 50, backgroundColor: '#e0e0e0', textAlign: 'center', fontSize: 13 }}>Sl.No</th>
                                    <th size='sm' style={{ width: 100, backgroundColor: '#e0e0e0', fontSize: 13 }}>Patient ID</th>
                                    <th size='sm' style={{ width: 150, backgroundColor: '#e0e0e0', fontSize: 13 }}>Patient Name</th>
                                    <th size='sm' style={{ width: 150, backgroundColor: '#e0e0e0', fontSize: 13 }}>Arrival Time</th>
                                    {opCheck === true ? <th size='sm' style={{ width: 170, backgroundColor: '#e0e0e0', fontSize: 13 }}>Assessment StartTime</th> : null}
                                    <th size='sm' style={{ width: 170, backgroundColor: '#e0e0e0', fontSize: 13 }}>Assessment EndTime</th>
                                    <th size='sm' style={{ width: 170, backgroundColor: '#e0e0e0', fontSize: 13 }}>Procedure StartTime</th>
                                    <th size='sm' style={{ width: 170, backgroundColor: '#e0e0e0', fontSize: 13 }}>Procedure EndTime</th>
                                    {opCheck === true ? <th size='sm' style={{ width: 150, backgroundColor: '#e0e0e0', fontSize: 13 }}>Pre Assessment Time</th> : null}
                                    <th size='sm' style={{ width: 130, backgroundColor: '#e0e0e0', fontSize: 13 }}>Assessment Time</th>
                                    <th size='sm' style={{ width: 130, backgroundColor: '#e0e0e0', fontSize: 13 }}>Procedure Time</th>
                                    <th size='sm' style={{ width: 120, backgroundColor: '#e0e0e0', fontSize: 13 }}>Total Time</th>
                                </tr>
                            </thead>
                                ...
                                {tableData?.map((val, index) => {
                                    return (
                                        < tr key={index} size='small' style={{ maxHeight: 2, cursor: 'pointer' }}>
                                            <td size='sm' style={{ textAlign: 'center', fontSize: 12 }}>{index + 1}</td>
                                            <td size='sm' style={{ fontSize: 12 }}>{val.ptno}</td>
                                            <td size='sm' style={{ fontSize: 12, textTransform: 'capitalize' }}>
                                                {val.ptname.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</td>
                                            <td size='sm' style={{ fontSize: 12 }}>{val.patient_arrived_date}</td>
                                            {opCheck === true ? <td size='sm' style={{ fontSize: 12 }}>{val.assessment_start}</td> : null}
                                            <td size='sm' style={{ fontSize: 12 }}>{val.assessment_end}</td>
                                            <td size='sm' style={{ fontSize: 12 }}>{val.proc_start_time}</td>
                                            <td size='sm' style={{ fontSize: 12 }}>{val.proc_end_time}</td>
                                            {opCheck === true ? <td size='sm' style={{ fontSize: 12 }}>{val.start_time_diff}</td> : null}
                                            <td size='sm' style={{ fontSize: 12 }}>{val.assess_time_diff}</td>
                                            <td size='sm' style={{ fontSize: 12 }}>{val.proc_time_diff}</td>
                                            <td size='sm' style={{ fontSize: 12 }}>{val.arrival_to_ReportTime}</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
