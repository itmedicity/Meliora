...
const EndoscopyIPPatients = ({ ChangeOPList, ChangeIPList, opCheck, ipCheck, setSearchFlag, qidept, depName
    ...
    return (
        <Box>
                ...
                <Box sx={{ flex: 0.7, pl: 1 }}>
                    <CssVarsProvider>
                        <Tooltip title="Search" placement='right'>
                            <SearchIcon sx={{ cursor: 'pointer', height: 35, width: 35 }}
            ...
            {tabledata.length !== 0 ?
                <Box variant="outlined" sx={{ overflow: 'auto', minHeight: window.innerHeight - 220, '&::-webkit-scrollbar': { height: 8 } }}>
                    <CssVarsProvider>
                        <Table aria-label="table with sticky header" borderAxis="both" padding={"none"} stickyHeader size='sm' stickyFooter hoverRow >
                            <thead style={{ alignItems: 'center' }}>
                                <tr style={{ height: 0.5 }}>
                                    ...
                                    <th size='sm' style={{ width: 150, backgroundColor: '#78909c', color: 'white', fontSize: 14 }}>&nbsp;IP Date</th>
                                    ...
                                    <th size='sm' style={{ width: 150, backgroundColor: '#78909c', color: 'white', fontSize: 14 }}>&nbsp;Nursing Station </th>
                            ...
                            <tbody size='small'>
                                {tabledata?.map((val, index) => {
                                    return (< tr key={val.qi_endo_ip_slno} size='small'
                                        style={{
                                        ...
                                        <td size='sm' style={{ fontSize: 12, height: 5 }}>&nbsp;{val.ip_no}</td>
                                        <td size='sm' style={{ fontSize: 12, height: 5 }}>&nbsp;{format(new Date(val.ipd_date), 'dd-MM-yyyy hh:mm a')}</td>
