...
const EndoscopyModalForQI = ({ open, handleClose, rowSelect, count, setCount, depName, qidept, RefreshData, setQiflag, WaitingTimeData }) => {
    ...
    const OnchangeEntryTime = useCallback((value) => {
        const check = isAfter(new Date(value), new Date(patient_arrived_date))
        if (check === true) {
            setEntryTime(value)
        } else {
            warningNotify("Check the Entry Time")
            setEntryTime(addMinutes(new Date(patient_arrived_date), 30))
        }
    }, [patient_arrived_date])
    const OnchangeAssessTime = useCallback((value) => {
        const check = isAfter(new Date(value), new Date(entryTime))
        if (check === true) {
            setAssessmentTime(value)
        } else {
            warningNotify("Check the Assessment Time")
            setAssessmentTime(addMinutes(new Date(patient_arrived_date), 40))
        }
    }, [entryTime, patient_arrived_date])
    const OnchangeProcStartTime = useCallback((value) => {
        const check = isAfter(new Date(value), new Date(assessmentTime))
        if (check === true) {
            setStartTime(value)
        } else {
            warningNotify("Check the Procedure Start Time")
            setStartTime(addMinutes(new Date(patient_arrived_date), 45))
        }
    }, [assessmentTime, patient_arrived_date])

    const OnchangeProcEndTime = useCallback((value) => {
        const check = isAfter(new Date(value), new Date(startTime))
        if (check === true) {
            setEndTime(value)
        } else {
            warningNotify("Check the Procedure End Time")
            setEndTime(addMinutes(new Date(patient_arrived_date), 60))
        }
    }, [startTime, patient_arrived_date])

    const OnchangeGenerateTime = useCallback((value) => {
        const check = isAfter(new Date(value), new Date(endTime))
        if (check === true) {
            setReportTime(value)
        } else {
            warningNotify("Check the Report Generate Time")
            setReportTime(addMinutes(new Date(patient_arrived_date), 70))
        }
    }, [endTime, patient_arrived_date])

    const OnchangeDespatchTime = useCallback((value) => {
        ...
        if (check === true) {
            ...
            setDespatchTime(addMinutes(new Date(patient_arrived_date), 80))
        }
    }, [reportTime, patient_arrived_date])

    const OnchangeEquipStartTime = useCallback((value) => {
        ...
        if (check === true) {
            ...
            setEquipStartTime(addMinutes(new Date(patient_arrived_date), 45))
        }
    }, [equipEndTime, patient_arrived_date])

    const OnchangeEquipEndTime = useCallback((value) => {
        ...
        if (check === true) {
            ...
            setEquipEndTime(addMinutes(new Date(patient_arrived_date), 60))
        }
    }, [equipStartTime, patient_arrived_date])
    ...
    useEffect(() => {
        const getequipDetails = async (qi_slno) => {
            const result = await axioslogin.get(`/qiendoscopy/equip/${qi_slno}`)
            return result.data
        }
    ...
    const SaveDetails = useCallback(() => {
        if (errorYes === true && errorDetails === '' && errorReason === '') {
            ...
            const deleteequipDetails = async (qi_slno) => {
                const result = await axioslogin.delete(`/qiendoscopy/delete/${qi_slno}`)
                return result.data
            }
            UpdateData(patchdata).then((value) => {
                ...
                if (success === 1) {
                    ...
                    if (equipmentExit === 1) {
                        deleteequipDetails(qi_slno).then((value) => {
                            const { success } = value
                            if (success === 1) {
        ...
        <Fragment>
            {incFlag === 1 ?
                <IncidentModal open={incmodalopen} setIncModalOpen={setIncModalOpen} depName={depName} qidept={qidept}
                    ...
                    setErrorType={setErrorType} setRedosType={setRedosType} setIdentType={setIdentType} setFallsType={setFallsType}
                    setSentinelType={setSentinelType} setnearType={setnearType} ipOpCheck={ipOpCheck}
                /> : null}
                    ...
                    <Box sx={{ width: '100%', overflow: 'auto' }}>
                        ...
                        <Box sx={{}}>
                                        ...
                                        <Box sx={{ pl: 2, pt: 0.2 }}>
                                            <CssVarsProvider>
                                                <Button variant="outlined" color="neutral" size="sm"
                                                    sx={{ height: 20, width: 150, display: 'flex', justifyContent: 'flex-start' }}
                                                    startDecorator={< AddCircleOutlineIcon sx={{ height: 20, width: 20, color: '#970C10', cursor: 'pointer' }}
                                                        fontSize='large' />}
                                                ...
                                                < Box sx={{ display: 'flex', pt: 0.5, pl: 0.5 }}>
                                                    ...
                                                    <Box sx={{ pl: 1.5 }}>
                                                        <Typography sx={{ fontSize: 11, borderRadius: 1 }}>
                                                            {errorType === 1 ? 'GENERAL' : errorType === 2 ? 'NEAR MISSESS' :
                                                                errorType === 3 ? 'HARMFUL' : errorType === 4 ? 'SENTINEL' : 'Nil'}
                                                        </Typography>
                                                    </Box>
                                                </Box>
                                        ...
                                        {redosType !== 0 ?
                                            <>
                                                <Box sx={{ display: 'flex', pt: 0.5, pl: 0.5 }}>
                                                    ...
                                                    <Box sx={{ pl: 1.5 }}>
                                                        <Typography sx={{ fontSize: 11, borderRadius: 1 }}>
                                                            {redosType === 1 ? 'GENERAL' : redosType === 2 ? 'NEAR MISSESS' :
                                                                redosType === 3 ? 'HARMFUL' : redosType === 4 ? 'SENTINEL' : 'Nil'}</Typography>
                                                    </Box>
                                                </Box>
                                                <Box sx={{ display: 'flex', px: 0.5 }}>
                                                    <Box sx={{ flex: 1 }}>
                            ...
                            <Box sx={{ display: 'flex', pt: 0.5 }}>
                                <Box sx={{ flex: 2, pl: 1, pt: 1.3 }}>
                                    <Typography sx={{ fontSize: 12, fontWeight: 550, textTransform: 'uppercase' }}>
                                        Incidence of Patient Identification Error</Typography>
                                </Box>
                                <Box sx={{ pt: 1 }}>:</Box>
                                <Box sx={{ flex: 1, display: 'flex', pl: 1 }}>
                                    <Box sx={{ pt: 1.2 }}>
                                        ...
                                        {identType !== 0 ?
                                            <>
                                                <Box sx={{ display: 'flex', pt: 0.5, pl: 0.5 }}>
                                                    ...
                                                    <Box sx={{ pl: 1.5 }}>
                                                        <Typography sx={{ fontSize: 11, borderRadius: 1 }}>
                                                            {identType === 1 ? 'GENERAL' : identType === 2 ? 'NEAR MISSESS' :
                                                                identType === 3 ? 'HARMFUL' : identType === 4 ? 'SENTINEL' : 'Nil'}
                                                        </Typography>
                                                    </Box>
                                                </Box>
                                                <Box sx={{ display: 'flex', px: 0.5 }}>
                                        ...
                                        {fallsType !== 0 ?
                                            <>
                                                <Box sx={{ display: 'flex', pt: 0.5, pl: 0.5 }}>
                                                    ...
                                                    <Box sx={{ pl: 1.5 }}>
                                                        <Typography sx={{ fontSize: 11, borderRadius: 1 }}>
                                                            {fallsType === 1 ? 'GENERAL' : fallsType === 2 ? 'NEAR MISSESS' :
                                                                fallsType === 3 ? 'HARMFUL' : fallsType === 4 ? 'SENTINEL' : 'Nil'}
                                                        </Typography>
                                                    </Box>
                                                </Box>
                                                <Box sx={{ display: 'flex', px: 0.5 }}>
                                        ...
                                        {sentinelType !== 0 ?
                                            <>
                                                <Box sx={{ display: 'flex', pt: 0.5, pl: 0.5 }}>
                                                    ...
                                                    <Box sx={{ pl: 1.5 }}>
                                                        <Typography sx={{ fontSize: 11, borderRadius: 1 }}>
                                                            {sentinelType === 1 ? 'GENERAL' : sentinelType === 2 ? 'NEAR MISSESS' :
                                                                sentinelType === 3 ? 'HARMFUL' : sentinelType === 4 ? 'SENTINEL' : 'Nil'}
                                                        </Typography>
                                                    </Box>
                                                </Box>
                                                <Box sx={{ display: 'flex', px: 0.5 }}>
                            ...
                            <Box sx={{ display: 'flex', pt: 0.5 }}>
                                <Box sx={{ flex: 2, pl: 1, pt: 1.3 }}>
                                    <Typography sx={{ fontSize: 12, fontWeight: 550, textTransform: 'uppercase' }}>
                                        Near Misses Reported</Typography>
                                </Box>
                                <Box sx={{ pt: 1 }}>:</Box>
                                <Box sx={{ flex: 1, display: 'flex', pl: 1 }}>
                                    <Box sx={{ pt: 1.2 }}>
                                        ...
                                        {nearType !== 0 ?
                                            <>
                                                <Box sx={{ display: 'flex', pt: 0.5, pl: 0.5 }}>
                                                    ...
                                                    <Box sx={{ pl: 1.5 }}>
                                                        <Typography sx={{ fontSize: 11, borderRadius: 1 }}>
                                                            {nearType === 1 ? 'GENERAL' : nearType === 2 ? 'NEAR MISSESS' :
                                                                nearType === 3 ? 'HARMFUL' : nearType === 4 ? 'SENTINEL' : 'Nil'}
                                                        </Typography>
                                                    </Box>
                                                </Box>
                                                <Box sx={{ display: 'flex', px: 0.5 }}>
                            ...
                            <Box sx={{}}>
                                ...
                                <Box sx={{ display: 'flex', m: 0.5, bgcolor: '#eceff1', pb: 0.5, flex: 1 }}>
                                        ...
                                        <Box sx={{ display: 'flex', justifyContent: 'center', pt: 0.1, pl: 1 }}>
                                            {benchMarkFlag === 1 ?
                                                <Box sx={{
                                                    ...
                                                    border: '1px solid lightgrey', pl: 2, borderRadius: 1.5
                                                }}>
                                                    {sumOfAssesment}
                                                </Box>
                                                : <Box sx={{
                                    ...
                                    <Box sx={{ flex: 1 }}>
                                        ...
                                        <Box sx={{ display: 'flex', justifyContent: 'center', pt: 0.1 }}>
                                            <Box sx={{
                                                flex: 1, bgcolor: 'white', height: 38, mr: 1, pt: 0.8,
                                                border: '1px solid lightgrey', pl: 2, borderRadius: 1.5
                                            }}>
                                                {turnaroundTime}
                                            </Box>
                            ...
                            <Box sx={{ display: 'flex', mx: 0.5, bgcolor: '#eceff1', flex: 1, pb: 0.5 }}>
                                <Box sx={{ flex: 1 }}>
                                    ...
                                    <Box sx={{ ml: 1, mr: 0.6, pt: 0.8, bgcolor: 'white' }}>
                                        <QIEquipmentSelect equipment={equipment} setEquipment={setEquipment} setProcName={setProcName}
                                            setProcedureList={setProcedureList} setequipName={setequipName} />
                                    </Box>
                                </Box>
                                <Box sx={{ flex: 1 }}>
                                    <Box sx={{ pl: 1.5, pt: 0.5 }}>
                                    ...
                                    <Box sx={{ mx: 1, pt: 0.8, bgcolor: 'white' }}>
                                        <QIProcedureSelect procName={procName} setProcName={setProcName}
                                            procedureList={procedureList} setProcNamedisplay={setProcNamedisplay} />
                                    </Box>
                                </Box>
