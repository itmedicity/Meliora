      ...
      if (WaitingTimeData.length !== 0) {
        ...
        setReportTime(
          report_gene_time === null
            ? format(addMinutes(new Date(END_DATE), 30), 'yyyy-MM-dd HH:mm:ss')
            : report_gene_time
        )
        setDespatchTime(
          report_desp_time === null
            ? format(addMinutes(new Date(END_DATE), 40), 'yyyy-MM-dd HH:mm:ss')
            : report_desp_time
        )
        setEquipStartTime(
        ...
        setEntryTime(
          endo_arrival_time === null
            ? format(addMinutes(new Date(patient_arrived_date), 30), 'yyyy-MM-dd HH:mm:ss')
            : endo_arrival_time
        )
        setAssessmentTime(
          assessment_time === null
            ? format(addMinutes(new Date(patient_arrived_date), 40), 'yyyy-MM-dd HH:mm:ss')
            : assessment_time
        )
        setStartTime(
          proc_start_time === null
            ? format(addMinutes(new Date(patient_arrived_date), 45), 'yyyy-MM-dd HH:mm:ss')
            : proc_start_time
        )
        setEndTime(
          proc_end_time === null
            ? format(addMinutes(new Date(patient_arrived_date), 60), 'yyyy-MM-dd HH:mm:ss')
            : proc_end_time
        )
        setReportTime(
          report_gene_time === null
            ? format(addMinutes(new Date(patient_arrived_date), 70), 'yyyy-MM-dd HH:mm:ss')
            : report_gene_time
        )
        setDespatchTime(
          report_desp_time === null
            ? format(addMinutes(new Date(patient_arrived_date), 80), 'yyyy-MM-dd HH:mm:ss')
            : report_desp_time
        )
        setEquipStartTime(
          equip_start_time === null
    ...
    <Fragment>
      {incFlag === 1 ? (
        <IncidentModal
          ...
          patient_arrived_date={patient_arrived_date}
