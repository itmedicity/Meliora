...
const ProcedureEquipmentTable = ({ ProcedureArray, setProcedureArray, equipReport }) => {
    ...
    const DeleteProcedures = useCallback((val) => {
                ...
                <Table aria-label="table with sticky header" borderAxis="both" padding={"none"} stickyHeader size='sm' stickyFooter hoverRow >
                    <thead style={{ alignItems: 'center' }}>
                        <tr style={{ height: 0.5 }}>
                            ...
                            <th size='sm' style={{ width: 200, fontWeight: 650, fontSize: 14 }}>&nbsp;Equipment</th>
                            <th size='sm' style={{ width: 200, fontWeight: 650, fontSize: 14 }}>&nbsp;Procedure Name</th>
                    ...
                    <tbody size='small'>
                        {ProcedureArray?.map((val, index) => {
                            return (< tr key={index} size='small'
                                ...
                                <td size='sm' style={{ fontSize: 12, height: 5 }}>&nbsp;{val.equip_name}</td>
                                <td size='sm' style={{ fontSize: 12, height: 5 }}>&nbsp;{val.PDC_DESC}</td>
                                {equipReport === 0 ? <td size='sm' style={{ textAlign: 'center', height: 5 }}>
                                    <CssVarsProvider>
